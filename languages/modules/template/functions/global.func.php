 ...
 * @param $data 数据
 */
function creat_shy_tag($op, $data) {
	$str = '{pc:'.$op.' ';
	if (is_array($data)) {
		foreach ($data as $k=>$v) {
 ...
 * @param $dir 目录名
 */
function replace_shy_tag($filepath, $old_tag, $new_tag, $style, $dir) {
...
function creat_template_bak($filepath, $style, $dir) {
	...
	$template_bak_db = shy_base::load_model('template_bak_model');
...
function creat_form($id, $data, $value = '', $op = '') {
	...
	if($data['ajax']['name']) {
		if($data['ajax']['m']) {
			$url = '$.get(\'?app=content&controller=push&view=public_ajax_get\', {html: this.value, id:\''.$data['ajax']['id'].'\', action: \''.$data['ajax']['action'].'\', module: \''.$data['ajax']['m'].'\', safe_edi: \''.$_SESSION['safe_edi'].'\'}, function(data) {$(\'#'.$id.'_td\').html(data)});';
		} else {
			$url = '$.get(\'?app=template&controller=file&view=public_ajax_get\', { html: this.value, id:\''.$data['ajax']['id'].'\', action: \''.$data['ajax']['action'].'\', op: \''.$op.'\', style: \'default\', safe_edi: \''.$_SESSION['safe_edi'].'\'}, function(data) {$(\'#'.$id.'_td\').html(data)});';
		}
	}
...
function visualization($html, $style = '', $dir = '', $file = '') {
	$change = "<link href=\"".CSS_PATH."dialog.css\" rel=\"stylesheet\" type=\"text/css\" />
		...
		var safe_edi = '".$_SESSION['safe_edi']."';
		$(function(){
		$('a').attr('href', 'javascript:void(0)').attr('target', '');
		$('.admin_piao_edit').click(function(){
		var url = '?app=template&controller=file&view=edit_shy_tag';
		if($(this).parent('.admin_piao').attr('pc_action') == 'block') url = '?app=block&controller=block_admin&view=add';
		window.top.art.dialog({title:'".L('shy_tag','' ,'template')."',id:'edit',iframe:url+'&style=$style&dir=$dir&file=$file&'+$(this).parent('.admin_piao').attr('data'),width:'700',height:'500'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;d.document.getElementById('dosubmit').click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});})
		$('.admin_block').click(function(){
			window.top.art.dialog({title:'".L('shy_tag','' ,'template')."',id:'edit',iframe:'?app=block&controller=block_admin&view=block_update&id='+$(this).attr('blockid'),width:'700',height:'500'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;d.document.getElementById('dosubmit').click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
		});
