...
?>
<div class="pad-lr-10">
<?php include SHUYANG_PATH.'star_config.php';?>
...
<table width="100%" cellspacing="0" class="search-form">
 ...
 <table width="100%" cellspacing="0" class="search-form">
    <tbody>
		...
		&nbsp;&nbsp;<a href="<?php echo APP_PATH;?>index.php?app=dianping&controller=dianping&view=dianping_data&menuid=<?php echo $_GET['menuid']?>"><font color=red>全部</a></a>	
	<?php
	if(is_array($dianping_type_array)){
		foreach($dianping_type_array as $type=>$type_array){
	?>
	&nbsp;&nbsp;
<a href="<?php echo APP_PATH;?>index.php?app=dianping&controller=dianping&view=dianping_data&menuid=<?php echo $_GET['menuid'];?>&safe_edi=G6Ihra&typeid=<?php echo $type;?>"><?php echo $type_array['type_name'];?></a>	
	<?php }}?>
		</div>
		</td>
		</tr>
 ...
 <input type="hidden" name="app" value="reviews">
  <input type="hidden" name="controller" value="check">
   <input type="hidden" name="view" value="ajax_checks">
...
function check(id, type, dianpingid) {
	...
	$.get('?app=dianping&controller=dianping&view=ajax_checks&id='+id+'&type='+type+'&dianpingid='+dianpingid+'&'+Math.random()+'&safe_edi=<?php echo $_SESSION['safe_edi'];?>', function(data){if(data!=1){if(data==0){alert('<?php echo L('illegal_parameters')?>')}else{alert(data)}}else{$('#tbody_'+id).remove();}});
}
</script>
</body>
</html>
