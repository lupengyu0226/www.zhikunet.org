...
<form name="form" action="?app=dianping&controller=dianping&view=dianping_data&menuid=<?php echo $_GET['menuid'];?>" method="get" >
<input type="hidden" value="dianping" name="app">
<input type="hidden" value="dianping" name="controller">
<input type="hidden" value="dianping_data" name="view">
...
<form name="myform" id="myform" action="?app=dianping&controller=dianping&view=delete_dianping_type" method="post" >
...
<input type="button" class="button" value="<?php echo L('update_dianpingtype')?>" onClick="javascript:location.href='?app=dianping&controller=dianping&view=do_js&safe_edi=<?php echo $_SESSION['safe_edi']?>'">
</div>
...
function call(id) {
	...
	window.top.art.dialog({title:'<?php echo L('call_code')?>', id:'call', iframe:'?app=dianping&controller=dianping&view=public_call&typeid='+id, width:'600px', height:'300px'}, function(){window.top.art.dialog({id:'call'}).close();}, function(){window.top.art.dialog({id:'call'}).close();})
}

function edit_type(id, name) {
	window.top.art.dialog({id:'edit'}).close();
	window.top.art.dialog({title:'修改 '+name+' ',id:'edit',iframe:'?app=dianping&controller=dianping&view=edit_type&dianpingid='+id,width:'550',height:'350'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
} 
</script>
