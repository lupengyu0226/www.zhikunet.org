...
shy_base::load_app_func('global','dianping');//导入程序处理函数
class dianping extends admin {
	function __construct() {
		parent::__construct();
		$this->dianping_type = shy_base::load_model('dianping_type_model');
		$this->dianping_data = shy_base::load_model('dianping_data_model');
		$this->dianping = shy_base::load_model('dianping_model');
		$this->module = shy_base::load_model('module_model');
	...
	public function init() {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=dianping&controller=dianping&view=add_type\', title:\'添加点评类型\', width:\'500\', height:\'350\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', '添加点评类型');
	...
	/**
 	...
 	public function dianping_data() {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=dianping&controller=dianping&view=add_type\', title:\'添加点评类型\', width:\'500\', height:\'250\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', '添加点评类型');
 	...
 	public function ajax_checks(){
 		...
 		$result = $this->delete_dianpingdata_update($id);
  		if($result == 1){
 			...
 			$member_user_db = shy_base::load_app_class('member_interface','member');
	...
	public function delete_dianping_type() {
  		if(!isset($_POST['dianpingid']) || empty($_POST['dianpingid'])) {
			...
			if(is_array($_POST['dianpingid'])){
				...
				showmessage(L('operation_success'),'?app=dianping&controller=dianping');
			}
		} 
	}
	...
	public function edit_type() {
		if(isset($_POST['dosubmit'])){ 
			...
			setcache('dianping_type', $type_cache_array, 'dianping');
 			showmessage(L('operation_success'),'?app=dianping&controller=dianping&view=init','', 'edit');
 		}else{
 			$show_validator = $show_scroll = $show_header = true;
			//解出分类内容
