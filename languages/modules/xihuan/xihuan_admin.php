...
class xihuan_admin extends admin {
	...
	public function init() {
		$xihuan_program = getcache('xihuan_program', 'commons');
		$xihuan_program = isset($xihuan_program[$this->get_siteid()]) ? $xihuan_program[$this->get_siteid()] : array();
		...
		if ($catid) {
			...
			$data = $xihuan_db->listinfo($sql, $sql_order, $page);
			...
			$pages = $xihuan_db->pages;
		}
		$order_list = array('-1'=>L('total'));
		foreach ($xihuan_program as $k=>$v) {
			$order_list[$k]=$v['name'];
		}
		shy_base::load_sys_class('form', '', 0);
		include $this->admin_tpl('xihuan_list');
	}
	...
	public function setting() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$xihuan_program[$this->get_siteid()] = $data;
			setcache('xihuan_program', $xihuan_program, 'commons');
			showmessage(L('operation_success'), HTTP_REFERER);
		} else {
			$xihuan_program = isset($xihuan_program[$this->get_siteid()]) ? $xihuan_program[$this->get_siteid()] : array();
			include $this->admin_tpl('xihuan_setting');
		}
	}
}
