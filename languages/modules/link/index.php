...
class index {
	...
	public function register() { 
 		...
 		if(isset($_POST['dosubmit'])){
 			if($_POST['name']==""){
 				showmessage(L('sitename_noempty'),"?app=link&controller=index&view=register&siteid=$siteid");
 			}
 			if($_POST['url']=="" || !preg_match('/^http[s]?:\/\/(.*)/i', $_POST['url'])){
 				showmessage(L('siteurl_not_empty'),"?app=link&controller=index&view=register&siteid=$siteid");
 			}
 			if(!in_array($_POST['linktype'],array('0','1'))){
 				$_POST['linktype'] = '0';
 			}
 			$link_db = shy_base::load_model(link_model);
			...
			$url = trim_script($url);
 			...
 			$link_db->insert($sql);
 			showmessage(L('申请已经提交，请做好与本站的链接方可审核通过'), "?app=link&controller=index&siteid=$siteid",7000);
 		} else {
  			$setting = getcache('link', 'commons');
