...
<form name="myform" id="myform" action="?app=link&controller=link&view=listorder" method="post" >
<div class="table-list">
<table width="100%" cellspacing="0">
	<thead>
		<tr>
			<th width="15"><input type="checkbox" value="" id="check_box" onclick="selectall('linkid[]');"></th>
			<th width="15"><?php echo L('listorder')?></th>
			<th width="45" align="center"><?php echo L('link_name')?></th>
			<th width="45" align="center"><?php echo L('logo')?></th>
			<th width="45" align="center"><?php echo L('typeid')?></th>
			<th width='45' align="center"><?php echo L('link_type')?></th>
			<th width="45" align="center"><?php echo L('status')?></th>
			<th width="45" align="center"><?php echo L('robots')?></th>
			<th width="45" align="center"><?php echo L('addtime')?></th>
			<th width="45" align="center"><?php echo L('operations_manage')?></th>
		</tr>
	</thead>
<tbody>
<?php
if(is_array($infos)){
	...
	<tr>
		...
		<td width="8%" align="center"><?php if($info['passed']=='0'){?><a
			href='?app=link&controller=link&view=check&linkid=<?php echo $info['linkid']?>'
			onClick="return confirm('<?php echo L('pass_or_not')?>')"><font color=red><?php echo L('audit')?></font></a><?php }else{echo L('passed');}?></td>
			<td align="center"><?php echo $info['robots']==0 ? L('yes') : L('no')?></td>
		...
		<td align="center" width="12%"><a class="xbtn btn-info btn-xs"  href="###"
			...
			href='?app=link&controller=link&view=delete&linkid=<?php echo $info['linkid']?>'
...
<input name="dosubmit" type="submit" class="button"
	value="<?php echo L('listorder')?>">&nbsp;&nbsp;<input type="submit" class="button" name="dosubmit" onClick="document.myform.action='?app=link&controller=link&view=delete'" value="<?php echo L('delete')?>"/></div>
...
function listorder_up(id) {
	$.get('?app=link&controller=link&view=listorder_up&linkid='+id,null,function (msg) { 
	if (msg==1) { 
