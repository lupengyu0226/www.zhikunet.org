...
class link extends admin {
	function __construct() {
		...
		$this->db = shy_base::load_model('link_model');
	...
	public function init() {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=link&controller=link&view=add\', title:\''.L('link_add').'\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('link_add'));
	...
	public function public_name() {
		$link_title = isset($_GET['link_name']) && trim($_GET['link_name']) ? (shy_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['link_name'])) : trim($_GET['link_name'])) : exit('0');
 	...
 	public function add() {
 		if(isset($_POST['dosubmit'])) {
			...
			if(!$linkid) return FALSE; 
	 		...
	 		//更新附件状态
			if(shy_base::load_config('system','attachment_stat') & $_POST['link']['logo']) {
	...
	public function edit() {
		if(isset($_POST['dosubmit'])){
			...
			showmessage(L('operation_success'),'?app=link&controller=link&view=edit','', 'edit');
	...
	public function edit_type() {
		if(isset($_POST['dosubmit'])){ 
			...
			showmessage(L('operation_success'),'?app=link&controller=link&view=list_type','', 'edit');
	...
	public function setting() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			showmessage(L('setting_updates_successful'), '?app=link&controller=link&view=init');
		} else {
			@extract($now_seting);
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=link&controller=link&view=add\', title:\''.L('link_add').'\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('link_add'));
 	...
 	public function check_register(){
		if(isset($_POST['dosubmit'])) {
			...
			$page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
			$infos = $this->db->listinfo($where,'linkid DESC',$page, $pages = '20');
			$pages = $this->db->pages;
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=link&controller=link&view=add\', title:\''.L('link_add').'\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('link_add'));
