...
<input type="hidden" name="app" value="block">
<input type="hidden" name="controller" value="block_admin">
<input type="hidden" name="view" value="public_search_related">
<input type="hidden" name="contentid"  value="<?php echo $_GET['contentid'];?>">

<table width="100%" class="table_form">
		<tr>
			<td width="80"><?php echo L('category')?>:</td> 
			<td><?php if(isset($_GET['dosubmit'])){?><div class="rt"><a href="javascript:void(0)" onclick="$('#search').toggle()"><?php echo L('folded_up_in_search_of')?></a></div><?php } echo form::select_category('', $catid, 'name="catid" id="catid"', '', '', '0', 1)?> </td>
		</tr>
...
<!--
	...
	$("input[type='checkbox'][name='ids[]']:checked").each(function(i,n){parent.insert_forms_related(<?php  echo $_GET['contentid'];?>,$(n).val());});
