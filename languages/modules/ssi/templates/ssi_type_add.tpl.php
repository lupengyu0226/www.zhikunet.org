...
<!--
	...
	$("#type_name").formValidator({onshow:"<?php echo L("input").L('分类名称')?>",onfocus:"<?php echo L("input").L('分类名称')?>"}).inputValidator({min:1,onerror:"<?php echo L("input").L('分类名称')?>"}).ajaxValidator({type : "get",url : "",data :"app=ssi&controller=ssi&view=public_check_name",datatype : "html",async:'false',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "<?php echo L('分类名称').L('exists')?>",onwait : "<?php echo L('connecting')?>"});
	 
	})
//-->
</script>
<div class="pad-lr-10">
<form action="?app=ssi&controller=ssi&view=add_type" method="post" name="myform" id="myform">
<table cellpadding="2" cellspacing="1" class="table_form" width="100%">
	...
	<tr>
		<th></th>
		<td><input type="hidden" name="forward" value="?app=ssi&controller=ssi&view=add_type"> <input
