...
<!--
	$(function(){
		...
		$("#name").formValidator({onshow:"<?php echo L('input').L('name')?>",onfocus:"<?php echo L('input').L('name')?>"}).inputValidator({min:1,onerror:"<?php echo L('input').L('name')?>"}).ajaxValidator({type : "get",url : "",data :"app=block&controller=block_admin&view=public_name&id=<?php if(isset($id) && !empty($id))echo $id;?>",datatype : "html",async:'false',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "<?php echo L('name').L('exists')?>",onwait : "<?php echo L('connecting')?>"})<?php if(ROUTE_A=='edit')echo '.defaultPassed()';?>;
...
<div class="pad-10">
<form action="?app=block&controller=block_admin&view=<?php echo ROUTE_A?>&pos=<?php echo $_GET['pos']?>&id=<?php if(isset($id) && !empty($id))echo $id;?>" method="post" id="myform">
<div>
<fieldset>
