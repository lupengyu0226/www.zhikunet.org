...
class message extends admin {
	function __construct() {
		parent::__construct();
		$this->db = shy_base::load_model('message_model');
		$this->group_db = shy_base::load_model('message_group_model');
	...
	public function init() {
		...
		$pages = $this->db->pages;
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=message&controller=message&view=message_send\', title:\''.L('all_send_message').'\', width:\'550\', height:\'300\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('all_send_message'));
	 ...
	 */
	 public function public_name() {
		$tousername = isset($_GET['tousername']) && trim($_GET['tousername']) ? (shy_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['tousername'])) : trim($_GET['tousername'])) : exit('0');
	 	//不能发给自己
		if($tousername == $this->_username){
				exit('0');
			}
		//判断用户名是否存在
		$member_interface = shy_base::load_app_class('member_interface', 'member');
		if ($tousername) {
	...
	public function delete_group() {
		if((!isset($_GET['message_group_id']) || empty($_GET['message_group_id'])) && (!isset($_POST['message_group_id']) || empty($_POST['message_group_id']))) {
			...
			if(is_array($_POST['message_group_id'])){
				...
				showmessage(L('operation_success'),'?app=message&controller=message&view=message_group_manage');
			}else{
				...
				if($result){
					...
					showmessage(L("operation_failure"),'?app=message&controller=message&view=message_group_manage');
				}
			}
			showmessage(L('operation_success'), HTTP_REFERER);
		}
	...
	public function message_send() {
		if(isset($_POST['dosubmit'])) {
			...
			//LOAD 会员组模型
			$member_group = shy_base::load_model('member_group_model');
			$member_group_infos = $member_group->select('','*','',$order = 'groupid ASC');
			//LOAD 管理员角色模型
			$role = shy_base::load_model('admin_role_model');
	...
	public function my_outbox() {
		...
		$infos = $this->db->listinfo($where,$order = 'messageid DESC',$page, $pages = '12');
		$pages = $this->db->pages;
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=message&controller=message&view=message_send\', title:\''.L('all_send_message').'\', width:\'550\', height:\'350\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('all_send_message'));
		$trade_status = $this->trade_status;
	...
	public function search_message() {
		...
		$infos = $this->db->listinfo($where,$order = 'messageid DESC',$page, $pages = '12');
		$pages = $this->db->pages;
 		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=message&controller=message&view=message_send\', title:\''.L('all_send_message').'\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('all_send_message'));
		$trade_status = $this->trade_status;
