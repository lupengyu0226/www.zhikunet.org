...
?>
<div class="pad-lr-10">
<form name="searchform" action="?app=message&controller=message&view=search_message&menuid=<?php echo $_GET['menuid'];?>" method="post" >
...
<form name="myform" action="?app=message&controller=message&view=delete" method="post" onsubmit="checkuid();return false;">
...
if(is_array($infos)){
	...
	<tr>
		...
		<td align="center" width="15%"> <a class="xbtn btn-danger btn-xs"
			href='?app=message&controller=message&view=delete&messageid=<?php echo $info['messageid']?>'
...
function see_all(id, name) {
	...
	window.top.art.dialog({title:'<?php echo L('details');//echo L('edit')?> '+name+' ',id:'edit',iframe:'?app=message&controller=message&view=see_all&messageid='+id,width:'700',height:'450'}, function(){var d = window.top.art.dialog({id:'see_all'}).data.iframe;var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'see_all'}).close()});
}
function checkuid() {
	var ids='';
