...
shy_base::load_app_class('foreground','member');//加载foreground 应用类. 自动判断是否登陆.
...
class index extends foreground {
	...
	public function init() {
		...
		$where = array('send_to_id'=>$this->_username,'del_type'=>'0','me_del'=>'0');
 		$infos = $this->message_db->listinfo($where,$order = 'messageid DESC',$page, 8);
 		$infos = new_html_special_chars($infos);
 		$pages = $this->message_db->pages;
		include template('message', 'index');
	}
	/**
	 * 发送消息 
	 */
	public function send() {
		$cuser = trim($_GET['touser']);
		$memberinfo = $this->memberinfo;
		$this->_session_start();
	...
	public function public_name() {
		$username = isset($_GET['username']) && trim($_GET['username']) ? (shy_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['username'])) : trim($_GET['username'])) : exit('0');
		...
		if((!isset($_GET['messageid']) || empty($_GET['messageid'])) && (!isset($_POST['messageid']) || empty($_POST['messageid']))) {
			...
			if(is_array($_POST['messageid'])){
				foreach($_POST['messageid'] as $messageid_arr) {
					...
					$this->message_db->update(array('me_del'=>'1'),array('messageid'=>$messageid_arr,'send_to_id'=>$this->_username));
				}
				showmessage(L('operation_success'), HTTP_REFERER);
			}
 		}
