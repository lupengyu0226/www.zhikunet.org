...
class slider extends admin {
	function __construct() {
		parent::__construct();
		$this->db = shy_base::load_model('slider_model');
	...
	public function init() {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=slider&controller=slider&view=add\', title:\''.L('slider_add').'\', width:\'700\', height:\'330\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_slider'));
	...
	public function public_name() {
		$slider_title = isset($_GET['slider_name']) && trim($_GET['slider_name']) ? (shy_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['slider_name'])) : trim($_GET['slider_name'])) : exit('0');
 	...
 	public function add() {
 		if(isset($_POST['dosubmit'])) {
			...
			if(!$sliderid) return FALSE; 
	 		...
	 		//更新附件状态
			if(shy_base::load_config('system','attachment_stat') & $_POST['slider']['image']) {
	...
	//:分类管理
 	public function list_type() {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=slider&controller=slider&view=add\', title:\''.L('slider_add').'\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('slider_add'));
	...
	public function edit_type() {
		if(isset($_POST['dosubmit'])){ 
			...
			showmessage(L('operation_success'),'?app=slider&controller=slider&view=list_type','', 'edit');
	...
	public function delete() {
  		if((!isset($_GET['id']) || empty($_GET['id'])) && (!isset($_POST['id']) || empty($_POST['id']))) {
			...
			if(is_array($_POST['id'])){
				...
				showmessage(L('operation_success'),'?app=slider&controller=slider');
			}else{
				$id = intval($_GET['id']);
				if($id < 1) return false;
				//删除幻灯片
				...
				if($result){
					...
					showmessage(L("operation_failure"),'?app=slider&controller=slider');
				}
			}
			showmessage(L('operation_success'), HTTP_REFERER);
		}
