...
?>
<div class="pad-lr-10">
<form name="myform" action="?app=slider&controller=slider&view=delete_type" method="post" onsubmit="checkuid();return false;">
...
function edit(id, name) {
	...
	window.top.art.dialog({title:'<?php echo L('edit')?> '+name+' ',id:'edit',iframe:'?app=slider&controller=slider&view=edit_type&typeid='+id,width:'450',height:'70'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
}
function checkuid() {
	var ids='';
...
function view(id, name) {
	window.top.art.dialog({title:name+' 对应的标签调用',fixed:true, width:'600',height:'290',iframe:'?app=slider&controller=slider&view=view_lable&typeid='+id});	
}
