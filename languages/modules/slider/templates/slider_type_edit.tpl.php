...
<!--
	...
	$("#type_name").formValidator({onshow:"<?php echo L("input").L('slider_postion_name')?>",onfocus:"<?php echo L("input").L('slider_postion_name')?>"}).inputValidator({min:1,onerror:"<?php echo L("input").L('slider_postion_name')?>"}).ajaxValidator({type : "get",url : "",data :"app=slider&controller=slider&view=public_check_name&typeid=<?php echo $typeid;?>",datatype : "html",async:'false',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "<?php echo L('slider_postion_name').L('exists')?>",onwait : "<?php echo L('connecting')?>"}).defaultPassed(); 
	 
	})
//-->
</script>
<div class="pad-lr-10">
<form action="?app=slider&controller=slider&view=edit_type&typeid=<?php echo $typeid; ?>" method="post" name="myform" id="myform">
