...
<table width="100%" cellspacing="0" class="search-form">
	...
	foreach($type_arr as $typeid => $type){
		?><a href="?app=slider&controller=slider&typeid=<?php echo $typeid;?>"><?php echo $type;?></a>&nbsp;
		<?php }}?>
		</div>
		</td>
		</tr>
...
<form name="myform" id="myform" action="?app=slider&controller=slider&view=listorder" method="post" >
...
if(is_array($infos)){
	...
	<tr>
		...
		<td align="center" width="12%"><a class="xbtn btn-info btn-xs" href="###"
			...
			href='?app=slider&controller=slider&view=delete&id=<?php echo $info['id']?>'
...
<input name="dosubmit" type="submit" class="button"
	value="<?php echo L('listorder')?>">&nbsp;&nbsp;<input type="submit" class="button" name="dosubmit" onClick="document.myform.action='?app=slider&controller=slider&view=delete'" value="<?php echo L('delete')?>"/></div>
...
function edit(id, name) {
	...
	window.top.art.dialog({title:'<?php echo L('edit')?> '+name+' ',id:'edit',iframe:'?app=slider&controller=slider&view=edit&id='+id,width:'700',height:'330'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
}
...
function listorder_up(id) {
	$.get('?app=slider&controller=slider&view=listorder_up&id='+id,null,function (msg) { 
	if (msg==1) { 
