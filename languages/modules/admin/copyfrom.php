...
class copyfrom extends admin {
	...
	function __construct() {
		$this->db = shy_base::load_model('copyfrom_model');
	...
	public function init () {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=admin&controller=copyfrom&view=add\', title:\''.L('add_copyfrom').'\', width:\'580\', height:\'240\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_copyfrom'));
		$this->public_cache();
	...
	private function check($data = array()) {
		...
		if (!preg_match('/^(http(s)?:)?\/\/([^\/]+)/i', $data['siteurl'])) showmessage(L('input').L('copyfrom_url'));
		if (empty($data['sitename'])) showmessage(L('input').L('copyfrom_name'));
		if ($data['thumb'] && !preg_match('/^(http(s)?:)?\/\/([^\/]+)/i', $data['thumb'])) showmessage(L('copyfrom_logo').L('format_incorrect'));
