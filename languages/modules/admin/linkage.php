...
class linkage extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = shy_base::load_model('linkage_model');
		$this->sites = shy_base::load_app_class('sites');
	...
	public function init() {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=admin&controller=linkage&view=add\', title:\''.L('linkage_add').'\', width:\'500\', height:\'220\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('linkage_add'));
	...
	public function public_listorder() {
		...
		showmessage(L('operation_success'),'?app=admin&controller=linkage&view=init');
	}
	...
	public function public_manage_submenu() {
		...
		foreach($result as $areaid => $area){
			...
			$areas[$area['linkageid']]['str_manage'] = ($sum > 40 && $this->_is_last_node($area['keyid'],$area['linkageid'])) ? '<a href="?app=admin&controller=linkage&view=public_manage_submenu&keyid='.$area['keyid'].'&parentid='.$area['linkageid'].'">'.L('linkage_manage_submenu').'</a> | ' : '';
			$areas[$area['linkageid']]['str_manage'] .= '<a class="xbtn btn-info btn-xs" href="javascript:void(0);" onclick="add(\''.$keyid.'\',\''.new_addslashes($area['name']).'\',\''.$area['linkageid'].'\')">'.L('linkage_add_submenu').'</a>   <a class="xbtn btn-warning btn-xs" href="javascript:void(0);" onclick="edit(\''.$area['linkageid'].'\',\''.$area['name'].'\',\''.$area['parentid'].'\')">'.L('edit').'</a>   <a class="xbtn btn-danger btn-xs" href="javascript:confirmurl(\'?app=admin&controller=linkage&view=delete&linkageid='.$area['linkageid'].'&keyid='.$area['keyid'].'\', \''.L('linkage_is_del').'\')">'.L('delete').'</a> ';
		}
