...
class site extends admin {
	...
	public function __construct() {
		$this->db = shy_base::load_model('site_model');
		parent::__construct();
	}
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($this->db->insert(array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'template'=>$template,'setting'=>$setting, 'default_style'=>$default_style))) {
				$class_site = shy_base::load_app_class('sites');
				$class_site->set_cache();
				showmessage(L('operation_success'), '?app=admin&controller=site&view=init', '', 'add');
			} else {
	...
	public function edit() {
		...
		if ($data = $this->db->get_one(array('siteid'=>$siteid))) {
			if (isset($_POST['dosubmit'])) {
				...
				$setting = trim(array2string($_POST['setting']));
				$sql = array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'template'=>$template, 'setting'=>$setting, 'default_style'=>$default_style);
	...
	public function public_dirname() {
		$dirname = isset($_GET['dirname']) && trim($_GET['dirname']) ? (shy_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['dirname'])) : trim($_GET['dirname'])) : exit('0');
		$siteid = isset($_GET['siteid']) && intval($_GET['siteid']) ? intval($_GET['siteid']) : '';
		$data = array();
		if ($siteid) {
