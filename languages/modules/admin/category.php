...
class category extends admin {
	...
	public function init () {
		...
		if(!empty($result)) {
			foreach($result as $r) {
				...
				if(!$show_detail) {
					...
					$r['parentid'] = 0;
					$r['str_manage'] .= '<a href="?app=admin&controller=category&view=init&parentid='.$r['catid'].'&menuid='.$_GET['menuid'].'&s='.$r['type'].'&safe_edi='.$_SESSION['safe_edi'].'">'.L('manage_sub_category').'</a>   ';
				}
				$r['str_manage'] .= '<a class="xbtn btn-info btn-xs" href="?app=admin&controller=category&view=add&parentid='.$r['catid'].'&menuid='.$_GET['menuid'].'&s='.$r['type'].'&safe_edi='.$_SESSION['safe_edi'].'">'.L('add_sub_category').'</a>   ';
				
				$r['str_manage'] .= '<a class="xbtn btn-primary btn-xs" href="?app=admin&controller=category&view=edit&catid='.$r['catid'].'&menuid='.$_GET['menuid'].'&type='.$r['type'].'&safe_edi='.$_SESSION['safe_edi'].'">'.L('edit').'</a>   <a class="xbtn btn-danger btn-xs" href="javascript:confirmurl(\'?app=admin&controller=category&view=delete&catid='.$r['catid'].'&menuid='.$_GET['menuid'].'\',\''.L('confirm',array('message'=>addslashes($r['catname']))).'\')">'.L('delete').'</a>   <a class="xbtn btn-warning btn-xs" href="?app=admin&controller=category&view=remove&catid='.$r['catid'].'&safe_edi='.$_SESSION['safe_edi'].'">'.L('remove','','content').'</a>';
				if($r['type'] || $r['child']) {
				} else {
				$r['str_manage'] .= '   <a class="xbtn btn-inverse btn-xs" href="?app=content&controller=content&view=init&menuid=822&catid='.$r['catid'].'&safe_edi='.$_SESSION['safe_edi'].'">管理内容</a>';
				}
				...
				if($r['url']) {
					...
					$r['url'] = ' <a href="'.$r['url'].'" target="_blank">'.L('vistor').'</a>&nbsp;&nbsp;<a href="'.S($r['url']).'" target="_blank"><i class="icon-on icon-mobile"><em class="home"></em></i></a>' ;
				} else {
					$r['url'] = "<a href='?app=admin&controller=category&view=public_cache&menuid=43&module=admin'><font color='red'>".L('update_backup')."</font></a>";
				}
				$categorys[$r['catid']] = $r;
			}
		}
		$str  = "<tr>
					...
					<td align='center' width='180'>\$help</td>
					<td>\$str_manage</td>
				</tr>";
		$tree->init($categorys);
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			if($type==0) {
				...
				if(!$exists_model) showmessage(L('please_add_model'),'?app=content&controller=sitemodel&view=init&menuid=59',5000);
	...
	public function edit() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($_POST['info']['parentid']==$catid){
				showmessage(L('operation_failure'),'?app=admin&controller=category&view=init&module=admin&menuid=43');
			}
			//栏目生成静态配置
			if($_POST['type'] != 2) {
				if($setting['ishtml']) {
			...
			if($_POST['template_child']){
                                ...
                                $idstr = $this->get_arrchildid($catid);
                                 if(!empty($idstr)){
                                        $sql = "select catid,setting from shuyang_category where catid in($idstr)";
                                        $this->db->query($sql);
			...
			$this->cache();
			//更新附件状态
			if($_POST['info']['image'] && shy_base::load_config('system','attachment_stat')) {
				$this->attachment_db = shy_base::load_model('attachment_model');
				$this->attachment_db->api_update($_POST['info']['image'],'catid-'.$catid,1);
			}
			showmessage(L('operation_success').'<script type="text/javascript">window.top.art.dialog({id:"test"}).close();window.top.art.dialog({id:"test",content:\'<h2>'.L("operation_success").'</h2><span style="fotn-size:16px;">'.L("edit_following_operation").'</span><br /><ul style="fotn-size:14px;"><li><a href="?app=admin&controller=category&view=public_cache&menuid=43&module=admin" target="right"  onclick="window.top.art.dialog({id:\\\'test\\\'}).close()">'.L("following_operation_1").'</a></li></ul>\',width:"400",height:"200"});</script>','?app=admin&controller=category&view=init&module=admin&menuid=43');
		} else {
			//获取站点模板信息
			shy_base::load_app_func('global');
			$template_list = template_list($this->siteid, 0);
	...
	private function delete_category_video($catid, $modelid) {
		$content_model = shy_base::load_model('content_model');
	...
	public function public_cache() {
		$this->repair();
		$this->cache();
		showmessage(L('operation_success'),'?app=admin&controller=category&view=init&module=admin&menuid=43');
	}
	/**
	* 修复栏目数据
	*/
	...
	*/
	private function update_url($catid) {
		...
		$url = shy_base::load_app_class('url', 'content'); //调用URL实例
	...
	public function count_items() {
		...
		foreach($result as $r) {
			...
			if($r['type'] == 0) {
				...
				$this->db->update(array('items'=>0),array('catid'=>$r['catid']));
			}
		}
		showmessage(L('operation_success'),HTTP_REFERER);
	}
	...
	public function batch_edit() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			foreach ($infos as $catid=>$info) {
				...
				//更新附件状态
				if($info['image'] && shy_base::load_config('system','attachment_stat')) {
