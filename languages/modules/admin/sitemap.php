...
*/
function init() {
$hits_db = shy_base::load_model('hits_model');
$CATEGORYS = $this->categorys;
//读站点缓存
$siteid = $this->siteid;
$sitecache = getcache('sitelist','commons');
//根据当前站点,取得文件存放路径
$systemconfig = shy_base::load_config('system');
$html_root = substr($systemconfig['html_root'], 1);
//判断当前站点目录,是SHUYANG则把文件写到根目录下, 不是则写到分站目录下.(分站目录用由静态文件路经html_root和分站目录dirname组成)
if($siteid==1){
$dir = SHUYANG_PATH;
}else {
$dir = SHUYANG_PATH.$html_root.DIRECTORY_SEPARATOR.$sitecache[$siteid]['dirname'].DIRECTORY_SEPARATOR;
}
//模型缓存
