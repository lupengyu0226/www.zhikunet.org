...
class ipbanned extends admin {
	function __construct() {
		$this->db = shy_base::load_model('ipbanned_model');
	...
	function init () {
		...
		$pages = $this->db->pages;	
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=admin&controller=ipbanned&view=add\', title:\''.L('add_ipbanned').'\', width:\'450\', height:\'300\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_ipbanned'));
	...
	function add() {
		if(isset($_POST['dosubmit'])){
			...
			showmessage(L('operation_success'),'?app=admin&controller=ipbanned&view=add','', 'add');
		}else{
	...
	function delete() {
 		if(is_array($_POST['ipbannedid'])){
			...
			$this->public_cache_file();//更新缓存 
			showmessage(L('operation_success'),'?app=admin&controller=ipbanned');	
		} else {
			...
			if($result){
				...
				showmessage(L("operation_failure"),'?app=admin&controller=ipbanned');
			}
		}
	}
