...
class keylink extends admin {
	function __construct() {
		$this->db = shy_base::load_model('keylink_model');
		parent::__construct();
	}
	...
	function init () {
		...
		$pages = $this->db->pages;	
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=admin&controller=keylink&view=add\', title:\''.L('add_keylink').'\', width:\'450\', height:\'130\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_keylink'));
	...
	function add() {
		if(isset($_POST['dosubmit'])){
				...
				showmessage(L('operation_success'),'?app=admin&controller=keylink&view=add','', 'add');
			}else{
	...
	function edit() {
		if(isset($_POST['dosubmit'])){
			...
			$this->public_cache_file();//更新缓存
			showmessage(L('operation_success'),'?app=admin&controller=keylink&view=edit','', 'edit');
		}else{
			$show_validator = $show_scroll = $show_header = true;
	...
	function delete() {
 		if(is_array($_POST['keylinkid'])){
			...
			$this->public_cache_file();//更新缓存
			showmessage(L('operation_success'),'?app=admin&controller=keylink');	
		} else {
			$keylinkid = intval($_GET['keylinkid']);
			...
			if($result){
				...
				showmessage(L("operation_failure"),'?app=admin&controller=keylink');
			}
		}
	}
	/**
