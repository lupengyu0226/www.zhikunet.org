...
class role extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = shy_base::load_model('admin_role_model');
		$this->priv_db = shy_base::load_model('admin_role_priv_model');
		$this->op = shy_base::load_app_class('role_op');
	}
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->_cache();
			if($insert_id){
				showmessage(L('operation_success'),'?app=admin&controller=role&view=init');
			}
		} else {
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->_cache();
			showmessage(L('operation_success'),'?app=admin&controller=role');
		} else {					
	...
	public function role_priv() {
		...
		if(isset($_POST['dosubmit'])){
			...
			$roleid = intval($_GET['roleid']);
			if ($siteid) {
				$menu = shy_base::load_sys_class('tree');
				$menu->icon = array('│ ','├─ ','└─ ');
				$menu->nbsp = '&nbsp;&nbsp;&nbsp;';
	...
	public function member_manage() {
		$this->admin_db = shy_base::load_model('admin_model');
		$roleid = intval($_GET['roleid']);
	...
	public function setting_cat_priv() {
		...
		switch ($op) {
			...
			shy_base::load_app_class('role_cat', '', 0);
		...
		case 2:
			...
			showmessage(L('operation_success'),'?app=admin&controller=role&view=init', '', 'edit');
			break;
