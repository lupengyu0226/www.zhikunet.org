...
class admin_op {
	 ...
	 * 修改锁屏密码
	 */
	public function edit_lockscreen($userid, $lockscreen){
		$userid = intval($userid);
		if($userid < 1) return false;
		if(!is_lockscreen($lockscreen))
		{
			showmessage(L('pwd_incorrect'));
			return false;
		}
		$lockscreeninfo = lockscreen($lockscreen);
		return $this->db->update($lockscreeninfo,array('userid'=>$userid));
	}
	/*
	 * 检查用户名重名
	 */	
