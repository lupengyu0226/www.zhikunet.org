...
class card {
	...
	public static function get_release() {
		return shy_base::load_config('version','shy_release');
	}
	...
	public static function verification($sn, $code, $rand) {
		$key = self::get_key();
		return self::_get_data('?op=verification&release='.self::get_release().'&key='.urlencode($key).'&code='.urlencode(self::sys_auth("sn=$sn&code=$code&rand=$rand", 'ENCODE', $key)), 'index.php?app=admin&controller=index&view=public_card');
	} 
	...
	private static function sys_auth($txt, $operation = 'ENCODE', $key = '') {
