...
class push_api {
	...
	public function __construct() {
		$this->db = shy_base::load_model('position_model');  //加载数据模型
	}
	 ...
	 * 调用方式
	 * $push = shy_base::load_app_class('push_api','admin');
	...
	private function content_pos($id, $modelid) {
		...
		$modelid = intval($modelid);
		if ($id && $modelid) {
			$db_data = shy_base::load_model('position_data_model');
