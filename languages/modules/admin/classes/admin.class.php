...
class admin {
	...
	public function __construct() {
		...
		self::lock_screen();
		// self::check_hash();
		if(shy_base::load_config('system','admin_url') && $_SERVER["HTTP_HOST"]!= shy_base::load_config('system','admin_url')) {
	...
	final public function check_admin() {
		if(ROUTE_M =='admin' && ROUTE_C =='index' && in_array(ROUTE_A, array('login', 'public_card'))) {
			...
			if(!isset($_SESSION['userid']) || !isset($_SESSION['roleid']) || !$_SESSION['userid'] || !$_SESSION['roleid'] || $userid != $_SESSION['userid']) showmessage(L('admin_login'),'?app=admin&controller=index&view=login');
		}
	}
	...
	final public static function admin_tpl($file, $m = '') {
		...
		return SHY_PATH.'modules'.DIRECTORY_SEPARATOR.$m.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$file.'.tpl.php';
	}
	...
	final public static function admin_menu($parentid, $with_self = 0) {
		...
		$array = array();
		$privdb = shy_base::load_model('admin_role_priv_model');
	...
	final public static function submenu($parentid = '', $big_menu = false) {
		...
		foreach($array as $_value) {
			...
			if($classname) {
				...
				$string .= "<a href='?app=".$_value['m']."&controller=".$_value['c']."&view=".$_value['a']."&menuid=$parentid&safe_edi=$safe_edi".'&'.$_value['data']."' $classname><em>".L($_value['name'])."</em></a><span>|</span>";
			}
		}
	...
	final public static function current_pos($id) {
		$menudb = shy_base::load_model('menu_model');
		$r =$menudb->get_one(array('id'=>$id),'id,name,icon,parentid');
		$str = '';
		if($r['parentid']) {
			$str = self::current_pos($r['parentid']);
		}
		return !$r['parentid'] ? $str.'<i class="iconfont '.$r['icon'].'"></i> '.L($r['name']).'' : $str.' / '.L($r['name']).' ';
	}
		...
		extract($setconfig);
 		if($admin_log==1){
 			...
 			if($action == '' || strchr($action,'public') || $action == 'init' || $action=='public_current_pos') {
				...
				$ip = ip();
				$log = shy_base::load_model('log_model');
				$username = param::get_cookie('admin_username');
				$userid = isset($_SESSION['userid']) ? $_SESSION['userid'] : '';
				$time = date('Y-m-d H-i-s',SYS_TIME);
				$url = '?app='.ROUTE_M.'&controller='.ROUTE_C.'&view='.ROUTE_A;
	...
	final private function lock_screen() {
		if(isset($_SESSION['lock_screen']) && $_SESSION['lock_screen']==1) {
			...
			showmessage(L('admin_login'),'?app=admin&controller=index&view=login');
		}
	}
	...
	final private function check_hash() {
		...
		if(isset($_GET['safe_edi']) && $_SESSION['safe_edi'] != '' && ($_SESSION['safe_edi'] == $_GET['safe_edi'])) {
			return true;
		} elseif(isset($_POST['safe_edi']) && $_SESSION['safe_edi'] != '' && ($_SESSION['safe_edi'] == $_POST['safe_edi'])) {
			return true;
		} else {
	...
	final public function admin_list_template($id = '', $str = '') {
		$templatedir = SHY_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.'content'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR;
		$pre = 'content_list';
