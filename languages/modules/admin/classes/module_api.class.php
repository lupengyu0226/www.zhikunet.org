...
class module_api {
	...
	public function install($module = '') {
		...
		$this->installdir = SHY_PATH.'modules'.DIRECTORY_SEPARATOR.$this->module.DIRECTORY_SEPARATOR.'install'.DIRECTORY_SEPARATOR;
		...
		if (is_array($models) && !empty($models)) {
			foreach ($models as $m) {
				$this->m_db = shy_base::load_model($m.'_model');
		...
		if (file_exists($this->installdir.'extention.inc.php')) {
			...
			if(!defined('INSTALL_MODULE')) {
				$file = SHY_PATH.'languages'.DIRECTORY_SEPARATOR.shy_base::load_config('system', 'lang').DIRECTORY_SEPARATOR.'system_menu.lang.php';
				if(file_exists($file)) {
		...
		if(!defined('INSTALL_MODULE')) {
			if (file_exists($this->installdir.'languages'.DIRECTORY_SEPARATOR)) {
				dir_copy($this->installdir.'languages'.DIRECTORY_SEPARATOR, SHY_PATH.'languages'.DIRECTORY_SEPARATOR);
			}
			if(file_exists($this->installdir.'templates'.DIRECTORY_SEPARATOR)) {
				dir_copy($this->installdir.'templates'.DIRECTORY_SEPARATOR, SHY_PATH.'templates'.DIRECTORY_SEPARATOR.shy_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR.$this->module.DIRECTORY_SEPARATOR);
				if (file_exists($this->installdir.'templates'.DIRECTORY_SEPARATOR.'name.inc.php')) {
					$keyid = 'templates|'.shy_base::load_config('system', 'tpl_name').'|'.$this->module;
					$file_explan[$keyid] = include $this->installdir.'templates'.DIRECTORY_SEPARATOR.'name.inc.php';
					$templatepath = SHY_PATH.'templates'.DIRECTORY_SEPARATOR.shy_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR;
					...
					unlink(SHY_PATH.'templates'.DIRECTORY_SEPARATOR.shy_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR.$this->module.DIRECTORY_SEPARATOR.'name.inc.php');
				}
			}
		}
		return true;
	...
	define('INSTALL', true);
		...
		if(!defined('INSTALL_MODULE')) {
			if (dir_create(SHY_PATH.'languages'.DIRECTORY_SEPARATOR.shy_base::load_config('system', 'lang').DIRECTORY_SEPARATOR.'test_create_dir')) {
				sleep(1);
				dir_delete(SHY_PATH.'languages'.DIRECTORY_SEPARATOR.shy_base::load_config('system', 'lang').DIRECTORY_SEPARATOR.'test_create_dir');
		...
		if (is_array($models) && !empty($models)) {
			foreach ($models as $m) {
				if (!file_exists(SHY_PATH.'model'.DIRECTORY_SEPARATOR.$m.'_model.class.php')) {
	...
	public function uninstall($module) {
		...
		$this->module = $module;
		$this->uninstalldir = SHY_PATH.'modules'.DIRECTORY_SEPARATOR.$this->module.DIRECTORY_SEPARATOR.'uninstall'.DIRECTORY_SEPARATOR;
		...
		if (file_exists(SHY_PATH.'languages'.DIRECTORY_SEPARATOR.shy_base::load_config('system', 'lang').DIRECTORY_SEPARATOR.$this->module.'.lang.php')) {
			@unlink(SHY_PATH.'languages'.DIRECTORY_SEPARATOR.shy_base::load_config('system', 'lang').DIRECTORY_SEPARATOR.$this->module.'.lang.php');
		}
		if (is_dir(SHY_PATH.'templates'.DIRECTORY_SEPARATOR.shy_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR.$this->module)) {
			@dir_delete(SHY_PATH.'templates'.DIRECTORY_SEPARATOR.shy_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR.$this->module);
		}
		$templatepath = SHY_PATH.'templates'.DIRECTORY_SEPARATOR.shy_base::load_config('system', 'tpl_name').DIRECTORY_SEPARATOR;
		if (file_exists($templatepath.'config.php')) {
			$keyid = 'templates|'.shy_base::load_config('system', 'tpl_name').'|'.$this->module;
	...
	private function sql_split($sql) {
		...
		if (!$dbcharset) {
			$dbcharset = shy_base::load_config('database','default');
		...
		if($this->m_db->db_tablepre != "shuyang_") $sql = str_replace("shuyang_", $this->m_db->db_tablepre, $sql);
