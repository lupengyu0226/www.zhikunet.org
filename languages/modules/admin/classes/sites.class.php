...
class sites {
	...
	public function set_cache() {
		...
		foreach ($list as $key=>$val) {
			$data[$val['siteid']] = $val;
			$data[$val['siteid']]['url'] = $val['domain'] ? $val['domain'] : shy_base::load_config('system', 'web_path').$val['dirname'].'/';
		}
	...
	public function get_role_siteid($roleid) {
		...
		if(is_array($sitelist)) 
		{
			...
			showmessage(L('no_site_permissions'),'?app=admin&controller=index&view=login');
		}
	}
}
