...
class cache_api {
	...
	public function cache($model = '', $param = '') {
		if (file_exists(SHY_PATH.'model'.DIRECTORY_SEPARATOR.$model.'_model.class.php')) {
			$this->db = shy_base::load_model($model.'_model');
			if ($param) {
				$this->$model($param);
			} else {
				$this->$model();
	...
	public function category() {
		...
		foreach($this->categorys as $r) {
			...
			$r['category_mobilerule'] = $setting['category_mobilerule'];
			$r['show_mobilerule'] = $setting['show_mobilerule'];
			$r['workflowid'] = $setting['workflowid'];
			$r['isdomain'] = '0';
			if(!preg_match('/^(http(s)?:)?\/\//i', $r['url'])) {
	...
	public function cache_siteid($role) {
		$priv_db = shy_base::load_model('admin_role_priv_model');
		$sitelist = array();
		foreach($role as $n=>$r) {
	...
	public function sitemodel() {
		define('MODEL_PATH', SHY_PATH.'modules'.DIRECTORY_SEPARATOR.'content'.DIRECTORY_SEPARATOR.'fields'.DIRECTORY_SEPARATOR);
	...
	public function dbsource() {
		$db = shy_base::load_model('dbsource_model');
		$list = $db->select();
		$data = array();
		if ($list) {
			foreach ($list as $val) {
	...
	public function membermodel() {
		define('MEMBER_MODEL_PATH',SHY_PATH.'modules'.DIRECTORY_SEPARATOR.'member'.DIRECTORY_SEPARATOR.'fields'.DIRECTORY_SEPARATOR);
		//模型缓存路径
		define('MEMBER_CACHE_MODEL_PATH',SHUYANG_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_model'.DIRECTORY_SEPARATOR.'caches_data'.DIRECTORY_SEPARATOR);
		
		$sitemodel_db = shy_base::load_model('sitemodel_model');
	...
	public function member_model_field() {
		...
		$this->db = shy_base::load_model('sitemodel_field_model');
	...
	public function database() {
		...
		if (is_array($M)) {
			foreach ($M as $key => $m) {
				if (file_exists(SHY_PATH.'modules'.DIRECTORY_SEPARATOR.$key.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$key.'_tag.class.php') && !in_array($key, array('message', 'block'))) {
					$module[$key] = $m['name'];
				}
			}
		}
		...
		return $file_size = shy_base::load_config('system','lock_ex') ? file_put_contents($filepath.'modules.php', $module, LOCK_EX) : file_put_contents($filepath.'modules.php', $module);
	}
	...
	public function cache2database() {
		$cache = shy_base::load_model('cache_model');
		...
		if (is_array($result) && !empty($result)) {
			foreach ($result as $re) {
				if (!file_exists(CACHE_PATH.$re['path'].$re['filename'])) {
					$filesize = shy_base::load_config('system','lock_ex') ? file_put_contents(CACHE_PATH.$re['path'].$re['filename'], $re['data'], LOCK_EX) : file_put_contents(CACHE_PATH.$re['path'].$re['filename'], $re['data']);
				} else {
					continue;
				}
			}
		...
		if ($currpage>$pages) {
			...
			showmessage(L('part_cache_success'), '?app=admin&controller=cache_all&view=init&page='.$_GET['page'].'&currpage='.$currpage.'&pages='.$pages.'&dosubmit=1',0);
		}
	}
	...
	public function del_file() {
		$path = SHUYANG_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR;
		$files = glob($path.'*');
		...
		$path = SHUYANG_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_tpl_data'.DIRECTORY_SEPARATOR.'caches_data'.DIRECTORY_SEPARATOR;
		$files = glob($path.'*');
		if (is_array($files)) {
			foreach ($files as $f) {
				$dir = basename($f);
	 ...
	 * 更新 MIP 首页缓存方法
	 */
	 public function cache_mip() {
		$api = 'http://mipcache.bdstatic.com/update-ping/c/';
		$postData = 'key=a5b7a089cea47e6d6e9a7e65a90a1767';
		$url = $api.urlencode('wap.05273.cn/mip.html');
		$ch = curl_init();
