...
class position extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = shy_base::load_model('position_model');
		$this->db_data = shy_base::load_model('position_data_model');
		$this->db_content = shy_base::load_model('content_model');			
	...
	public function init() {
			...
			$pages = $this->db->pages;
			$show_dialog = true;
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=admin&controller=position&view=add\', title:\''.L('posid_add').'\', width:\'500\', height:\'360\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('posid_add'));
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->sitemodel_db = shy_base::load_model('sitemodel_model');
	...
	public function delete() {
		...
		$this->_set_cache();
		showmessage(L('posid_del_success'),'?app=admin&controller=position');
	}
	...
	public function listorder() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->_set_cache();
			showmessage(L('operation_success'),'?app=admin&controller=position');
		} else {
			showmessage(L('operation_failure'),'?app=admin&controller=position');
		}
	}
	...
	public function public_item() {	
		if(isset($_POST['dosubmit'])) {
			...
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=admin&controller=position&view=add\', title:\''.L('posid_add').'\', width:\'500\', height:\'300\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('posid_add'));			
	...
	public function public_item_manage() {
		if(isset($_POST['dosubmit'])) {
			...
			$array['title'] = trim($_POST['info']['title']);
                        $array['style'] = trim($_POST['info']['style_color']);
