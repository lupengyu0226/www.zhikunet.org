...
class badword extends admin {
	function __construct() {
		...
		$this->db = shy_base::load_model('badword_model');
		parent::__construct();
	}
	...
	function init () {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=admin&controller=badword&view=add\', title:\''.L('badword_add').'\', width:\'450\', height:\'180\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('badword_add'));
		...
		if(isset($_POST['dosubmit'])){
				...
				$_POST['info']['badword'] = str_replace("　","",trim($_POST['badword']));
	 			if(empty($_POST['info']['badword'])) {
					showmessage(L('enter_word'),'?app=admin&controller=badword&view=add');
					}
				$this->db->insert($_POST['info']);
				$this->public_cache_file();//更新缓存
		 		showmessage(L('operation_success'),'?app=admin&controller=badword&view=add','', 'add');
		 		}else{
	...
	public function public_name() {
			$badword = isset($_GET['badword']) && trim($_GET['badword']) ? (shy_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['badword'])) : trim($_GET['badword'])) : exit('0');
	...
	function listorder() {
		if(!is_array($_POST['listorders'])) return false;
			...
			showmessage(L('operation_success'),'?app=admin&controller=badword');
	}
	...
	function edit() {
		if(isset($_POST['dosubmit'])){
				...
				$this->public_cache_file();//更新缓存
				showmessage(L('operation_success'),'?app=admin&controller=badword&view=edit','', 'edit');
			}else{
				$show_validator = $show_scroll = $show_header = true;
				$info = array();
	...
	function delete() {
 		if(is_array($_POST['badid'])){
				...
				if($result){
					...
					showmessage(L("operation_failure"),'?app=admin&controller=badword');
				}
		}
	}
	...
	function export() {
		...
		if(!is_array($result) || empty($result)){
			showmessage('暂无敏感词设置，正在返回！','?app=admin&controller=badword');
		}
  		foreach($result as $s){
 			extract($s);
	...
	function import(){
		if(isset($_POST['dosubmit'])){
				$arr = $s = $str = $level_arr = '';
				$s = trim($_POST['info']);
			    if(empty($s)) showmessage(L('not_information'),'?app=admin&controller=badword&view=import');
	 			$arr = explode("\n",$s); 
