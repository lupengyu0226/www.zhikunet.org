	...
	public function init() {
		if (isset($_POST['dosubmit']) || isset($_GET['dosubmit'])) {
			...
			$modules = array(
				...
				array('name' => L('update_mip_index'), 'function' => 'cache_mip'),
			);
			$this->cache_api = shy_base::load_app_class('cache_api', 'admin');
			$m = $modules[$page];
			if ($m['mod'] && $m['function']) {
				if ($m['file'] == '') $m['file'] = $m['function'];
				$M = getcache('modules', 'commons');
				if (in_array($m['mod'], array_keys($M))) {
					$cache = shy_base::load_app_class($m['file'], $m['mod']);
					$cache->{$m['function']}();
				}
			...
			if (!empty($modules[$page])) {
				...
				showmessage(L('update').$m['name'].L('cache_file_success'), '?app=admin&controller=cache_all&page='.$page.'&dosubmit=1&safe_edi='.$_SESSION['safe_edi'], 0);
			} else {
