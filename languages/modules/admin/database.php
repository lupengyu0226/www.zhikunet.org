...
class database extends admin {
	...
	function __construct() {
		...
		shy_base::load_sys_class('db_factory');
	...
	public function import() {
		$database = shy_base::load_config('database');
		if($_GET['dosubmit']) {
			$admin_founders = explode(',',shy_base::load_config('system','admin_founders'));
			...
			if(is_array($sqlfiles)) {
				...
				foreach($sqlfiles as $id=>$sqlfile) {
					//老的数据库备份文件转换为新格式
					if(preg_match("/([shuyangtables_|db_][0-9]{8}_[0-9a-z]{20}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
					...
					if(preg_match("/([0-9a-z]{20}_[shuyangtables_|db_]+[0-9]{8}_)([0-9]+)\.sql/i",basename($sqlfile),$num)) {
	...
	public function public_repair() {
		...
		if($tables && in_array($operation,array('repair','optimize'))) {
			...
			showmessage(L('operation_success'),'?app=admin&controller=database&view=export&pdoname='.$pdo_name);
			...
			showmessage(L('select_tbl'),'?app=admin&controller=database&view=export&pdoname='.$pdo_name);
		}
	}
	...
	public function delete() {
		...
		if($filenames) {
			if(is_array($filenames)) {
				...
				showmessage(L('operation_success'),'?app=admin&controller=database&view=import&pdoname='.$pdo_name);
			} else {
	...
	private function status($tables,$tablepre) {
		...
		return array('shuyangtables'=>$shuyang, 'othertables'=>$other);
	}
	 ...
	 * @param unknown_type $tabletype 备份数据库类型 （非shuyang数据与shuyang数据）
	 */
	private function export_database($tables,$sqlcompat,$sqlcharset,$sizelimit,$action,$fileid,$random,$tableid,$startfrom,$tabletype) {
		...
		if(trim($tabledump)) {
			$tabledump = "# shuyang bakfile\n# version:SHUYANG V9\n# time:".date('Y-m-d H:i:s')."\n# type:shuyang\n# shuyang:http://www.05273.cn\n# --------------------------------------------------------\n\n\n".$tabledump;
			$tableid = $i;
			...
			showmessage(L('bakup_file')." $filename ".L('bakup_write_succ'), '?app=admin&controller=database&view=export&sizelimit='.$sizelimit.'&sqlcompat='.$sqlcompat.'&sqlcharset='.$sqlcharset.'&tableid='.$tableid.'&fileid='.$fileid.'&startfrom='.$startrow.'&random='.$random.'&dosubmit=1&tabletype='.$tabletype.'&allow='.$allow.'&pdo_select='.$this->pdo_name);
		} else {
		   ...
		   delcache('bakup_tables','commons');
		   showmessage(L('bakup_succ'),'?app=admin&controller=database&view=import&pdoname='.$this->pdo_name);
		}
	}
	/**
	 * 数据库恢复
	...
	private function import_database($filename) {
		if($filename && fileext($filename)=='sql') {
			...
			if(file_exists($filepath)) {
				...
				$fileid++;
				showmessage(L('bakup_data_file')." $filename ".L('load_success'),"?app=admin&controller=database&view=import&pdoname=".$this->pdo_name."&pre=".$pre."&fileid=".$fileid."&dosubmit=1");
			} else {
				showmessage(L('data_recover_succ'),'?app=admin&controller=database&view=import');
			}
		}
	}
	 ...
	 */
 	...
 	private function sql_split($sql) {
		...
		if($this->db_tablepre != "shuyang_") $sql = str_replace("`shuyang_", '`'.$this->db_tablepre, $sql);
		$sql = str_replace("\r", "\n", $sql);
		$ret = array();
		$num = 0;
