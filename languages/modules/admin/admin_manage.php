...
class admin_manage extends admin {
	...
	function __construct() {
		...
		$this->op = shy_base::load_app_class('admin_op');
	}
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$lockscreeninfo = lockscreen($info['lockscreen']);
			$info['lockscreen'] = $lockscreeninfo['lockscreen'];
			$info['lockscreenencrypt'] = $lockscreeninfo['lockscreenencrypt'];
			
			$admin_fields = array('username', 'email', 'password', 'encrypt', 'lockscreen', 'lockscreenencrypt','roleid','realname');
			...
			if($this->db->insert_id()){
				showmessage(L('operation_success'),'?app=admin&controller=admin_manage');
			}
		} else {
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			if(isset($info['lockscreen']) && !empty($info['lockscreen']))
			{
				$this->op->edit_lockscreen($info['userid'], $info['lockscreen']);
			}
			$userid = $info['userid'];
	...
	public function public_edit_pwd() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			showmessage(L('password_edit_succ_logout'),'?app=admin&controller=index&view=public_logout');			
		} else {
	 ...
	 * 管理员修改锁屏密码
	 */
	public function public_lockscreen_pw() {
		$userid = $_SESSION['userid'];
		if(isset($_POST['dosubmit'])) {
			$r = $this->db->get_one(array('userid'=>$userid),'lockscreen,lockscreenencrypt');
			if ( lockscreen($_POST['old_lockscreen'],$r['lockscreenencrypt']) !== $r['lockscreen'] ) showmessage(L('old_password_wrong'),HTTP_REFERER);
			if(isset($_POST['new_lockscreen']) && !empty($_POST['new_lockscreen'])) {
				$this->op->edit_lockscreen($userid, $_POST['new_lockscreen']);
			}
			showmessage(L('lockscreen_edit_succ'),'?app=admin&controller=index&view=public_main');	
		} else {
			$info = $this->db->get_one(array('userid'=>$userid));
			extract($info);
			include $this->admin_tpl('admin_lockscreen_pwd');			
		}
	...
	public function public_edit_info() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$lang_dirs = glob(SHY_PATH.'languages/*');
			$dir_array = array();
			foreach($lang_dirs as $dirs) {
				$dir_array[] = str_replace(SHY_PATH.'languages/','',$dirs);
			}
	 ...
	 * 异步检测锁屏密码
	 */
	function public_lockscreen_ajx() {
		$userid = $_SESSION['userid'];
		$r = array();
		$r = $this->db->get_one(array('userid'=>$userid),'lockscreen,lockscreenencrypt');
		if ( lockscreen($_GET['old_lockscreen'],$r['lockscreenencrypt']) == $r['lockscreen'] ) {
			exit('1');
		}
		exit('0');
	}
	...
	//电子口令卡
	function card() {
		if (shy_base::load_config('system', 'safe_card') != 1) {
		...
		if ($data = $this->db->get_one(array('userid'=>$userid), '`card`,`username`')) {
			if (empty($data['card'])) {
				...
				if ($this->db->update(array('card'=>$card), array('userid'=>$userid))) {
					showmessage(L('password_card_application_success'), '?app=admin&controller=admin_manage&view=card&userid='.$userid);
				} else {
	...
	function remove_card() {
		...
		if ($data = $this->db->get_one(array('userid'=>$userid), '`card`,`username`,`userid`')) {
			...
			if ($result = card::remove_card($data['card'])) {
					...
					showmessage(L('the_binding_success'), '?app=admin&controller=admin_manage&view=card&userid='.$userid);
			}
		} else {
			showmessage(L('users_were_not_found'));
		}
	...
	private function check_admin_manage_code(){
		...
		$shy_auth_key = md5(shy_base::load_config('system','auth_key').'adminuser');
		$admin_manage_code = sys_auth($admin_manage_code, 'DECODE', $shy_auth_key);	
		if($admin_manage_code==""){
			return false;
		}
		$admin_manage_code = explode("_", $admin_manage_code);
		if($admin_manage_code[0]!="adminuser" || $admin_manage_code[1]!=$_POST['safe_edi']){
			return false;
		}
		return true;
	}
	...
	private function get_admin_manage_code(){
		$shy_auth_key = md5(shy_base::load_config('system','auth_key').'adminuser');
		$code = sys_auth("adminuser_".$_GET['safe_edi']."_".time(), 'ENCODE', $shy_auth_key);
		return $code;
	}	
}
?>