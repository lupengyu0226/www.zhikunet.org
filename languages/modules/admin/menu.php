...
class menu extends admin {
	function __construct() {
		parent::__construct();
		$this->db = shy_base::load_model('menu_model');
	}
	...
	function init () {
		...
		foreach($result as $r) {
			$r['cname'] = L($r['name']);
			$r['str_manage'] = '<a class="xbtn btn-info btn-xs" href="?app=admin&controller=menu&view=add&parentid='.$r['id'].'&menuid='.$_GET['menuid'].'">'.L('add_submenu').'</a>   <a class="xbtn btn-warning btn-xs" href="?app=admin&controller=menu&view=edit&id='.$r['id'].'&menuid='.$_GET['menuid'].'">'.L('modify').'</a>   <a class="xbtn btn-danger btn-xs" href="javascript:confirmurl(\'?app=admin&controller=menu&view=delete&id='.$r['id'].'&menuid='.$_GET['menuid'].'\',\''.L('confirm',array('message'=>$r['cname'])).'\')">'.L('delete').'</a> ';
			$array[] = $r;
		}
	...
	function add() {
		if(isset($_POST['dosubmit'])) {
			...
			//开发过程中用于自动创建语言包
			$file = SHY_PATH.'languages'.DIRECTORY_SEPARATOR.'zh-cn'.DIRECTORY_SEPARATOR.'system_menu.lang.php';
			if(file_exists($file)) {
	...
	private function update_menu_models($id, $old_data, $new_data) {
		$models_config = shy_base::load_config('model_config');
