...
$(function(){
	...
	$("#username").formValidator({onshow:"<?php echo L('input').L('username')?>",onfocus:"<?php echo L('username').L('between_2_to_20')?>"}).inputValidator({min:2,max:20,onerror:"<?php echo L('username').L('between_2_to_20')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data :"app=admin&controller=admin_manage&view=public_checkname_ajx",
		datatype : "html",
		async:'false',
	...
	$("#lockscreen").formValidator({onshow:"<?php echo L('input').L('lockscreentips')?>",onfocus:"<?php echo L('lockscreen').L('between_6_to_20')?>"}).inputValidator({min:6,max:20,onerror:"<?php echo L('lockscreen').L('between_6_to_20')?>"});
...
<form name="myform" action="?app=admin&controller=admin_manage&view=add" method="post" id="myform">
...
<tr>
<td><?php echo L('lockscreen')?></td> 
<td><input type="password" name="info[lockscreen]" class="input-text" id="lockscreen" value="12345678"></input></td>
</tr>
</tr>
<tr>
