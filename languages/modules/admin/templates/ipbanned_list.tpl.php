...
?>
<div class="pad-lr-10">
<form name="searchform" id="searchform" action="?app=admin&controller=ipbanned&view=search_ip&menuid=<?php echo $_GET['menuid'];?>" method="get"  >
<input type="hidden" value="admin" name="app">
<input type="hidden" value="ipbanned" name="controller">
<input type="hidden" value="search_ip" name="view">
...
<form name="myform" id="myform" action="?app=admin&controller=ipbanned&view=delete" method="post" onsubmit="checkuid();return false;">
...
?>
	...
	<input type="checkbox" name="ipbannedid[]" value="<?php echo $info['ipbannedid']?>">
        ...
        <td align="center"><?php echo date('Y-m-d', $info['expires']);?></td>
         <td align="center"><a class="xbtn btn-danger btn-xs" href="javascript:confirmurl('?app=admin&controller=ipbanned&view=delete&ipbannedid=<?php echo $info['ipbannedid']?>', '<?php echo L('confirm', array('message' => L('selected')))?>')"><?php echo L('delete')?></a> </td>
    </tr>
<?php
	}
}
