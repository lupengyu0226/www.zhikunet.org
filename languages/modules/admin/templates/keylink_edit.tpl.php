...
<script type="text/javascript">
	$(function(){
		...
		$("#word").formValidator({onshow:"<?php echo L('input').L('keylink');?>",onfocus:"<?php echo L('input').L('keylink');?>"}).inputValidator({min:1,onerror:"<?php echo L('input').L('keylink');?>"}).regexValidator({regexp:"notempty",datatype:"enum",param:'i',onerror:"<?php echo L('en_tips_type');?>"}).ajaxValidator({type : "get",url : "",data :"app=admin&controller=keylink&view=public_name&keylinkid=<?php echo $keylinkid?>",datatype : "html",async:'false',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "<?php echo L('keylink').L('exists');?>",onwait : "<?php echo L('connecting');?>"}).defaultPassed(); 
...
<form action="?app=admin&controller=keylink&view=edit&keylinkid=<?php echo $keylinkid?>" method="post" name="myform" id="myform">
 	<tr> 
