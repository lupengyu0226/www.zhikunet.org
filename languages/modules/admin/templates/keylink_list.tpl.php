...
?>
<div class="pad-lr-10">
<form name="myform" id="myform" action="?app=admin&controller=keylink&view=delete" method="post" onsubmit="checkuid();return false;">
...
?>
	...
	<input type="checkbox" name="keylinkid[]" value="<?php echo $info['keylinkid']?>">
        ...
        <td align="center"><?php echo $info['url']?></td>
         <td align="center"><a class="xbtn btn-info btn-xs" href="javascript:edit(<?php echo $info['keylinkid']?>, '<?php echo new_addslashes($info['word'])?>')"><?php echo L('edit')?></a>   <a class="xbtn btn-danger btn-xs" href="javascript:confirmurl('?app=admin&controller=keylink&view=delete&keylinkid=<?php echo $info['keylinkid']?>', '<?php echo L('keylink_confirm_del')?>')"><?php echo L('delete')?></a> </td>
    </tr>
<?php
	}
}
...
function edit(id, name) {
	...
	window.top.art.dialog({title:'<?php echo L('keylink_edit')?> '+name+' ',id:'edit',iframe:'?app=admin&controller=keylink&view=edit&keylinkid='+id,width:'450',height:'130'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
}
