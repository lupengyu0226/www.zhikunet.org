...
<!--
	$(function(){
		...
		$("#siteurl").formValidator({onshow:"<?php echo L('input').L('copyfrom_url');?>",onfocus:"<?php echo L('input').L('copyfrom_url');?>",empty:false}).inputValidator({onerror:"<?php echo L('input').L('copyfrom_url');?>"}).regexValidator({regexp:"^http[s]?:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&]*([^<>])*$",onerror:"<?php echo L('copyfrom_url_tips');?>"}).defaultPassed();
	})
//-->
</script>

<div class="pad_10">
<form action="?app=admin&controller=copyfrom&view=edit" method="post" name="myform" id="myform" >
