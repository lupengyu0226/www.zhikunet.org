...
<!--
	$(function(){
		...
		$("#catdir").formValidator({onshow:"<?php echo L('input_dirname');?>",onfocus:"<?php echo L('input_dirname');?>"}).regexValidator({regexp:"^([a-zA-Z0-9、-]|[_]){0,30}$",onerror:"<?php echo L('enter_the_correct_catname');?>"}).inputValidator({min:1,onerror:"<?php echo L('input_dirname');?>"}).ajaxValidator({type : "get",url : "",data :"app=admin&controller=category&view=public_check_catdir",datatype : "html",cached:false,getdata:{parentid:'parentid'},async:'false',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "<?php echo L('catname_have_exists');?>",onwait : "<?php echo L('connecting');?>"});
		$("#url").formValidator({onshow:" ",onfocus:"<?php echo L('domain_name_format');?>",tipcss:{width:'300px'},empty:true}).inputValidator({onerror:"<?php echo L('domain_name_format');?>"}).regexValidator({regexp:"^(http(s)?:)?\\/\\/([a-z0-9\\-\\.]+)\\/$",onerror:"<?php echo L('domain_end_string');?>"});
...
<form name="myform" id="myform" action="?app=admin&controller=category&view=add" method="post">
<div class="pad-10">
<div class="col-tab">
<ul class="tabBut cu-li">
<li id="tab_setting_1" class="on" onclick="SwapTab('setting','on','',6,1);"><i class="iconfont icon-shezhi"></i> <?php echo L('catgory_basic');?></li>
<li id="tab_setting_2" onclick="SwapTab('setting','on','',6,2);"><i class="iconfont icon-html"></i> <?php echo L('catgory_createhtml');?></li>
<li id="tab_setting_3" onclick="SwapTab('setting','on','',6,3);"><i class="iconfont icon-houtaimobanguanlinew"></i> <?php echo L('catgory_template');?></li>
<li id="tab_setting_4" onclick="SwapTab('setting','on','',6,4);"><i class="iconfont icon-seo"></i> <?php echo L('catgory_seo');?></li>
<li id="tab_setting_5" onclick="SwapTab('setting','on','',6,5);"><i class="iconfont icon-quanxian"></i> <?php echo L('catgory_private');?></li>
<li id="tab_setting_6" onclick="SwapTab('setting','on','',6,6);"><i class="iconfont icon-charge"></i> <?php echo L('catgory_readpoint');?></li>
</ul>
 ...
 <tr>
     <th><?php echo L('add_category_types');?>：</th>
      ...
      <tr>
        ...
        <td>
        <span id="normal_add"><input type="text" name="info[catname]" id="catname" class="input-text" value="" onblur="getpy(this.value,'catdir')"></span>
	...
	<tr>
     ...
     <tr>
        <th><?php echo L('area_id')?>：</th>
     <td><?php echo form::menu_linkage(39, 'area_id', 0);?></td>
      </tr>
	  <tr>
        <th><?php echo L('major_id')?>：</th>
     <td><?php echo form::menu_linkage(510, 'major_id', 0);?></td>
      </tr>
	<tr>
...
<!--
	...
	function change_tpl(modelid) {
		if(modelid) {
			$.getJSON('?app=admin&controller=category&view=public_change_tpl&modelid='+modelid, function(data){$('#template_list').val(data.template_list);$('#category_template').html(data.category_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);});
		}
	}
	function load_file_list(id) {
		if(id=='') return false;
		$.getJSON('?app=admin&controller=category&view=public_tpl_file_list&style='+id+'&catid=<?php echo $parentid?>', function(data){$('#category_template').html(data.category_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);});
	}
	<?php if($modelid) echo "change_tpl($modelid)";?>
function getpy(str,toid){
        if(!str) {
   alert('未填写栏目名称');
   return false;
   }
  $.get('<?php echo APP_PATH;?>index.php?app=api&controller=index&op=pinyin&rands='+Math.random(),
           {pinyinstr:str},
     function(data){
  if( data!='0' && $('#' + toid) ){
  $('#' + toid).val(data);
