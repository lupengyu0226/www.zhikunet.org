...
?>   
	<tr>
	<td width="10%">
<input type="checkbox" name="filenames[]" value="<?php echo $info['filename']?>" id="sql_shuyang" boxid="sql_shuyang">
	</td>
	...
	<a href="javascript:confirmurl('?app=admin&controller=database&pdoname=<?php echo $pdoname?>&view=import&pre=<?php echo $info['pre']?>&dosubmit=1', '<?php echo L('confirm_recovery')?>')"><?php echo L('backup_import')?></a>
	</td>
	</tr>
<?php 
	}
...
<input type="submit" class="button" name="dosubmit" value="<?php echo L('backup_del')?>" onclick="document.myform.action='?app=admin&controller=database&view=delete&pdoname=<?php echo $pdoname?>';return confirm('<?php echo L('bakup_del_confirm')?>')"/>
</div>
</form>
</div>
</div>
...
function show_tbl(obj) {
	var pdoname = $(obj).val();
	location.href='?app=admin&controller=database&view=import&pdoname='+pdoname+'&menuid='+<?php echo $_GET['menuid']?>+'&safe_edi=<?php echo $_SESSION['safe_edi']?>';
}
//-->
</script>
