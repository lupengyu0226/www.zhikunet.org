...
<form method="post" name="myform" id="myform" action="?app=admin&controller=database&view=export">
<input type="hidden" name="tabletype" value="db" id="shuyangtables">
...
?>   
    ...
    <tbody>
	<?php foreach($infos['shuyangtables'] as $v){?>
	<tr>
	...
	<td  width="15%" align="center"><a class="xbtn btn-info btn-xs" href="?app=admin&controller=database&view=public_repair&operation=optimize&pdo_name=<?php echo $pdo_name?>&tables=<?php echo $v['name']?>"><?php echo L('database_optimize')?></a>   <a class="xbtn btn-warning btn-xs" href="?app=admin&controller=database&view=public_repair&operation=repair&pdo_name=<?php echo $pdo_name?>&tables=<?php echo $v['name']?>"><?php echo L('database_repair')?></a>   <a class="xbtn btn-inverse btn-xs" href="javascript:void(0);" onclick="showcreat('<?php echo $v['name']?>','<?php echo $pdo_name?>')"><?php echo L('database_showcreat')?></a></td>
	</tr>
	<?php } ?>
	</tbody>
<?php 
...
<input type="submit" class="button" name="dosubmit" onclick="document.myform.action='?app=admin&controller=database&view=public_repair&operation=optimize&pdo_name=<?php echo $pdo_name?>'" value="<?php echo L('batch_optimize')?>"/>
<input type="submit" class="button" name="dosubmit" onclick="document.myform.action='?app=admin&controller=database&view=public_repair&operation=repair&pdo_name=<?php echo $pdo_name?>'" value="<?php echo L('batch_repair')?>"/>
</div>
<?php 
}
?>
...
function show_tbl(obj) {
	var pdoname = $(obj).val();
	location.href='?app=admin&controller=database&view=export&pdoname='+pdoname+'&safe_edi=<?php echo $_SESSION['safe_edi']?>';
}
function showcreat(tblname, pdo_name) {
	window.top.art.dialog({title:tblname, id:'show', iframe:'?app=admin&controller=database&view=public_repair&operation=showcreat&pdo_name='+pdo_name+'&tables=' +tblname,width:'500px',height:'350px'});
}
function reselect() {
