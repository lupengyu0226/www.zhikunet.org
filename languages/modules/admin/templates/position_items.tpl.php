...
?>
<form name="myform" action="?app=admin&controller=position&view=public_item" method="post">
...
?>   
	...
	<td width="15%" align="center"><a class="xbtn btn-info btn-xs" href="<?php echo $info['url']?>" target="_blank"><?php echo L('posid_item_view')?></a>   <a class="xbtn btn-primary btn-xs" onclick="javascript:openwinx('?app=content&controller=content&view=edit&catid=<?php echo $info['catid']?>&id=<?php echo $info['id']?>','')" href="javascript:;"><?php echo L('posid_item_edit');?></a>   <a class="xbtn btn-warning btn-xs" href="javascript:item_manage(<?php echo $info['id']?>,<?php echo $info['posid']?>, <?php echo $info['modelid']?>,'<?php echo $info['title']?>')"><?php echo L('posid_item_manage')?></a>
	</td>
	</tr>
<?php 
	}
...
<script type="text/javascript">
	...
	window.top.art.dialog({title:'<?php echo L('edit')?>--'+name, id:'edit', iframe:'?app=admin&controller=position&view=public_item_manage&id='+id+'&posid='+posid+'&modelid='+modelid ,width:'550px',height:'430px'}, 	function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;
