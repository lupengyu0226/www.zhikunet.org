...
<script language="javascript" type="text/javascript" src="<?php echo JS_PATH?>jquery.sgallery1.2.js"></script>
<script type="text/javascript">
var safe_edi = '<?php echo $_SESSION['safe_edi']?>'
</script>
<style type="text/css">
.objbody{overflow:hidden}
.btns{position: absolute; top:128px; right:13px; z-index:1000;}
.btns2{background-color:rgba(0,0,0,0.5); color:#fff; padding:2px; border-radius:3px; box-shadow:0px 0px 2px #333; padding:0px 6px; border:0px solid #ddd;}
...
.bg_btn{width:32px; height:32px;}
#loading{width:80px;height:44px;position: absolute;top:50%;left:50%;line-height:44px;color:#fff;padding-left:60px;font-size:15px;background: #000 url(<?php echo IMG_PATH?>loading087.gif) no-repeat 10px 50%;opacity: 0.7;z-index:9999;-moz-border-radius:10px;-webkit-border-radius:10px;border-radius:10px;filter:progid:DXImageTransform.Microsoft.Alpha(opacity=70);}
</style>
</head>
<body scroll="no" class="objbody">
<div id="loading">加载中...</div>
<div class="btns btns2" id="btnx">
<div class="bg_btn"><i class="iconfont icon-2x icon-caidan"></i></div>
<?php $model_types = shy_base::load_config('model_config');?>
...
<div class="header">
	<div class="logo lf"><a href="<?php echo $currentsite['domain']?>" target="_blank"><span class="invisible"><?php echo L('shuyang_title')?></span></a></div>
    <div class="rt-col">
    	<div class="tab_style white cut_line text-r"><?php echo L('hello'),$admin_username?>  <a href="?app=admin&controller=index&view=public_logout"><?php echo L('exit')?></a> <a href="javascript:;" onclick="lock_screen()"> <?php echo L('lockscreens')?></a>
    <ul id="Skin">
    ...
    <div class="col-auto">
    	<div class="white cut_line">   		
    	</div>
        <ul class="headermenu white" id="top_menu">
        <?php
        $array = admin::admin_menu(0);
        foreach((array)$array as $_value) {
        	if($_value['id']==10) {
        		echo '<li id="_M'.$_value['id'].'" class="on top_menu"><a href="javascript:_M('.$_value['id'].',\'?app='.$_value['m'].'&controller='.$_value['c'].'&view='.$_value['a'].'\')" hidefocus="true" style="outline:none;"><span class="icon"><i class="iconfont icon-2x '.$_value['icon'].'"></i></span>'.L($_value['name']).'</a></li>';
        		
        	} else {
        		echo '<li id="_M'.$_value['id'].'" class="top_menu"><a href="javascript:_M('.$_value['id'].',\'?app='.$_value['m'].'&controller='.$_value['c'].'&view='.$_value['a'].'\')"  hidefocus="true" style="outline:none;"><span class="icon"><i class="iconfont icon-2x '.$_value['icon'].'"></i></span>'.L($_value['name']).'</a></li>';
        	}      	
        }
        ?>
        </ul>
...
<div id="content">
    ...
    <div class="crumbs">
    <div class="shortcut cu-span"><a href="?app=content&controller=create_html&view=public_index&safe_edi=<?php echo $_SESSION['safe_edi'];?>" target="right"><span><?php echo L('create_index')?></span></a><a href="?app=content&controller=create_html&view=public_gundong&safe_edi=<?php echo $_SESSION['safe_edi'];?>" target="right"><span>生成最新</span></a><a href="?app=admin&controller=cache_all&view=init&safe_edi=<?php echo $_SESSION['safe_edi'];?>" target="right"><span><?php echo L('update_backup')?></span></a><a href="javascript:art.dialog({id:'map',iframe:'?app=admin&controller=index&view=public_map', title:'<?php echo L('background_map')?>', width:'700', height:'500', lock:true});void(0);"><span><?php echo L('background_map')?></span></a><div class="admin_top_left_menu" id="admin_top_left_menu"><?php echo runhook('admin_top_left_menu')?></div>

    </div>
		<div class="shortcutl cu-span">
		<ul class="clearfix list-inline" id="current_pos">
		     <li><i class="iconfont icon-houtaiguanli"></i> <?php echo L('current_position')?></li>
		</ul></div></div>
    	<div class="col-1">
        	<div class="content" style="position:relative; overflow:hidden">
                <iframe name="right" id="rightMain" src="?app=admin&controller=index&view=public_main" frameborder="false" scrolling="auto" style="border:none; margin-bottom:30px" width="100%" height="auto" allowtransparency="true" onload="showloading()"></iframe>
                <div class="fav-nav">
					<div id="panellist">
					<?php $delete_panel = shy_base::load_config('system','delpanel'); foreach($adminpanel as $v) {?>
					<span><a onclick="paneladdclass(this);" target="right" href="<?php echo $v['url'].'menuid='.$v['menuid'].'&safe_edi='.$_SESSION['safe_edi'];?>"><?php echo L($v['name'])?></a>
					<?php if($delete_panel==0){?><a class="panel-delete" href="javascript:delete_panel(<?php echo $v['menuid']?>, this);"></a><?php }?></span>
					<?php }?>
					</div>
					<div id="paneladd"></div>
					<input type="hidden" id="menuid" value="">
					<input type="hidden" id="bigid" value="" />
					<div class="fav-help">Copyright © 2007-2019 智库网 zhikunet.org All Rights Reserved.</div>
				</div>
        	</div>
        </div>
    </div>
...
<?php foreach ($sitelist as $key=>$v):?>
	<li style="margin:0"><a href="javascript:site_select(<?php echo $v['siteid']?>, '', '<?php echo $v['domain']?>', '<?php echo $v['siteid']?>')"><?php echo $v['name']?></a></li>
<?php endforeach;?>
</ul>
</div>
...
$(function(){
	...
	//默认载入左侧菜单
	$("#leftMain").load("?app=admin&controller=index&view=public_menu_left&menuid=10");
...
function _M(menuid,targetUrl) {
	$("#menuid").val(menuid);
	$("#bigid").val(menuid);
	$("#paneladd").html('<a class="panel-add" href="javascript:add_panel();"><em><i class="iconfont icon-tianjia" style="color: white;"></em></a>');
	if(menuid!=8) {
		$("#leftMain").load("?app=admin&controller=index&view=public_menu_left&menuid="+menuid, {limit: 25}, function(){
		   windowW();
		 });
	} else {
		$("#leftMain").load("?app=admin&controller=phpsso&view=public_menu_left&menuid="+menuid, {limit: 25}, function(){
		   windowW();
		 });
	}
...
function _MP(menuid,targetUrl) {
	...
	$("#rightMain").attr('src', targetUrl+'&menuid='+menuid+'&safe_edi='+safe_edi);
...
function add_panel() {
	...
	$.ajax({
		type: "POST",
		url: "?app=admin&controller=index&view=public_ajax_add_panel",
		data: "menuid=" + menuid,
...
function delete_panel(menuid, id) {
	$.ajax({
		type: "POST",
		url: "?app=admin&controller=index&view=public_ajax_delete_panel",
		data: "menuid=" + menuid,
		success: function(data){
			$("#panellist").html(data);
		}
...
function session_life() {
	$.get("?app=admin&controller=index&view=public_session_life");
}
function lock_screen() {
	$.get("?app=admin&controller=index&view=public_lock_screen");
...
function check_screenlock() {
	...
	$.get("?app=admin&controller=index&view=public_login_screenlock", { lock_password: lock_password},function(data){
		if(data==1) {
...
function _Site_M(project) {
	...
	$.getJSON('index.php', {app:'admin', controller:'index', view:'public_set_model', 'site_model':project, 'time':Math.random()}, function (data){
		$.each(data, function(i, n){
			$('#_M'+n).show();
		})
	})
	$("#leftMain").load("?app=admin&controller=index&view=public_menu_left&menuid="+id+'&time='+Math.random());
}

<?php if($site_model) { ?> _Site_M('<?php echo $site_model?>'); <?php }?>
function createpos(){
        $.get("index.php?app=ssi&controller=ssi&view=public_ssi");
        $("#admin_top_left_menu").load(location.href+" #admin_top_left_menu>*","");
        }
setInterval(createpos,30000);
</script>
</body>
</html>
