...
?>
<div class="pad-lr-10">
<form name="myform" id="myform" action="?app=admin&controller=badword&view=delete" method="post" onsubmit="checkuid();return false;">
...
?>
	...
	<input type="checkbox" name="badid[]" value="<?php echo $info['badid']?>">
        ...
        <td align="center"><?php echo $info['lastusetime'] ? date('Y-m-d H:i', $info['lastusetime']):''?></td>
         <td align="center"><a class="xbtn btn-info btn-xs" href="javascript:edit(<?php echo $info['badid']?>, '<?php echo new_addslashes($info['badword'])?>')"><?php echo L('edit')?></a>   <a class="xbtn btn-danger btn-xs" href="javascript:confirmurl('?app=admin&controller=badword&view=delete&badid=<?php echo $info['badid']?>', '<?php echo L('badword_confirm_del')?>')"><?php echo L('delete')?></a> </td>
    </tr>
<?php
	}
}
...
function edit(id, name) {
	...
	window.top.art.dialog({title:'<?php echo L('badword_edit')?> '+name+' ',id:'edit',iframe:'?app=admin&controller=badword&view=edit&badid='+id,width:'450',height:'180'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
}
