...
?>
<div class="pad-lr-10">
<form name="myform" action="?app=license&controller=admin_license&view=delete_type" method="post" onsubmit="checkuid();return false;">
...
<a class="xbtn btn-info btn-xs"  href="###" onclick="edit(<?php echo $info['typeid']?>, '<?php echo new_addslashes($info['name'])?>')" title="<?php echo L('edit')?>"><?php echo L('edit')?></a> 
  <a class="xbtn btn-danger btn-xs" href='?app=license&controller=admin_license&view=delete_type&typeid=<?php echo $info['typeid']?>'onClick="return confirm('<?php echo L('confirm', array('message' => new_addslashes($info['name'])))?>')"><?php echo L('delete')?></a>
		</td>
	</tr>
	<?php
	}
...
function edit(id, name) {
	...
	window.top.art.dialog({title:'<?php echo L('edit')?> '+name+' ',id:'edit',iframe:'?app=license&controller=admin_license&view=edit_type&typeid='+id,width:'450',height:'280'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
}
function checkuid() {
	var ids='';
