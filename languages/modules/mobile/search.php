...
class search {
	function __construct() {
		...
		define('MOBILE_SITEURL',shy_base::load_config('system','mobile_path'));
	}
	...
	public function init() {
		...
		if(isset($_GET['q'])) {
			if(trim($_GET['q'])=='') {
				header('Location: '.MOBILE_SITEURL.'index.php?controller=search');exit;
			}
			...
			$mori = '';
			if($q == '2012世界末日'){
			$mori = '<script type="text/javascript" src="https://statics.05273.cn/statics/js/mori.js"></script>';
			}
			if($q == '世界末日'){
			$mori = '<script type="text/javascript" src="https://statics.05273.cn/statics/js/mori.js"></script>';
			}
			...
			if($page == 1 && $_GET['q']==$q) {//翻页不重复记录
				...
				if($search_word){
					...
					if($_SESSION['check_search'] == time()) showmessage(L('未知错误'));
			...
			include	template('mobile','search_list');
		} else {
			include	template('mobile','search_index');
		}
	}
