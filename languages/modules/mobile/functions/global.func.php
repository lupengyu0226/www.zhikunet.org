...
function list_url($catid) {
    return MOBILE_SITEURL."list-$catid-1.html";
}
...
function show_url($catid, $id) {
    return MOBILE_SITEURL."show-$catid-$id-1.html";
}
/**
 * 解析分类MIPurl路径
 */
function miplist_url($catid) {
    return MOBILE_SITEURL."miplist-$catid-1.html";
}
/**
 * 解析内容 MIP url路径
 * $catid 栏目id
...
function mip_url($catid, $id) {
    return MOBILE_SITEURL."mip-$catid-$id-1.html";
}
...
function mobile_pos($catid, $symbol=' > '){
	$category_arr = array();
	...
	foreach($arrparentid as $catid) {
		...
		$pos .= '<a href="'.S($url).'">'.$category_arr[$catid]['catname'].'</a>'.$symbol;
		}
	}else{
		$url=$category_arr[$category_arr[$catid]["parentid"]]['url'];
		$pos .= '<a href="'.S($url).'">'.$category_arr[$category_arr[$catid]["parentid"]]['catname'].'</a>';
...
function mip_pos($catid, $symbol=' > '){
	...
	foreach($arrparentid as $catid) {
		...
		$pos .= '<a href="'.miplist_url($catid).'">'.$category_arr[$catid]['catname'].'</a>'.$symbol;
	}
	return $pos;
}
