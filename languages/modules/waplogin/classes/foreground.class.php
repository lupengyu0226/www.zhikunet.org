...
class foreground {
	...
	final public function check_member() {
		...
		if(ROUTE_M =='waplogin' && ROUTE_C =='index' && in_array(ROUTE_A, array('login', 'register', 'mini','top_mini','weixin','blog_mini','send_newmail'))) {
			if ($shuyang_auth && ROUTE_A != 'mini'&& ROUTE_A != 'top_mini'&& ROUTE_A != 'blog_mini') {
				mobilemsg(L('login_success', '', 'member'), 'index.php?app=waplogin&controller=index');
			} else {
				return true;
			}
		} else {
			...
			if ($shuyang_auth) {
				...
				if($this->memberinfo && $this->memberinfo['password'] === $password) {
					...
					if($this->memberinfo['groupid'] == 1) {
						...
						mobilemsg(L('userid_banned_by_administrator', '', 'member'), 'index.php?app=waplogin&controller=index&view=login');
						...
						mobilemsg(L('need_emial_authentication', '', 'member'), 'index.php?app=waplogin&controller=index&view=register&t=2');
					}
				} else {
					param::set_cookie('auth', '');
					param::set_cookie('_userid', '');
				...
				mobilemsg(L('please_login', '', 'member'), 'index.php?app=waplogin&controller=index&view=login&forward='.$forward);
			}
		}
	}
	/**
