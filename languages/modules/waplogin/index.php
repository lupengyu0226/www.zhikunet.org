...
shy_base::load_app_class('foreground');
...
class index extends foreground {
	...
	public function init() {
		...
		include template('mobile', 'login_index');
	}
	...
	public function register() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($member_setting['mobile_checktype']=='1'){
				...
				if($mobile_verify=='') mobilemsg('请提供正确的手机验证码！', HTTP_REFERER);
 				$sms_report_db = shy_base::load_model('sms_report_model');
				...
				$r = $sms_report_db->get_one($where,'*','id DESC');
 				if(!empty($r)){
					...
					mobilemsg('未检测到正确的手机号码！', HTTP_REFERER);
				}
			...
			if($userinfo['mobile']!=""){
				if(!preg_match('/^1([0-9]{10})$/',$userinfo['mobile'])) {
					mobilemsg('请提供正确的手机号码！', HTTP_REFERER);
				}
			} 
			...
			if($member_setting['enablemailcheck']) {	//是否需要邮件验证
				...
				$this->verify_db = shy_base::load_model('member_verify_model');
				unset($userinfo['lastdate'],$userinfo['connectid'],$userinfo['from']);
				$userinfo['modelinfo'] = $modelinfo_str;
				$this->verify_db->insert($userinfo);
				mobilemsg(L('operation_success'), 'index.php?app=waplogin&controller=index&view=register&t=3');
			} else {
				//查看当前模型是否开启了短信验证功能
				$model_field_cache = getcache('model_field_'.$userinfo['modelid'],'model');
				if($member_setting['mobile_checktype']=='2') {
					$mobile = $_POST['mobile'];
					if(!preg_match('/^1([0-9]{10})$/',$mobile)) mobilemsg(L('input_right_mobile'));
					$sms_report_db = shy_base::load_model('sms_report_model');
					$posttime = SYS_TIME-300;
					$where = "`mobile`='$mobile' AND `posttime`>'$posttime'";
					$order = "`posttime` desc";
					$r = $sms_report_db->get_one($where, '*', $order);
					if(!$r || $r['id_code']!=$_POST['mobile_verify']) mobilemsg(L('error_sms_code'));
					$where = "`mobile`='$mobile' AND `posttime`>'$posttime'";
					$sms_report_db->get_one($where,'id,id_code','id DESC');
				}
				...
				if($status > 0) {
					...
					if($userid > 0) {
						...
						if($userinfo['groupid'] == 7) {
							...
							$shuyang_auth = sys_auth($userid."\t".$userinfo['password'], 'ENCODE', get_auth_key('login'));
					...
					if($member_setting['enablemailcheck']) {
						...
						$url = APP_PATH."index.php?app=waplogin&controller=index&view=register&code=$code&verify=1";
						...
						mobilemsg(L('operation_success'), 'index.php?app=waplogin&controller=index&view=register&t=2');
					} else {
						//如果不需要邮箱认证、直接登录其他应用
						$synloginstr = $this->client->ps_member_synlogin($userinfo['phpssouid']);
						mobilemsg(L('operation_success').$synloginstr, 'index.php?app=waplogin&controller=index');
					}
					
				}
			} else {
				mobilemsg(L('enable_register').L('enable_phpsso'), 'index.php?app=waplogin&controller=index&view=login');
			}
			mobilemsg(L('operation_failure'), HTTP_REFERER);
		} else {
			if(!shy_base::load_config('system', 'phpsso')) {
				mobilemsg(L('enable_register').L('enable_phpsso'), 'index.php?app=waplogin&controller=index&view=login');
			}
			...
			if(!empty($_GET['verify'])) {
				$code = isset($_GET['code']) ? trim($_GET['code']) : mobilemsg(L('operation_failure'), 'index.php?app=waplogin&controller=index');
				$code_res = sys_auth($code, 'DECODE', get_auth_key('email'));
				$code_arr = explode('|', $code_res);
				$userid = isset($code_arr[0]) ? $code_arr[0] : '';
				$userid = is_numeric($userid) ? $userid : mobilemsg(L('operation_failure'), 'index.php?app=waplogin&controller=index');

				$this->db->update(array('groupid'=>$this->_get_usergroup_bypoint()), array('userid'=>$userid));
				mobilemsg(L('operation_success'), 'index.php?app=waplogin&controller=index');
			} elseif(!empty($_GET['protocol'])) {

				include template('mobile', 'protocol');
			} else {
				//过滤非当前站点会员模型
				...
				if(empty($modellist)) {
					mobilemsg(L('site_have_no_model').L('deny_register'), HTTP_REFERER);
				}
				//是否开启选择会员模型选项
	...
	public function comment() {
		...
		$comment_data_db->delete(array('userid'=>$memberinfo['userid'], 'id'=>intval($_GET['id'])));
			mobilemsg(L('operation_success'), HTTP_REFERER);
		} else {
                ...
                include template('mobile', 'login_comment_list');
		}
	}
	public function account_manage() {
		...
		include template('mobile', 'account_manage');
	}
	...
	public function account_manage_avatar() {
		...
		//初始化phpsso
		$phpsso_api_url = $this->_init_phpsso();
		$ps_auth_key = shy_base::load_config('system', 'phpsso_auth_key');
		$auth_data = $this->client->auth_data(array('uid'=>$this->memberinfo['phpssouid'],'sys_auth_time'=>microtime(true)), '', $ps_auth_key);
		$upurl = base64_encode($phpsso_api_url.'/index.php?app=phpsso&controller=index&view=uploadavatar&auth_data='.$auth_data);
		//获取头像数组
		$avatar = $this->client->ps_getavatar($this->memberinfo['phpssouid']);
	...
	public function account_manage_info() {
		if(isset($_POST['dosubmit'])) {
			...
			include template('mobile', 'account_manage_info');
		}
	}
	...
	public function account_manage_password() {
		if(isset($_POST['dosubmit'])) {
			...
			if(!is_password($_POST['info']['password'])) {
				mobilemsg(L('password_format_incorrect'), HTTP_REFERER);
			}
			if($this->memberinfo['password'] != password($_POST['info']['password'], $this->memberinfo['encrypt'])) {
				mobilemsg(L('old_password_incorrect'), HTTP_REFERER);
			}
			...
			if(shy_base::load_config('system', 'phpsso')) {
				...
				if ($res < 0) mobilemsg($message_error[$res]);
			}
			...
			include template('mobile', 'account_manage_password');
		}
	}
	//更换手机号码
	public function account_change_mobile() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($this->memberinfo['password'] != password($_POST['password'], $this->memberinfo['encrypt'])) {
				mobilemsg(L('old_password_incorrect'));
			}
			$sms_report_db = shy_base::load_model('sms_report_model');
			...
			if($mobile){
				...
				if($r && $r['id_code']==$mobile_verify) {
					...
					mobilemsg("手机号码更新成功！",'?app=member&controller=index&view=account_change_mobile&t=1');
				} else {
					mobilemsg("短信验证码错误！请重新获取！");
				}
			}else{
				mobilemsg("短信验证码已过期！请重新获取！");
			}
		} else {
	...
	public function public_get_password_type() {
		...
		include template('mobile', 'get_password_type');
	}
	...
	public function account_manage_upgrade() {
		...
		if(empty($grouplist[$memberinfo['groupid']]['allowupgrade'])) {
			mobilemsg(L('deny_upgrade'), HTTP_REFERER);
		}
		...
		if(isset($_POST['dosubmit'])) {
			$groupid = isset($_POST['groupid']) ? intval($_POST['groupid']) : mobilemsg(L('operation_failure'), HTTP_REFERER);
			
			$upgrade_type = isset($_POST['upgrade_type']) ? intval($_POST['upgrade_type']) : mobilemsg(L('operation_failure'), HTTP_REFERER);
			$upgrade_date = !empty($_POST['upgrade_date']) ? intval($_POST['upgrade_date']) : mobilemsg(L('operation_failure'), HTTP_REFERER);
	...
	public function login() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$username = isset($_POST['username']) && is_username($_POST['username']) ? trim($_POST['username']) : mobilemsg(L('username_empty'), HTTP_REFERER);
			$password = isset($_POST['password']) && trim($_POST['password']) ? trim($_POST['password']) : mobilemsg(L('password_empty'), HTTP_REFERER);
			...
			if(shy_base::load_config('system', 'phpsso')) {
				...
				if(isset($memberinfo['uid'])) {
					...
					if($status == -1) {	//用户不存在
						mobilemsg(L('user_not_exist'), 'index.php?app=waplogin&controller=index&view=login');
					} elseif($status == -2) { //密码错误
						mobilemsg(L('password_error'), 'index.php?app=waplogin&controller=index&view=login');
					} else {
						mobilemsg(L('login_failure'), 'index.php?app=waplogin&controller=index&view=login');
					}
				}
				...
				if($rtime['times'] > 4) {
					...
					mobilemsg(L('wait_1_hour', array('minute'=>$minute)));
				}
				
				//查询帐号
				$r = $this->db->get_one(array('username'=>$username));

				if(!$r) mobilemsg(L('user_not_exist'),'index.php?app=waplogin&controller=index&view=login');
				...
				if($r['password'] != $password) {				
					...
					mobilemsg(L('password_error', array('times'=>$times)), 'index.php?app=waplogin&controller=index&view=login', 3000);
				}
			...
			if($r['overduedate'] < SYS_TIME) {
				$updatearr = array('vip'=>'0', 'groupid'=>'6');
			}		
			...
			$shuyang_auth = sys_auth($userid."\t".$password, 'ENCODE', get_auth_key('login'));
			...
			param::set_cookie('_nickname', $nickname, $cookietime);
			//param::set_cookie('cookietime', $_cookietime, $cookietime);
			$forward = isset($_POST['forward']) && !empty($_POST['forward']) ? urldecode($_POST['forward']) : 'index.php?app=waplogin&controller=index';
			mobilemsg(L('login_success').$synloginstr, $forward);
		} else {
	...
	public function logout() {
		$setting = shy_base::load_config('system');
			...
			$forward = isset($_GET['forward']) && trim($_GET['forward']) ? $_GET['forward'] : 'index.php?app=waplogin&controller=index&view=login';
			mobilemsg(L('logout_success').$synlogoutstr, $forward);
	}
	...
	public function favorite() {
		...
		if(isset($_GET['id']) && trim($_GET['id'])) {
			...
			include template('mobile', 'favorite_list');
		}
	}
	...
	public function friend() {
		$memberinfo = $this->memberinfo;
		$this->friend_db = shy_base::load_model('friend_model');
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($from == 1) {
				if($memberinfo['point'] < $fromvalue) {
					mobilemsg(L('need_more_point'), HTTP_REFERER);
				}
			} elseif($from == 2) {
				if($memberinfo['amount'] < $fromvalue) {
					mobilemsg(L('need_more_amount'), HTTP_REFERER);
				}
			} else {
				mobilemsg(L('credit_setting_error'), HTTP_REFERER);
			}
			...
			if(!is_numeric($_POST['money']) || $_POST['money'] < 0) {
				mobilemsg(L('money_error'), HTTP_REFERER);
			} else {
				$money = intval($_POST['money']);
			}
			
			if($memberinfo['amount'] < $money) {
				mobilemsg(L('short_of_money'), HTTP_REFERER);
			}
			//此处比率读取用户配置
			...
			$credit_list = shy_base::load_config('credit');
	...
	public function mini() {
		...
		include template('mobile', 'mini_login');
	}
	...
	public function public_sina_login() {
		define('WB_AKEY', shy_base::load_config('system', 'sina_akey'));
		define('WB_SKEY', shy_base::load_config('system', 'sina_skey'));
		define('WEB_CALLBACK', APP_PATH.'index.php?app=waplogin&controller=index&view=public_sina_login&callback=1');
		shy_base::load_app_class('saetv2.ex', '' ,0);
		$this->_session_start();
		...
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
			...
			if(!empty($me['id'])) {
				...
				if(!empty($r)) {
					...
					param::set_cookie('cookietime', $_cookietime, $cookietime);
					param::set_cookie('_nickname', $nickname, $cookietime);
					$forward = isset($_GET['forward']) && !empty($_GET['forward']) ? $_GET['forward'] : 'index.php?app=waplogin&controller=index';
					mobilemsg(L('login_success').$synloginstr, $forward);
					...
					if(!$member_setting['allowregister']) {
						mobilemsg(L('deny_register'), 'index.php?app=waplogin&controller=index&view=login');
					}
	...
	public function public_qq_loginnew(){
                $appid = shy_base::load_config('system', 'qq_appid');
                $appkey = shy_base::load_config('system', 'qq_appkey');
                $callback = shy_base::load_config('system', 'wap_qq_callback');
                shy_base::load_app_class('qqapi','',0);
					...
					if(!empty($openid)){
						$r = $this->db->get_one(array('connectid'=>$openid,'from'=>'qq'));
						 ...
						 if(!empty($r)){
								...
								$username = $r['username'];
								$nickname = empty($r['nickname']) ? $username : $r['nickname'];
								$this->db->update(array('lastip'=>ip(), 'lastdate'=>SYS_TIME, 'nickname'=>$nickname, 'loginnum'=>'+=1'), array('userid'=>$userid));
								if(!$cookietime) $get_cookietime = param::get_cookie('cookietime');
								$_cookietime = $cookietime ? intval($cookietime) : ($get_cookietime ? $get_cookietime : 0);
								$cookietime = $_cookietime ? TIME + $_cookietime : 0;
								$shuyang_auth = sys_auth($userid."\t".$password, 'ENCODE', get_auth_key('login'));
								...
								$user = preg_replace('|[^a-zA-Z0-9\x{4e00}-\x{9fa5}]|u', '', $user);
								...
								if(CHARSET != 'utf-8') {//转编码
 								...
 								include template('mobile', 'connect');
						}
					}
                }
    }
	...
	* 该函数为登录回调地址
	*/
	public function public_xzh(){
                $appid = shy_base::load_config('system', 'xzh_appid');
                $appkey = shy_base::load_config('system', 'xzh_appkey');
                $callback = shy_base::load_config('system', 'wap_xzh_callback');
                shy_base::load_app_class('xzh','',0);
                $info = new xzh($appid,$appkey,$callback);
                $this->_session_start();
                if(!isset($_GET['code'])){
                         $info->redirect_to_login();
                }else{
					...
					$openid = $_SESSION['openid'] = $info->get_openid($code);
					if(!empty($openid)){
						$r = $this->db->get_one(array('connectid'=>$openid,'from'=>'baidu'));
						 if(!empty($r)){
								//百度帐号已存在于数据库，则直接转向登陆操作
								$password = $r['password'];
								$this->_init_phpsso();
								$synloginstr = $this->client->ps_member_synlogin($r['phpssouid']);
								$userid = $r['userid'];
	...
	* 该函数为登录回调地址
	*/
	public function public_wx(){
                $appid = shy_base::load_config('system', 'wx_appid');
                $appkey = shy_base::load_config('system', 'wx_appkey');
                $callback = shy_base::load_config('system', 'wx_callback');
                shy_base::load_app_class('wx','',0);
                $info = new wx($appid,$appkey,$callback);
                $this->_session_start();
                if(!isset($_GET['code'])){
                         $info->redirect_to_login();
                }else{
					...
					$openid = $_SESSION['openid'] = $info->get_openid($code);
					if(!empty($openid)){
						$r = $this->db->get_one(array('connectid'=>$openid,'from'=>'wx'));
						 if(!empty($r)){
								//百度帐号已存在于数据库，则直接转向登陆操作
								$password = $r['password'];
								$this->_init_phpsso();
	...
	public function public_forget_password () {
		...
		//SMTP MAIL 二种发送模式
 		if($email_config['mail_type'] == '1'){
			if(empty($email_config['mail_user']) || empty($email_config['mail_password'])) {
				mobilemsg(L('email_config_empty'), HTTP_REFERER);
			}
		}
		$this->_session_start();
		$member_setting = getcache('member_setting', 'member');
		if(isset($_POST['dosubmit'])) {
			if ($_SESSION['code'] != strtolower($_POST['code'])) {
				mobilemsg(L('code_error'), HTTP_REFERER);
			}
			...
			if(!empty($memberinfo['email'])) {
				...
				mobilemsg(L('email_error'), HTTP_REFERER);
			}
			...
			$url = APP_PATH."index.php?app=waplogin&controller=index&view=public_forget_password&code=$code";
			...
			if(isset($sitelist[$memberinfo['siteid']]['name'])) {
				...
				$sitename = 'SHUYANG_V9_MAIL';
			}
			sendmail($email, L('forgetpassword'), $message, '', '', $sitename);
			mobilemsg(L('operation_success'), 'index.php?app=waplogin&controller=index&view=login');
		} elseif($_GET['code']) {
			...
			if(is_array($code) && is_numeric($code[0]) && date('y-m-d h', SYS_TIME) == date('y-m-d h', $code[1])) {
				...
				if(empty($memberinfo['phpssouid'])) {
					mobilemsg(L('operation_failure'), 'index.php?app=waplogin&controller=index&view=login');
				}
				$updateinfo = array();
				...
				shy_base::load_sys_func('mail');
				sendmail($email, L('forgetpassword'), "尊敬的".$username."您好，接您申请，我们已经把你密码重置为:".$password, '', '', $sitename);
				mobilemsg(L('operation_success').L('newpassword').':'.$password);
			...
			include template('mobile', 'forget_password');
		}
	}
	
	/**
	*通过手机修改密码
	*方式：用户发送HHPWD afei985#821008 至 1065788 ，SHUYANG进行转发到网站运营者指定的回调地址，在回调地址程序进行密码修改等操作,处理成功时给用户发条短信确认。
	*shuyang 以POST方式传递相关数据到回调程序中
	...
	*/
	...
	public function public_forget_password_mobile () {
		...
		//处理提交申请，以手机号为准
			...
			if($r['mobile']=='') {
				...
				$_SESSION['userid'] = '';
				$_SESSION['code'] = '';
				mobilemsg("该账号没有绑定手机号码，请选择其他方式找回！");
			}
			...
			if($mobile){
				...
				if($r && $r['id_code']==$mobile_verify) {
					...
					if($status!==0) mobilemsg($status);
					$_SESSION['mobile'] = '';
					$_SESSION['userid'] = '';
					$_SESSION['code'] = '';
					mobilemsg("密码已重置成功！请查收手机",'index.php?app=waplogin&controller=index&view=login');
				} else {
					mobilemsg("短信验证码错误！请重新获取！");
				}
			}else{
			...
			$siteid = isset($_REQUEST['siteid']) && trim($_REQUEST['siteid']) ? intval($_REQUEST['siteid']) : 1;
			$siteinfo = siteinfo($siteid);
 			include template('mobile', 'forget_password_mobile');
		}
	}
	//通过用户名找回密码
	public function public_forget_password_username() {
		...
		//处理提交申请，以手机号为准
			...
			include template('mobile', 'forget_password_username');
		} elseif(isset($_POST['dosubmit']) && $step==3) {
			$sms_report_db = shy_base::load_model('sms_report_model');
			$mobile_verify = $_POST['mobile_verify'];
			...
			if($email){
				...
				if($_SESSION['emc_times']=='' || $_SESSION['emc_times']<=0){
					mobilemsg("验证次数超过5次,验证码失效，请重新获取邮箱验证码！",HTTP_REFERER,3000);
				}
				...
				if($_SESSION['emc']!='' && $_POST['email_verify']==$_SESSION['emc']) {
					...
					mobilemsg("验证码错误！请重新获取！",HTTP_REFERER,3000);
				}
			} else {
				mobilemsg("非法请求！");
			}
