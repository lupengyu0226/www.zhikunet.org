...
class index {
	...
	public function init() {
		...
		if(isset($_GET['q'])) {
			if(trim($_GET['q'])=='') {
				header('Location: '.APP_PATH.'index.php?app=search');exit;
			}
			...
			$badword = shy_base::load_model('badword_model');
			$q = $badword->replace_badword($q);
			//按时间搜索
			if($time == 'day') {
			...
			if($page == 1 && $_GET['q']==$q) {//翻页不重复记录
				$condition = array();
				$condition['search_word']  = $search_q;
				$condition['search_from'] = 'pc';
				$condition['ip'] = ip();
				$time = time();
			...
			if($setting['sphinxenable']) {
				$sphinx = shy_base::load_app_class('search_interface', '', 0);
			...
			$result_datas = $this->type_db->listinfo(array('typeid'=>$typeid,'module'=>'search'));
			foreach($result_datas as $r) {
                $list_template = $r['template'];
			}
			$template = $list_template ? $list_template : 'list';
			include	template('search',$template);
		} else {
