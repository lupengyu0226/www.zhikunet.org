...
class search_admin extends admin {
	...
	public function setting() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=search&controller=search_type&view=add\', title:\''.L('add_search_type').'\', width:\'580\', height:\'240\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_search_type'));
		...
		if(isset($_GET['dosubmit'])) {
			...
			foreach ($types as $_k=>$_v) {
				if($key==$_k) {
					...
					if($_v['modelid']) {
						if ($_v['typedir']!=='yp') {
							$search_api = shy_base::load_app_class('search_api','content');
						} else {
							$search_api = shy_base::load_app_class('search_api',$_v['typedir']);
						}
						if(!isset($_GET['total'])) {
						...
						$search_api = shy_base::load_app_class('search_api',$module);
						if(!isset($_GET['total'])) {
					...
					$page++;
					if($pages>=$page) showmessage("正在更新 <span style='color:#ff0000;font-size:14px;text-decoration:underline;' >{$_v['name']}</span> - 总数：{$total} - 当前第 <font color='red'>{$page}</font> 页","?app=search&controller=search_admin&view=createindex&menuid=909&page={$page}&total={$total}&key={$key}&pagesize={$pagesize}&have_truncate=1&dosubmit=1");
					$key++;
					showmessage("开始更新： <span style='color:#ff0000;font-size:14px;text-decoration:underline;' >{$_v['name']}</span> - 总数：{$total}条","?app=search&controller=search_admin&view=createindex&menuid=909&page=1&key={$key}&pagesize={$pagesize}&have_truncate=1&dosubmit=1");
