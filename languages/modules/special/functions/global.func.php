...
function content_url($contentid = 0, $page = 1, $addtime, $type = 'html', $site_info = '') {
	...
	switch ($type) {
		case 'html':
			if ($site_info['dirname']) {
				if ($page==1) {
					...
					$url[1] = shy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.date('Y', $addtime).'/'.date('md', $addtime).'/'.$contentid.'.html';
				} else {
					$url[0] = $site_info['domain'].'special/'.date('Y', $addtime).'/'.date('md', $addtime).'/'.$contentid.'-'.$page.'.html';
					$url[1] = shy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.date('Y', $addtime).'/'.date('md', $addtime).'/'.$contentid.'-'.$page.'.html';
				}
			} else {
				if ($page==1) {
					$url[0] = $url[1] = shy_base::load_config('system', 'html_root').'/special/'.date('Y', $addtime).'/'.date('md', $addtime).'/'.$contentid.'.html';
					$url[0] = $app_path.$url[0];
				} else {
					$url[0] = $url[1] = shy_base::load_config('system', 'html_root').'/special/'.date('Y', $addtime).'/'.date('md', $addtime).'/'.$contentid.'-'.$page.'.html';
					$url[0] = $app_path.$url[0];
				}
			}
			break;
		...
		case 'php':
			if ($page==1) {
				$url[0] = APP_PATH.'index.php?app=special&controller=index&view=show&id='.$contentid;
			} else {
				$url[0] = APP_PATH.'index.php?app=special&controller=index&view=show&id='.$contentid.'&page='.$page;
			}
			break;
	}
	return $url;
