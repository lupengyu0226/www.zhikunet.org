...
class html {
	...
	public function __construct() {
		...
		$this->site = shy_base::load_app_class('sites', 'admin');
	...
	public function _create_content($contentid = 0) {
		...
		if ($arr_content['paginationtype']) {			//文章使用分页时
			if($arr_content['paginationtype']==1) {
				...
				$contentpage = shy_base::load_app_class('contentpage', 'content'); //调用自动分页类
		...
		if ($CONTENT_POS !== false) {
			...
			for ($i=1; $i<=$pagenumber; $i++) {
				...
				$file = SHUYANG_PATH.$file; //生成文件的路径
	...
	public function _index($specialid = 0, $pagesize = 20, $pages_num = 0) {
		...
		if ($siteid>1) {
			...
			$file = shy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$filename.'/index.html';
		} else {
			$file = shy_base::load_config('system', 'html_root').'/special/'.$filename.'/index.html';
		}
		if(!$ispage) {
		...
		if($ispage) {
			...
			for ($i=1; $i<=$pages_num; $i++) {
				...
				$file_root = SHUYANG_PATH.$file_root;
				ob_start();
				include template('special', $template);
	...
	public function create_list($page = 1) {
        ...
        //URL规则
        $site_info = $this->site->get_by_id($siteid);
        $urlrules = array( "url" => $site_info['domain'].substr(shy_base::load_config('system', 'html_root'), 1).'/special/index.html|'.$site_info['domain'].substr(shy_base::load_config('system', 'html_root'), 1).'/special/index-{$page}.html');
        $urlrules = str_replace('|', '~',$urlrules["url"]);
        ...
        //分站时计算路径
        if ($siteid>1) {
                if ($page==1) $file = shy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/index.html';
                else $file = shy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/index-'.$page.'.html';
        } else {
                if ($page==1) $file = shy_base::load_config('system', 'html_root').'/special/index.html';
                else $file = shy_base::load_config('system', 'html_root').'/special/index-'.$page.'.html';
        }
        $this->queue->add_queue('add', $file, $siteid);
        $file  = SHUYANG_PATH.$file;
        ob_start();
	...
	public function create_type($typeid = 0, $page = 1) {
		...
		extract($s_info);
		$site_info = $this->site->get_by_id($siteid);
		define('URLRULE', $site_info['domain'].substr(shy_base::load_config('system', 'html_root'), 1).'/special/{$specialdir}/{$typedir}/lists-{$typeid}.html~'.$site_info['domain'].substr(shy_base::load_config('system', 'html_root'), 1).'/special/{$specialdir}/{$typedir}/lists-{$typeid}-{$page}.html');
		...
		if ($siteid>1) {
			if ($page==1) $file = shy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$filename.'/'.$info['typedir'].'/lists-'.$typeid.'.html';
			else $file = shy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$filename.'/'.$info['typedir'].'/lists-'.$typeid.'-'.$page.'.html';
		} else {
			if ($page==1) $file = shy_base::load_config('system', 'html_root').'/special/'.$filename.'/'.$info['typedir'].'/lists-'.$typeid.'.html';
			else $file = shy_base::load_config('system', 'html_root').'/special/'.$filename.'/'.$info['typedir'].'/lists-'.$typeid.'-'.$page.'.html';
		}
		$this->queue->add_queue('add', $file, $siteid);
		$file = SHUYANG_PATH.$file;
		ob_start();
	 ...
	 * @param intval $pages 最大更新页数
 	 */
	public function _list($typeid = 0, $pagesize = 10, $pages = 0) {
		if (!$typeid) return false;
