...
class special_api {
	...
	public function __construct() {
		$this->db = shy_base::load_model('special_model'); //专题数据模型
		$this->type_db = shy_base::load_model('type_model'); //专题分类数据模型
		$this->c_db = shy_base::load_model('special_content_model'); //专题内容数据模型
		$this->data_db = shy_base::load_model('special_c_data_model'); 
	}
			...
			if ($a == 'add' && !$v['del']) {
				...
				if ($siteid>1) {
					...
					if ($special_info['ishtml']) {
						...
						$url = $site_info['domain'].'index.php?app=special&controller=index&view=type&specialid='.$specialid.'&typeid='.$typeid;
					}
				} else {
					if($special_info['ishtml']) $url = addslashes($app_path.shy_base::load_config('system', 'html_root').'/special/'.$special_info['filename'].'/'.$v['typedir'].'/'.'type-'.$typeid.'.html');
					else $url = APP_PATH.'index.php?app=special&controller=index&view=type&specialid='.$specialid.'&typeid='.$typeid;
				}
				...
				if ((!isset($v['del']) || empty($v['del'])) && $v['typeid']) {
					...
					if ($siteid>1) {
						...
						if ($special_info['ishtml']) {
							...
							$url = $site_info['domain'].'index.php?app=special&controller=index&view=type&specialid='.$specialid.'&typeid='.$v['typeid'];
						}
					} else {
						if($special_info['ishtml']) $url = addslashes($app_path.shy_base::load_config('system', 'html_root').'/special/'.$special_info['filename'].'/'.$v['typedir'].'/'.'type-'.$v['typeid'].'.html');
						else $url = APP_PATH.'index.php?app=special&controller=index&view=type&specialid='.$specialid.'&typeid='.$v['typeid'];
					}
					$v['url'] = $url;
					$typeid = $v['typeid'];
					unset($v['typeid']);
	...
	public function _get_import_data($modelid = 0, $where = '', $page) {
		$c = shy_base::load_model('content_model');
		if(!$modelid) return '';
		$c->set_model($modelid);
	...
	public function _del_special($id = 0) {
		...
		if ($info['siteid']>1) {
			if ($info['ishtml']) {
				$queue = shy_base::load_model('queue_model');
				$site = shy_base::load_app_class('sites', 'admin');
				$site_info = $site->get_by_id($info['siteid']);
				$file = shy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$info['filename'].'/index.html';
				if ($info['ispage']) {
					for ($i==1; $i>0; $i++) {
						if ($i>1) {
							$file = str_replace('.html', '-'.$i.'.html', $file);
						}
						if (!file_exists(SHUYANG_PATH.$file)) {
							break;
						} else {
							$queue->add_queue('del', $file, $info['siteid']);
							unlink(SHUYANG_PATH.$file);
						}
					}
				} else {
					$queue->add_queue('del', $file, $info['siteid']);
				...
				$queue->add_queue('del', shy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$info['filename'].'/', $info['siteid']);
				dir_delete(shy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/special/'.$info['filename']);
			}
		} else {
			if ($info['ishtml']) {
				dir_delete(SHUYANG_PATH.shy_base::load_config('system', 'html_root').DIRECTORY_SEPARATOR.'special'.DIRECTORY_SEPARATOR.$info['filename']); //删除专题目录
			}
		}
	...
	private function delete_type($typeid = 0, $siteid = 0, $ishtml = 0) {
		...
		if ($ishtml) {
			...
			if ($siteid>1) {
				...
				$queue = shy_base::load_model('queue_model');
				for ($i = 1; $i>0; $i++) {
					if ($i==1) $file = str_replace($site_info['domain'], shy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/', $info['url']);
					else $file = str_replace(array($site_info['domain'], '.html'), array(shy_base::load_config('system', 'html_root').'/'.$site_info['dirname'].'/', '-'.$i.'.html'), $info['url']);
					if (!file_exists(SHUYANG_PATH.$file)) {
						break;
					} else {
						$queue->add_queue('del', $file, $siteid); //并加入到消息队列中，便以其他站点删除文件
						unlink(SHUYANG_PATH.$file);	//删除生成的静态文件
					}
				}
			} else {
				for ($i = 1; $i>0; $i++) {
	...
	public function _delete_content($cid = 0, $siteid = 0, $ishtml = 0) {
		...
		if ($info['isdata']) {
			if ($ishtml) {	
				shy_base::load_app_func('global', 'special');
				$siteid = $siteid ? intval($siteid) : get_siteid();
				if ($siteid>1) {
					$site = shy_base::load_app_class('sites', 'admin');
					$site_info = $site->get_by_id($siteid);
					...
					for ($i = 1; $i>0; $i++) {
						...
						if (!file_exists(SHUYANG_PATH.$file[1])) {
							break;
						} else {
							$queue->add_queue('del', $file[1], $siteid); //并加入到消息队列中，便以其他站点删除文件
							unlink(SHUYANG_PATH.$file[1]);	//删除生成的静态文件
						}
					}
				} else {
					for ($i = 1; $i>0; $i++) {
			...
			// 删除数据统计表数据
			$count = shy_base::load_model('hits_model');
	...
	public function importfalbum($info = array()) {
		...
		if (is_array($info)) {
			...
			$url = $sitelists[$siteid]['domain'].'index.php?app=special&controller=index&id='.$specialid;
			...
			$url = $sitelists[$siteid]['domain'].'index.php?app=special&controller=index&view=type&specialid='.$specialid.'&typeid='.$typeid;
			$this->type_db->update(array('url'=>$url), array('typeid'=>$typeid));
			return $specialid;
		} else {
			return false;
