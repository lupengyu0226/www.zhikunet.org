...
class special extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = shy_base::load_model('special_model');
		$this->special_api = shy_base::load_app_class('special_api', 'special');
	}
			...
			if ($id) {
				...
				if ($special['siteid']>1) {
					...
					if ($special['ishtml']) {
						...
						$url = $site_info['domain'].'index.php?app=special&controller=index&id='.$id;
					}
				} else {
					$url = $special['ishtml'] ? APP_PATH.substr(shy_base::load_config('system', 'html_root'), 1).'special/'.$special['filename'].'/' : APP_PATH.'index.php?app=special&controller=index&id='.$id;
				}
				$this->db->update(array('url'=>$url), array('id'=>$id, 'siteid'=>$this->get_siteid()));
				
				//调用生成静态类
				if ($special['ishtml']) {
					$html = shy_base::load_app_class('html', 'special'); 
					$html->_index($id, 20, 5);
				}
				//更新附件状态
		...
		if (isset($_POST['dosubmit']) && !empty($_POST['dosubmit'])) {
			...
			if ($special['ishtml'] && $special['filename']) {
				...
				if ($siteid>1) {
					...
					$special['url'] = APP_PATH.substr(shy_base::load_config('system', 'html_root'), 1).'special/'.$special['filename'].'/';
				}
			} elseif ($special['ishtml']=='0') {
				if ($siteid>1) {
					$special['url'] = $site_info['domain'].'index.php?app=special&controller=index&specialid='.$_GET['specialid'];
				} else {
					$special['url'] = APP_PATH.'index.php?app=special&controller=index&specialid='.$_GET['specialid'];
				}
			}
	...
	public	function create_special_list() {
		$siteid = get_siteid();
        $html = shy_base::load_app_class('html');
        $page = $page ? $page : 1;
        $pagesize = $pagesize ? $pagesize : 10;
        $j = 1;
        do {
                $html->create_list($page);
                $page++;
                $j++;
        ...
        if($page <= $total_number) {
                ...
                $message = L('updating').L('start_to_end_id',array('page'=>$page,'endpage'=>$endpage));
                $forward = "index.php?app=special&controller=special&view=create_special_list&pagesize=$pagesize&page=$page&total_number=$total_number";
        } else {
                $message = L('index_create_finish');
                $forward = "index.php?app=special&controller=special&view=init";
        }
	...
	public function public_create_html() {
		...
		if (is_array($specials) && !empty($specials)) {
			...
			showmessage(L('update_special_success'), '?app=special&controller=special&view=init');
		}
	}
	...
	private function create_index($specialid) {
		...
		if (!$info['ishtml']) {
			showmessage($info['title'].L('update_success'), '?app=special&controller=special&view=public_create_html');
		}
		$html = shy_base::load_app_class('html');
		$html->_index($specialid);
		showmessage($info['title'].L('index_update_success'), '?app=special&controller=special&view=public_create_type&specialid='.$specialid);
	}
	...
	public function public_create_type() {
		...
		if (is_array($types) && !empty($types) || $pages) {
			...
			if (!$pages) {
				$c = shy_base::load_model('special_content_model');
			...
			if (empty($types) && $pages==$maxpage) {
				...
				showmessage($typename.L('type_update_success'), '?app=special&controller=special&view=public_create_content&specialid='.$specialid);
			}
			if ($pages<=$maxpage) {
				showmessage($typename.L('update_success'), '?app=special&controller=special&view=public_create_type&specialid='.$specialid);
			} else {
				showmessage($typename.L('type_from').($_GET['page'] ? $_GET['page'] : 1).L('type_end').$maxpage.'</font> '.L('update_success'), '?app=special&controller=special&view=public_create_type&typeid='.$typeid.'&typename='.$typename.'&page='.$page.'&pages='.$pages.'&specialid='.$specialid);
			}
			
		} else {
			$special_api = shy_base::load_app_class('special_api');
			$types = $special_api->_get_types($specialid);
			setcache('create_types', $types, 'commons');
			showmessage(L('start_update_type'), '?app=special&controller=special&view=public_create_type&specialid='.$specialid);
		}
	}
	...
	public function public_create_content() {
		...
		if ($page>=$pages) {
			showmessage(L('content_update_success'), '?app=special&controller=special&view=public_create_html&specialid='.$specialid);
		} else {
			$page++;
			showmessage(L('content_from').' <font color="red">'.intval($offset+1).L('type_end').intval($offset+10).'</font> '.L('update_success'), '?app=special&controller=special&view=public_create_content&specialid='.$specialid.'&page='.$page.'&pages='.$pages);
		}
	}
	...
	private function check($data, $a = 'add') {
		...
		if ($data['voteid']) {
			if (strpos($data['voteid'], '|')===false) {
				$vote_db = shy_base::load_model('vote_subject_model');
