...
class content extends admin {
	...
	public function __construct() {
		parent::__construct();
		$this->db = shy_base::load_model('special_content_model');
	...
	public function init() {
		...
		$datas = $this->db->listinfo(array('specialid'=>$_GET['specialid']), '`inputtime` DESC , `id` DESC', $page);
		$pages = $this->db->pages;
		$big_menu = array(array('javascript:openwinx(\'?app=special&controller=content&view=add&specialid='.$_GET['specialid'].'\',\'\');void(0);', L('add_content')), array('javascript:window.top.art.dialog({id:\'import\',iframe:\'?app=special&controller=special&view=import&specialid='.$_GET['specialid'].'\', title:\''.L('import_content').'\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'import\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'import\'}).close()});void(0);', L('import_content')));
	...
	public function delete() {
		...
		$special = shy_base::load_model('special_model');
		$info = $special->get_one(array('id'=>$specialid));
		$special_api = shy_base::load_app_class('special_api', 'special');
