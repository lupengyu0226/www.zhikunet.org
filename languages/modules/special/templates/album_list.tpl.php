...
<input type="hidden" value="album" name="controller">
<input type="hidden" value="import" name="view">
<input type="hidden" value="1" name="search">
<input type="hidden" value="<?php echo $_GET['menuid']?>" name="menuid">
<input type="hidden" value="<?php echo $_GET['safe_edi'];?>" name="safe_edi">
...
<div class="pad-lr-10">
<form name="myform" action="?app=special&controller=album&view=import&page=<?php echo $_GET['page'];?>" method="post">
...
?>   
	...
	<td align="center"><span style="height:22"><?php if (!empty($imported) && in_array($info['id'], $imported)){?><font color="red">专辑已载入<?php } else {?><a href='?app=special&controller=album&view=import&id=<?php echo $info['id']?>&page=<?php echo $_GET['page'];?>&safe_edi=<?php echo $_GET['safe_edi'];?>'>载入此专辑</a><?php }?></span><br /><span style="height:22"><a href="?app=special&controller=album&view=content_list&id=<?php echo $info['id']?>&menuid=<?php echo $_GET['menuid']?>&safe_edi=<?php echo $_GET['safe_edi'];?>">查看专辑下的视频</a></span></td>
	</tr>
<?php 
	}
}
