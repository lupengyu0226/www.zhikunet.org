...
<form name="myform" id="myform" action="?app=special&controller=special&view=import&specialid=<?php echo $_GET['specialid']?>&modelid=<?php echo $_GET['modelid']?>" method="post">
    <table width="100%">
        <thead>
            <tr>
...
<script type="text/javascript">
	function select_categorys(modelid, id) {
		if(modelid) {
			$.get('', {app: 'special', controller: 'special', view: 'public_categorys_list', modelid: modelid, catid: id, safe_edi: safe_edi }, function(data){
				if(data) {
					$('#catids').html(data);
				} else $('#catids').html('');
			});
