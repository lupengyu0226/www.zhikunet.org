...
<form name="myform" id="myform" action="?app=special&controller=content&view=edit&specialid=<?php echo $_GET['specialid']?>&id=<?php echo $_GET['id']?>" method="post" enctype="multipart/form-data">
...
<div class="col-right">
	...
	<h6> <?php echo L('show_template')?></h6> <span id="show_template"><?php echo '<script type="text/javascript">$.getJSON(\'?app=admin&controller=category&view=public_tpl_file_list&style='.$style.'&id='.$data['show_template'].'&module=special&templates=show&name=data\', function(data){$(\'#show_template\').html(data.show_template);});</script>'?></span> 
          </div>
        </div>
    </div>
    <div class="col-auto">
...
<table width="100%" cellspacing="0" class="table_form">
	...
	<tr>
      ...
      <td><input type="text" style="width:350px;" name="info[title]" id="title" value="<?php echo new_html_special_chars($info['title'])?>" class="measure-input " onBlur="$.post('index.php?app=api&controller=index&op=get_keywords&number=3&sid='+Math.random()*5, {data:$('#title').val()}, function(data){if(data && $('#keywords').val()=='') $('#keywords').val(data); })" />
