...
?>
<form method="post" action="?app=special&controller=special&view=edit&specialid=<?php echo $_GET['specialid']?>" id="myform">
<div class="pad-10">
<div class="col-tab">
...
function load_file_list(id) {
	$.getJSON('?app=admin&controller=category&view=public_tpl_file_list&style='+id+'&module=special&templates=index|list|show&name=special', function(data){$('#index_template').html(data.index_template);$('#list_template').html(data.list_template);$('#show_template').html(data.show_template);});
}
...
$(document).ready(function(){
	...
	$('#title').formValidator({autotip:true,onshow:"<?php echo L('please_input_special_title')?>",onfocus:"<?php echo L('min_3_title')?>",oncorrect:"<?php echo L('true')?>"}).inputValidator({min:1,onerror:"<?php echo L('please_input_special_title')?>"}).ajaxValidator({type:"get",url:"",data:"app=special&controller=special&view=public_check_special&id=<?php echo $_GET['specialid']?>",datatype:"html",cached:false,async:'true',success : function(data) {
        if( data == "1" )
		{
            return true;
		}
