...
<form name="myform" action="?app=special&controller=content&view=listorder&specialid=<?php echo $_GET['specialid']?>" method="post">
    <table width="100%">
        <thead>
            <tr>
...
<tbody>
    ...
    ?>
		...
		<td align='center' width="200"><a href="javascript:;" onclick="javascript:openwinx('?app=special&controller=content&view=edit&specialid=<?php echo $r['specialid']?>&id=<?php echo $r['id']?>','')"><?php echo L('content_edit')?></a> </td>
	</tr>
     <?php }?>
</tbody>
     </table>
    <div class="btn"><label for="check_box"><?php echo L('selected_all')?>/<?php echo L('cancel')?></label> <input type="submit" class="button" value="<?php echo L('listorder')?>" /> <input type="submit" class="button" value="<?php echo L('delete')?>" onclick="if(confirm('<?php echo L('confirm', array('message' => L('selected')))?>')){document.myform.action='?app=special&controller=content&view=delete&specialid=<?php echo $_GET['specialid']?>'}else{return false;}"/></div>
