...
class dbsource_admin extends admin {
	...
	public function __construct() {
		$this->db = shy_base::load_model('dbsource_model');
		parent::__construct();
	...
	public function init() {
		...
		$pages = $this->db->pages;
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=dbsource&controller=dbsource_admin&view=add\', title:\''.L('added_external_data_source').'\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('added_external_data_source'));
	...
	public function del() {
		...
		if ($this->db->get_one(array('id'=>$id))) {
			if ($this->db->delete(array('id'=>$id))) {
				dbsource_cache();
				showmessage(L('operation_success'), '?app=dbsource&controller=dbsource_admin&view=init');
			} else {
				showmessage(L('operation_failure'),  '?app=dbsource&controller=dbsource_admin&view=init');
			}
		} else {
			showmessage(L('notfound'),   '?app=dbsource&controller=dbsource_admin&view=init');
		}
	}
