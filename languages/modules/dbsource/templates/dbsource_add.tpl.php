...
<div class="pad-10">
<form action="?app=dbsource&controller=dbsource_admin&view=add" method="post" id="myform">
<div>
<fieldset>
...
<!--
	function test_connect() {
		$.get('?app=dbsource&controller=dbsource_admin&view=public_test_mysql_connect', {host:$('#host').val(),username:$('#username').val(), password:$('#password').val(), port:$('#port').val()}, function(data){if(data==1){alert('<?php echo L('connect_success')?>')}else{alert('<?php echo L('connect_failed')?>')}});
}
//-->
</script>
</body>
