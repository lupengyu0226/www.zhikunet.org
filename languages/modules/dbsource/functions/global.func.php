 ...
 * @param $module 模型名
 */
function shy_tag_class ($module) {
	$filepath = SHY_PATH.'modules'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$module.'_tag.class.php';
	if (file_exists($filepath)) {
		$shy_tag = shy_base::load_app_class($module.'_tag', $module); 
		if (!method_exists($shy_tag, 'shy_tag')) {
			showmessage(L('the_module_will_not_support_the_operation'));
		}
		$html  = $shy_tag->shy_tag();
	} else {
...
function template_url($id) {
	...
	if (!file_exists($filepath)) {
		$datacall = shy_base::load_model('datacall_model');
