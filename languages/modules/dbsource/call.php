		...
		if ($data = $this->db->get_one(array('id'=>$id))) {
			if (!$str = tpl_cache('dbsource_'.$id,$data['cache'])) {
				if ($data['type'] == 1) { //自定义SQL调用
					$get_db = shy_base::load_model("get_model");
					...
					$filepath = SHY_PATH.'modules'.DIRECTORY_SEPARATOR.$data['module'].DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$data['module'].'_tag.class.php';
					if (file_exists($filepath)) {
						$shy_tag = shy_base::load_app_class($data['module'].'_tag', $data['module']); 
						if (!method_exists($shy_tag, $data['action'])) {
							exit();
						}
