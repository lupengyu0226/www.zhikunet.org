...
<div class="pad_10">
<form action="?app=vote&controller=vote&view=edit&subjectid=<?php echo $subjectid?>" method="post" name="myform" id="myform">
...
<tr> 
	...
	<tr>
		...
		<script type="text/javascript">$.getJSON('?app=admin&controller=category&view=public_tpl_file_list&style=<?php echo $info['default_style']?>&module=vote&templates=vote_tp&id=<?php echo $template?>&name=vote_subject&safe_edi='+safe_edi, function(data){$('#show_template').html(data.vote_tp_template);});</script>
		</td>
	</tr>
...
function del_old(id) {
	$.get('?app=vote&controller=vote&view=del_option&optionid='+id+'&safe_edi='+safe_edi,null,function (msg) { 
	if (msg==1) { 
...
function load_file_list(id) {
	$.getJSON('?app=admin&controller=category&view=public_tpl_file_list&style='+id+'&module=vote&templates=vote_tp&name=subject&safe_edi='+safe_edi, function(data){$('#show_template').html(data.vote_tp_template);});
}
