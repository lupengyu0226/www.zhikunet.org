...
<input name="jscode1" id="jscode1" value='<script language="javascript" src="<?php echo APP_PATH;?>index.php?app=vote&controller=index&view=show&action=js&subjectid=<?php echo $r['subjectid']?>&type=3"></script>' style="width:410px"> <input type="button" onclick="$('#jscode1').select();document.execCommand('Copy');" value="<?php echo L('copy_code')?>" class="button" style="width:114px">
</fieldset>
<div class="bk10"></div>
<fieldset>
	<legend><?php echo L('vote_call_2')?></legend>
    <?php echo L('vote_phpcall')?><br />
<input name="jscode2" id="jscode2" value='<script language="javascript" src="<?php echo APP_PATH;?>index.php?app=vote&controller=index&view=show&action=js&subjectid=<?php echo $r['subjectid']?>&type=2"></script>' style="width:410px">
