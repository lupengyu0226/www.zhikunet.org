...
class vote extends admin {
	...
	function __construct() {
		...
		$this->db = shy_base::load_model('vote_subject_model');
		$this->db2 = shy_base::load_model('vote_option_model');
	}
	...
	public function init() {
		...
		$pages = $this->db->pages;
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=vote&controller=vote&view=add\', title:\''.L('vote_add').'\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('vote_add'));
	...
	public function public_name() {
		$subject_title = isset($_GET['subject_title']) && trim($_GET['subject_title']) ? (shy_base::load_config('system', 'charset') == 'gbk' ? iconv('utf-8', 'gbk', trim($_GET['subject_title'])) : trim($_GET['subject_title'])) : exit('0');
	...
	public function add() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(empty($_POST['subject']['subject'])) {
				showmessage(L('vote_title_noempty'),'?app=vote&controller=vote&view=add');
			}
 			//记录选项条数 optionnumber 
			...
			if(isset($_POST['from_api'])&& $_POST['from_api']) {
				showmessage(L('operation_success'),'?app=vote&controller=vote&view=add','100', '',"window.top.$('#voteid').val('".$subjectid."');window.top.art.dialog({id:'addvote'}).close();");
			} else {
				showmessage(L('operation_success'),'?app=vote&controller=vote','','add');
 			}
		} else {
	...
	public function edit() {
		...
		if(isset($_POST['dosubmit'])){
			//验证数据正确性
				...
				//模版 
				$_POST['subject']['template'] = $_POST['vote_subject']['vote_tp_template'];
				$_POST['subject']['optionnumber'] = ( ( is_array( $_POST['option'] ) && !empty( $_POST['option'] ) ) ? count( $_POST['option'] ) : 0 ) + ( ( is_array( $_POST['newoption'] ) && !empty( $_POST['newoption'] ) ) ? count( $_POST['newoption'] ) : 0);
	 			$this->db->update($post_data['subject'],array('subjectid'=>$subjectid));//更新投票选项总数
				$this->update_votejs($subjectid);//生成JS文件
				showmessage(L('operation_success'),'?app=vote&controller=vote&view=edit','', 'edit');
			}else{
				$show_validator = $show_scroll = $show_header = true;
				shy_base::load_sys_class('form', '', 0);
	...
	public function delete() {
		if((!isset($_GET['subjectid']) || empty($_GET['subjectid'])) && (!isset($_POST['subjectid']) || empty($_POST['subjectid']))) {
			...
			if(is_array($_POST['subjectid'])){
				...
				showmessage(L('operation_success'),'?app=vote&controller=vote');
			}else{
				$subjectid = intval($_GET['subjectid']);
				if($subjectid < 1) return false;
				//删除对应投票的选项
				...
				if($result)
				{
					...
					showmessage(L("operation_failure"),'?app=vote&controller=vote');
				}
			}
	...
	public function setting() {
		...
		$now_seting = $setting[$siteid]; 
 		if(isset($_POST['dosubmit'])) {
			...
			showmessage(L('setting_updates_successful'), '?app=vote&controller=vote&view=init');
		} else {
			@extract($now_seting);
			shy_base::load_sys_class('form', '', 0);
			//模版
	...
	public function statistics() {
			...
			$show_validator = $show_scroll = $show_header = true;
 			//获取投票信息
			$sdb = shy_base::load_model('vote_data_model'); //加载投票统计的数据模型
	...
	public function statistics_userlist() {
			...
			$sdb = shy_base::load_model('vote_data_model'); //调用统计的数据模型
 			$page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
