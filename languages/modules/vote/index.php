...
class index {
	...
	function __construct() {
		...
		$this->vote = shy_base::load_model('vote_subject_model');//投票标题
		$this->vote_option = shy_base::load_model('vote_option_model');//投票选项
		$this->vote_data = shy_base::load_model('vote_data_model'); //投票统计的数据模型
	...
	public function post(){
		...
		$return = $this->check($subjectid);
 		switch ($return) {
		case 0:
		  showmessage(L('vote_voteyes'),"?app=vote&controller=index&view=result&subjectid=$subjectid&siteid=$siteid");
		  break;
		case -1:
		  showmessage(L('vote_voteyes'),"?app=vote&controller=index&view=result&subjectid=$subjectid&siteid=$siteid");
		  break;
		...
		showmessage(L('vote_votesucceed'), "?app=vote&controller=index&view=result&subjectid=$subjectid&siteid=$siteid");
	}
	...
	public function check($subjectid){
		...
		if($subject_arr['enabled']==0){
			showmessage(L('vote_votelocked'),"?app=vote&controller=index&view=result&subjectid=$subjectid&siteid=$siteid");
 		}
 		if(date("Y-m-d",SYS_TIME)>$subject_arr['todate']){
			showmessage(L('vote_votepassed'),"?app=vote&controller=index&view=result&subjectid=$subjectid&siteid=$siteid");
 		}
 		//游客是否可以投票
		if($subject_arr['allowguest']==0 ){
			if(!$this->username){
				showmessage(L('vote_votenoguest'),"?app=vote&controller=index&view=result&subjectid=$subjectid&siteid=$siteid");
 			}elseif($this->groupid == '7'){
				showmessage('对不起，不允许邮件待验证用户投票！',"?app=vote&controller=index&view=result&subjectid=$subjectid&siteid=$siteid");
			}
 		}
