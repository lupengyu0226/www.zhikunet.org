 ...
 * @author chenzhouyu
 * 
 * 直接使用shy_base::load_app_class('spend', 'pay', 0);
 * 进行加载。
 * 使用spend::amount()进行金钱的消费
 * 当函数返回的结果为false时，可使用spend::get_msg()获取错误原因
 * 
 */
class spend {
	...
	protected  static function connect() {
		self::$db = shy_base::load_model("pay_spend_model");
	}
	 ...
	 */
	public static function spend_time($userid, $logo) {
		if (empty(self::$db)) {
			self::connect();
		}
