...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    $modules[$i]['name']    = L('wxpay', '', 'pay');
    $modules[$i]['desc']    = L('wxpay_tip', '', 'pay');
    $modules[$i]['is_cod']  = '0';
    $modules[$i]['is_online']  = '1';
    $modules[$i]['author']  = '沭阳网';
...
shy_base::load_app_class('pay_abstract','','0');
shy_base::load_sys_class('payhttp','',0);

class Wxpay extends paymentabstract{
    protected $gateway_type = 'qrcode';
    public function __construct($config = array()) {
        if (!empty($config)) $this->set_config($config);
        $this->config['gateway_url'] = 'https://api.mch.weixin.qq.com/pay/unifiedorder';
        $this->config['gateway_method'] = 'POST';
        $this->config['notify_url'] = return_url('wxpay', 'notify');
        $this->config['return_url'] = return_url('wxpay', 'return');
    }
    ...
    public function _return(){
        ...
        payhttp::postRequest($this->config['gateway_url'], $this->_array2xml($return),'xml');
    }
    ...
    public function getpreparedata(){
        ...
        $prepare_data['mch_id'] = $this->config['mchid'];
        $prepare_data['nonce_str'] = getNonceStr(32);
        ...
        // 商品信息
        $prepare_data['spbill_create_ip'] = ip();
    ...
    public function response($result){
        if (FALSE == $result)
        showmessage(L('pay_failed'),PASSPORT_PATH.'index.php?app=pay&controller=deposit');
        else
        showmessage(L('pay_success'),PASSPORT_PATH.'index.php?app=pay&controller=deposit');
    }
    public function gateway_url() {
        $result = payhttp::postRequest($this->config['gateway_url'], $this->_array2xml($this->getpreparedata()),'xml');
