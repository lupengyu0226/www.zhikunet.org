...
<input type="hidden" value="pay_list" name="view">
...
?>   
	...
	<font class="xbtn btn-inverse btn-xs"><?php echo L('change_price')?></font>   <font class="xbtn btn-white btn-xs"><?php echo L('closed')?>  </font> <a class="xbtn btn-danger btn-xs" href="javascript:confirmurl('?app=pay&controller=payment&view=pay_del&id=<?php echo $info['id']?>&menuid=<?php echo $_GET['menuid']?>', '<?php echo L('trade_record_del')?>')"><?php echo L('delete')?></a>
	<?php } elseif($info['status'] =='waitting' ) {?>
	<a class="xbtn btn-primary btn-xs" href="javascript:confirmurl('?app=pay&controller=payment&view=public_check&id=<?php echo $info['id']?>', '<?php echo L('check_confirm',array('sn'=>$info['trade_sn']))?>')"><?php echo L('check')?></a>   <a class="xbtn btn-primary btn-xs" href="?app=pay&controller=payment&view=pay_cancel&id=<?php echo $info['id']?>"><?php echo L('closed')?></a>   <a  class="xbtn btn-primary btn-xs" href="javascript:confirmurl('?app=pay&controller=payment&view=pay_del&id=<?php echo $info['id']?>&menuid=<?php echo $_GET['menuid']?>', '<?php echo L('trade_record_del')?>')"><?php echo L('delete')?></a>
	<?php } else {?>
	<a class="xbtn btn-primary btn-xs" href="javascript:void(0);" onclick="discount('<?php echo $info['id']?>', '<?php echo $info['trade_sn']?>')"><?php echo L('change_price')?></a>   <a class="xbtn btn-warning btn-xs" href="?app=pay&controller=payment&view=pay_cancel&id=<?php echo $info['id']?>"><?php echo L('closed')?></a>    <a class="xbtn btn-danger btn-xs" href="javascript:confirmurl('?app=pay&controller=payment&view=pay_del&id=<?php echo $info['id']?>&menuid=<?php echo $_GET['menuid']?>', '<?php echo L('trade_record_del')?>')"><?php echo L('delete')?></a>
...
<!--
	...
	window.top.art.dialog({title:'<?php echo L('discount')?>--'+name, id:'discount', iframe:'?app=pay&controller=payment&view=discount&id='+id ,width:'500px',height:'200px'}, 	function(){var d = window.top.art.dialog({id:'discount'}).data.iframe;
	var form = d.document.getElementById('dosubmit');form.click();return false;}, function(){window.top.art.dialog({id:'discount'}).close()});
}
function detail(id, name) {
	window.top.art.dialog({title:'<?php echo L('discount')?>--'+name, id:'discount', iframe:'?app=pay&controller=payment&view=public_pay_detail&id='+id ,width:'500px',height:'550px'});
}
//-->
</script>
