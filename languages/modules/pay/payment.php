...
class payment extends admin {
	...
	function __construct() {
		...
		parent::__construct();
		$this->db = shy_base::load_model('pay_payment_model');
		$this->account_db = shy_base::load_model('pay_account_model');
		$this->member_db = shy_base::load_model('member_model');
		$this->modules_path = SHY_PATH.'modules'.DIRECTORY_SEPARATOR.'pay';
		shy_base::load_app_class('pay_method','','0');
	...
	public function delete() {
		...
		showmessage(L('delete_succ'),'?app=pay&controller=payment');
	}
	...
	public function modify_deposit() {
		if(isset($_POST['dosubmit'])) {
			...
			if($userinfo) {
				//如果增加金钱，向pay_account 中记录数据
				if($_POST['pay_unit']) {
					$value = floatval($_POST['unit']);
					$payment = L('admin_recharge');
					$receipts = shy_base::load_app_class('receipts');
					$receipts->amount($value, $userinfo['userid'] , $username, create_sn(), 'offline', $payment, param::get_cookie('admin_username'), $status = 'succ',$usernote);

				} else {
					$value = floatval($_POST['unit']);
					$msg = L('background_operation').$usernote;
					$spend = shy_base::load_app_class('spend');
					$spend->amount($value,$msg,$userinfo['userid'],$username,param::get_cookie('userid'),param::get_cookie('admin_username'));
				}
	...
	public function pay_del() {
		...
		showmessage(L('delete_succ'),'?app=pay&controller=payment&view=pay_list&menuid='.$_GET['menuid']);
	}
	...
	public function public_check() {
		...
		showmessage(L('check_passed'),'?app=pay&controller=payment&view=pay_list');
	}
	...
	public function public_checkname_ajax() {
		...
		if ($r = $this->member_db->get_one(array('username'=>$username))){
			exit(L('user_balance').$r['amount'].' 元');
		} else {
			exit('FALSE');
		}
	}
