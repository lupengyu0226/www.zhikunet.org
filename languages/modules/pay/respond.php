...
class respond {
	...
	public function dnotify() {
		...
		$payment = $this->get_by_code(ucwords(_PAYMENT_));
		if(!$payment) error_log(date('m-d H:i:s',SYS_TIME).'| POST: payment is null |'."\r\n", 3, CACHE_PATH.'pay_error_log.php');
		$cfg = unserialize_config($payment['config']);
		$pay_name = ucwords($payment['pay_code']);
		shy_base::load_app_class('pay_factory','',0);
		$payment_handler = new pay_factory($pay_name, $cfg);
		$return_data = $payment_handler->_notify();
		if($return_data) {
			$this->update_member_amount_by_sn($return_data['out_trade_no']);
			$this->update_recode_status_by_sn($return_data['out_trade_no'],$return_data['result']);
            $result = TRUE;
		} else {
			$result = FALSE;
		}
