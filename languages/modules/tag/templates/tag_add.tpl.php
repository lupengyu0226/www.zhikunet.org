...
<!--
	$(function(){
		...
		$("#name").formValidator({onshow:"<?php echo L('input').L('name')?>",onfocus:"<?php echo L('input').L('name')?>"}).inputValidator({min:1,onerror:"<?php echo L('input').L('name')?>"}).ajaxValidator({type : "get",url : "",data :"app=tag&controller=tag&view=public_name",datatype : "html",async:'false',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "<?php echo L('name').L('exists')?>",onwait : "<?php echo L('connecting')?>"});
...
<div class="pad-10">
<form action="?app=tag&controller=tag&view=add&ac=<?php echo $ac?>" method="post" id="myform">
<div>
<fieldset>
