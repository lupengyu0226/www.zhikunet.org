...
class tag extends admin {
	...
	public function __construct() {
		$this->db = shy_base::load_model('tag_model');
	...
	public function init() {
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=tag&controller=tag&view=add\', title:\''.L('add_tag').'\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_tag'));
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($type == '1') { //自定义SQL
				...
				$html = shy_tag_class($module);
				$data = array();
				$tag = '{edi:'.$module.' action="'.$action.'" ';
			...
			$modules = array_merge(array(''=>L('please_select')),shy_base::load_config('modules'));
			...
			if ($module) $html = shy_tag_class($module);
			shy_base::load_app_func('global','template');
