...
<td align="center"><a class="xbtn btn-info btn-xs" href="javascript:block_update(<?php echo $v['id']?>, '<?php echo $v['name']?>')"><?php echo L('updates')?></a>
	<a class="xbtn btn-primary btn-xs" href="javascript:edit(<?php echo $v['id']?>, '<?php echo $v['name']?>')"><?php echo L('edit')?></a>
	<a class="xbtn btn-danger btn-xs" href="?app=block&controller=block_admin&view=del&id=<?php echo $v['id']?>" onclick="return confirm('<?php echo L('confirm', array('message'=>$v['name']))?>')"><?php echo L('delete')?></a></td>
</tr>
<?php 
	endforeach;
endif;
