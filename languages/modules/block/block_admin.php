...
class block_admin extends admin {
	...
	public function __construct() {
		$this->db = shy_base::load_model('block_model');
		$this->priv_db = shy_base::load_model('block_priv_model');
		$this->history_db = shy_base::load_model('block_history_model');
	...
	public function add() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($id = $this->db->insert(array('name'=>$name, 'pos'=>$pos, 'type'=>$type, 'siteid'=>$this->siteid), true)) {
				...
				showmessage(L('operation_success'), '?app=block&controller=block_admin&view=block_update&id='.$id);
			} else {
	...
	public function block_update() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($data['type'] == 2) {
				...
				if ($template) {
					$block = shy_base::load_app_class('block_tag');
			...
			if (!empty($data['data'])) {
				...
				$total = (is_array($data['data']) && !empty($data['data']))?count($data['data']):0;
			}
			shy_base::load_sys_class('form');
