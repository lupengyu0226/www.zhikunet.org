...
class sms extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->log_db = shy_base::load_model('sms_report_model');
		...
		//初始化smsapi
		shy_base::load_app_class('smsapi', '', 0);
	...
	public function sms_sent() {
		...
		if(empty($this->smsapi->userid)) {
			showmessage(L('need_band'), 'index.php?a=sms&c=sms&a=sms_setting&menuid='.$_GET[menuid].'&safe_edi='.$_SESSION['safe_edi']);
		}
		//检查短信余额
	...
	public function public_show_tpl(){
		...
		public function exportmobile() {
			...
			for ( $i = 0; $i <= ceil( $total / $perpage ); $i++ ) {
				$start = $i*$perpage;
				$_data = $this->member_db->select( $where,'userid,mobile',"$start, $perpage" );
				...
				foreach ( $data as $v ) {
					$datas[ $v['userid'] ] = $v;
					$userid_arr[ $v['userid'] ] = $v;
					if(isset( $_datas[ $v['userid'] ] )) {
						$datas[ $v['userid'] ] = array_merge( $_datas[ $v['userid'] ],$datas[ $v['userid'] ] );
					}
				}
