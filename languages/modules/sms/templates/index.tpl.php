...
<input type="hidden" value="sms" name="app">
<input type="hidden" value="sms" name="controller">
...
<?php } else {?>
<span class="font-fixh green">未绑定平台账户，请点击<a href="index.php?app=sms&controller=sms&view=sms_setting&menuid=1539&safe_edi=<?php echo $_GET['safe_edi'];?>"><span class="font-fixh">平台设置</span></a>绑定。</span>
<?php }?>
</div><br>
...
<?php if(is_array($smsprice_arr)) foreach($smsprice_arr as $k=>$v) {?>
	...
	<td width="10%" align="left"><a class="xbtn btn-danger btn-xs" href="<?php echo $this->smsapi->get_buyurl($v['productid']);?>" target="_blank"><?php echo L('buy')?></a></td>
	</tr>
<?php }?>
    </tbody>
    </table>
