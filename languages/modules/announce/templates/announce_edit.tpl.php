...
?>
<div class="pad-10">
<form method="post" action="?app=announce&controller=admin_announce&view=edit&aid=<?php echo $_GET['aid']?>" name="myform" id="myform">
...
<tbody>
	...
	<tr>
		...
		<td  id="show_template"><?php if ($an_info['style']) echo '<script type="text/javascript">$.getJSON(\'?app=admin&controller=category&view=public_tpl_file_list&style='.$an_info['style'].'&id='.$an_info['show_template'].'&module=announce&templates=show&name=announce&safe_edi=\'+safe_edi, function(data){$(\'#show_template\').html(data.show_template);});</script>'?></td>
	</tr>
	<tr>
	...
	<tr>
		<th><?php echo L('会员中心显示')?></th>
		<td><input name="announce[mpass]" type="radio" value="1" <?php if($an_info['mpass']==1) {?>checked<?php }?>></input>&nbsp;<?php echo L('yes')?>&nbsp;&nbsp;<input name="announce[mpass]" type="radio" value="0" <?php if($an_info['mpass']==0) {?>checked<?php }?>>&nbsp;<?php echo L('no')?></td>
	</tr>
    </tbody>
</table>
