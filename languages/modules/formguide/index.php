	...
	public function show() {
		...
		if ($setting['enabletime']) {
			if ($setting['starttime']>SYS_TIME || ($setting['endtime']+3600*24)<SYS_TIME) {
				$_GET['action'] ? exit : showmessage(L('form_expired'), APP_PATH.'index.php?app=formguide&controller=index&view=index');
			}
		}
		$userid = param::get_cookie('_userid');
		if ($setting['allowunreg']==0 && !$userid && $_GET['action']!='js') showmessage(L('please_login_in'), PASSPORT_PATH.'member-login.html?forward='.urlencode(HTTP_REFERER));
		if (isset($_POST['dosubmit'])) {
			...
			if ($dataid) {
				if ($setting['sendmail']) {
					...
					if (is_array($mails)) {
						foreach ($mails as $m) {
							sendmail($m, L('tips'), $this->M['mailmessage'].'<br>用户名:'.$data['username'].'<br>提交时间:'.date('Y-m-d H:i:s', $data['datetime']).'<br>提交IP:'.$data['ip']);
						}
					}
				}
