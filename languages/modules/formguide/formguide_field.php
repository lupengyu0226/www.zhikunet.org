...
class formguide_field extends admin {
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			showmessage(L('add_success'),'?app=formguide&controller=formguide_field&view=init&formid='.$formid);
		} else {
			$show_header = $show_validator = $show_dialog = '';
	...
	public function edit() {
		if (isset($_POST['dosubmit'])) {
			...
			showmessage(L('update_success'),'?app=formguide&controller=formguide_field&view=init&formid='.$formid);
		} else {
			if (isset($_GET['formid']) && !empty($_GET['formid'])) {
				shy_base::load_sys_class('form','',0);
				require MODEL_PATH.'fields.inc.php'; 
