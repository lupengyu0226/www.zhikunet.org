...
class formguide extends admin {
	...
	public function init() {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=formguide&controller=formguide&view=add\', title:\''.L('formguide_add').'\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('formguide_add'));
	...
	public function add() {
		if (isset($_POST['dosubmit'])) {
			...
			define('MODEL_PATH',SHY_PATH.'modules'.DIRECTORY_SEPARATOR.'formguide'.DIRECTORY_SEPARATOR.'fields'.DIRECTORY_SEPARATOR);
			$create_sql = file_get_contents(MODEL_PATH.'create.sql');
			$this->m_db = shy_base::load_model('sitemodel_field_model');
			...
			showmessage(L('add_success'), '?app=formguide&controller=formguide_field&view=init&formid='.$formid, '', 'add');
		} else {
			...
			shy_base::load_sys_class('form', '', false);
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			showmessage(L('update_success'), '?app=formguide&controller=formguide&view=init&formid='.$formid, '', 'edit');
		} else {
			$siteid = $this->get_siteid();
			$template_list = template_list($siteid, 0);
			$site = shy_base::load_app_class('sites','admin');
	...
	public function delete() {
		...
		if (isset($_GET['formid']) && !empty($_GET['formid'])) {
			$formid = intval($_GET['formid']);
			$m_db = shy_base::load_model('sitemodel_field_model');
	...
	private function sql_split($sql) {
		...
		$sql = str_replace("shuyang_form_table", $this->m_db->db_tablepre.'form_'.$this->tablename, $sql);
		$ret = array();
		$num = 0;
