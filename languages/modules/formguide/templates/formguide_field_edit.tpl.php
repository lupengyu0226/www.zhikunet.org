...
$(function(){
	...
	$("#field").formValidator({onshow:"<?php echo L('input').L('fieldname')?>",onfocus:"<?php echo L('fieldname').L('between_1_to_20')?>"}).regexValidator({regexp:"^[a-zA-Z]{1}([a-zA-Z0-9]|[_]){0,19}$",onerror:"<?php echo L('fieldname_was_wrong');?>"}).inputValidator({min:1,max:20,onerror:"<?php echo L('fieldname').L('between_1_to_20')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data : "app=formguide&controller=formguide_field&view=public_checkfield&formid=<?php echo $formid?>&oldfield=<?php echo $field;?>",
		datatype : "html",
		cached:false,
...
<a href="javascript:;" class="on"><em><?php echo L('edit_field');?></em></a><span>|</span><a href="?app=formguide&controller=formguide_field&view=init&formid=<?php echo $formid?>&menuid=<?php echo $_GET['menuid']?>"><em><?php echo L('manage_field');?></em></a><span>|</span></div>
  <div class="bk10"></div>
</div>
<form name="myform" id="myform" action="?app=formguide&controller=formguide_field&view=edit" method="post">
