...
?>
<div class="pad-10">
<form method="post" action="?app=formguide&controller=formguide&view=edit&formid=<?php echo $_GET['formid']?>" name="myform" id="myform">
...
<tbody>
	...
	<tr>
		...
		<td id="show_template"><script type="text/javascript">$.getJSON('?app=admin&controller=category&view=public_tpl_file_list&style=<?php echo $data['default_style']?>&id=<?php echo $data['show_template']?>&module=formguide&templates=show&name=info&safe_edi='+safe_edi, function(data){$('#show_template').html(data.show_template);});</script></td>
	</tr>
	<tr>
		<th><strong>js调用使用的模板：</strong></th>
		<td id="show_js_template"><script type="text/javascript">$.getJSON('?app=admin&controller=category&view=public_tpl_file_list&style=<?php echo $data['default_style']?>&id=<?php echo $data['js_template']?>&module=formguide&templates=show_js&name=info&safe_edi='+safe_edi, function(data){$('#show_js_template').html(data.show_js_template);});</script></td>
	</tr>
	</tbody>
</table>
...
$(document).ready(function(){
	...
	$('#tablename').formValidator({onshow:"<?php echo L('please_input_tallename')?>", onfocus:"<?php echo L('standard')?>", oncorrect:"<?php echo L('right')?>"}).regexValidator({regexp:"^[a-zA-Z]{1}([a-zA-Z0-9]|[_]){0,19}$",onerror:"<?php echo L('tablename_was_wrong');?>"}).inputValidator({min:1,onerror:"<?php echo L('tablename_no_empty')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data : "app=formguide&controller=formguide&view=public_checktable&formid=<?php echo $_GET['formid']?>",
		datatype : "html",
		cached:false,
