...
$(function(){
	...
	$("#field").formValidator({onshow:"<?php echo L('input').L('fieldname')?>",onfocus:"<?php echo L('fieldname').L('between_1_to_20')?>"}).regexValidator({regexp:"^[a-zA-Z]{1}([a-zA-Z0-9]|[_]){0,19}$",onerror:"<?php echo L('fieldname_was_wrong');?>"}).inputValidator({min:1,max:20,onerror:"<?php echo L('fieldname').L('between_1_to_20')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data : "app=formguide&controller=formguide_field&view=public_checkfield&modelid=<?php echo $formid?>",
		datatype : "html",
		cached:false,
...
<div class="content-menu ib-a blue line-x"><a class="add fb" href="?app=formguide&controller=formguide_field&view=add&modelid=<?php echo $formid?>&menuid=<?php echo $_GET['menuid']?>"><em><?php echo L('add_field');?></em></a>
　<?php if (isset($formid) && !empty($formid)) {?><a href="?app=formguide&controller=formguide_field&view=init&modelid=<?php echo $formid?>&menuid=<?php echo $_GET['menuid']?>"><em><?php echo L('manage_field');?></em></a><?php } else {?><a href="?app=formguide&controller=formguide_field&view=init&menuid=<?php echo $_GET['menuid']?>"><em><?php echo L('public_field_manage')?></em></a><?php }?></div>
  <div class="bk10"></div>
</div>
<form name="myform" id="myform" action="?app=formguide&controller=formguide_field&view=add" method="post">
...
<script type="text/javascript">
	...
	$('#css').css('display','none');
		...
		$.getJSON("?app=formguide&controller=formguide_field&view=public_field_setting&fieldtype="+fieldtype, function(data){
