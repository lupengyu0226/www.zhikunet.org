...
<?php foreach ((array)$url as $key=>$val):?>
...
function show_content(url) {
	...
	$.get("?app=collection&controller=node&view=public_test_content&nodeid=<?php echo $nodeid?>&url="+url+'&safe_edi=<?php echo $_SESSION['safe_edi']?>', function(data){art.dialog({id:'test'}).close();
