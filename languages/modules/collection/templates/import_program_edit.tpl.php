...
<div class="pad-lr-10">
<form name="myform" action="<?php echo U('collection/node/import_program_edit',array('nodeid'=>(isset($nodeid)?$nodeid:0),'type'=>$type,'id'=>$id,'catid'=>$catid));?>" method="post" id="myform">
<fieldset>
	<legend><?php echo L('the_edit_publication_solutions')?></legend>
		...
		<tr>
			...
			<td>
			<label><input name="add_introduce" type="checkbox"  <?php if($r['config']['add_introduce']) { ?>checked<?php } ?>  value="1"><?php echo L('if_the_contents_of_intercepted')?></label><input type="text" name="introcude_length" value="<?php echo $r['config']['introcude_length'] ?>" size="3"><?php echo L('characters_to_a_summary_of_contents')?>
			</td>
		</tr>
		<tr>
			<td width="120"><?php echo L('the_withdrawal_of_thumbnails')?>：</td> 
			<td>
			<label><input type='checkbox' name='auto_thumb'  <?php if($r['config']['auto_thumb']) { ?>checked<?php } ?> value="1"><?php echo L('whether_access_to_the_content_of')?></label><input type="text" name="auto_thumb_no" value="1" size="2" class=""><?php echo L('picture_a_caption_pictures')?>
		...
		<tr>
			...
			<td>
			<?php if(!empty($cat['setting']['workflowid'])) {
				echo form::radio(array('1'=>L('pendingtrial'), '99'=>L('fantaoboys')),$r['config']['content_status'], 'name="content_status"');
				} else {
...
<?php
	foreach($model as $k=>$v) {
		if (in_array($v['formtype'], array('catid','posids', 'groupid', 'readpoint','template'))) continue;
?>
    <tr>
		...
		<td align="left"><input type="hidden" name="model_field[]" value="<?php echo $v['field']?>">
		
		<?php echo form::select($node_field,$r['config']['map'][$v['field']], 'name="node_field[]"')?>
       
        </td>
		
        <td align="left"><div class="funcs_div"><input type="text" name="funcs[]" class="funcs"  value="<?php echo $r['config']['funcs'][$v['field']]?>"><a href="javascript:void(0)" onclick="clearh();show_funcs(this);" onmouseout="hidden_funcs_div_1()"><img src="<?php echo IMG_PATH?>admin_img/toggle-collapse-dark.png"></a></div></td>
