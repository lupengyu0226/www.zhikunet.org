...
$(document).ready(function() {
	...
	$("#name").formValidator({onshow:"<?php echo L('input').L('nodename')?>",onfocus:"<?php echo L('input').L('nodename')?>"}).inputValidator({min:1,onerror:"<?php echo L('input').L('nodename')?>"}).ajaxValidator({type : "get",url : "",data :"app=collection&controller=node&view=public_name",datatype : "html",async:'false',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "<?php echo L('nodename').L('exists')?>",onwait : "<?php echo L('connecting')?>"});

});
</script>
<div class="pad-10">
<form name="myform" action="?app=collection&controller=node&view=copy&nodeid=<?php if(isset($nodeid)) echo $nodeid?>" method="post" id="myform">
