...
class collection {
		...
		if ($html = self::get_html($url, $config)) {
			...
			if ($page == 0) {
				...
				if (empty($page) && !empty($data['content']) && $config['down_attachment'] == 1) {
					...
					shy_base::load_sys_class('attachment','',0);
	...
	protected static function download_img($old, $out) {
		if (!empty($old) && !empty($out) && strpos($out, '//') === false && IsDomain(parse_url($out)['host']) === false) {
			return str_replace($out, self::url_check($out,self::$url, self::$config), $old);
		} else {
			if(substr($out,0,2)=='//'&&!isset(parse_url($out)['scheme'])){
			return str_replace($out,parse_url(self::$url)['scheme'].':'.$out, $old);  
			}else{
			return $old;
			}
		}
	...
	public static function get_url_lists($url, &$config) {
		if ($html = self::get_html($url, $config)) {
			if ($config['sourcetype'] == 4) { //RSS
				$xml = shy_base::load_sys_class('xml');
	...
	protected static function get_html($url, &$config) {
		if (!empty($url) && $html = self::get_curl($url)) {//@file_get_contents($url)
...
/**
	 ...
	 * @param array $config  配置
	 */
	protected static function get_curl($url) {
		$ch = curl_init();
		$url = self::getrealurl($url);
		$robotip = mt_rand(0,255).'.'.mt_rand(0,255).'.'.mt_rand(0,255).'.'.mt_rand(0,255);
		$CURLOPT_HTTPHEADER=array(array('X-FORWARDED-FOR:'.$robotip, 'CLIENT-IP:'.$robotip));
		...
		curl_setopt($ch,CURLOPT_HTTPHEADER,$CURLOPT_HTTPHEADER[array_rand($CURLOPT_HTTPHEADER)]);//伪造ip
	...
	protected static function getrealurl($url) {
		if (preg_match('/^https:\/\//', $url)) { //针对ssl进行处理
		...
		stream_context_get_default([
			'http' => [
				'method'=>"GET",
				'header'=>"User-Agent: Mozilla/5.0 (Linux; U; Android 2.3.7; zh-cn; c8650 Build/GWK74) AppleWebKit/533.1 (KHTML, like Gecko)Version/4.0 MQQBrowser/4.5 Mobile Safari/533.1s",
			]
		]);
	...
	protected static function replace_sg($html) {
		$list = explode(L('[content]',array(),'collection'), $html);
