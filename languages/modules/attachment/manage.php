...
class manage extends admin {
	...
	function __construct() {
		...
		$this->db= shy_base::load_model('attachment_model');
		$this->attachment = shy_base::load_sys_class('attachment');
	...
	public function init() {
		...
		$infos = $this->db->listinfo($where, 'aid DESC', $page, $pagesize = 20);
		$pages = $this->db->pages;
	...
	public function pulic_dirmode_del() {
		...
		$tmpdir = SHUYANG_PATH.$tmpdirs[0].'/';
		if($tmpdir!=$this->upload_path) {
			showmessage(L('illegal_operation'));
		}
		$file = SHUYANG_PATH.$dir.DIRECTORY_SEPARATOR.$filename;
	...
	public function delete() {
		$aid = $_GET['aid'];
		$attachment_index = shy_base::load_model('attachment_index_model');
		if($this->attachment->delete(array('aid'=>$aid))) {
			$attachment_index->delete(array('aid'=>$aid));
			//至尊版功能 删除附件统计
			m_stats('delete_attachment');
			exit('1');
		} else {
			exit('0');
		}
