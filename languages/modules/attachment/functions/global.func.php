<?php
	...
	function file_icon($file,$type = 'png') {
		...
		if(in_array($ext,$ext_arr)) return 'https://statics.05273.cn/statics/images/ext/'.$ext.'.'.$type;
		else return 'https://statics.05273.cn/statics/images/ext/blank.'.$type;
	}
	...
	function file_list($dirpath,$currentdir) {
		...
		$list['local'] = str_replace(array(SHY_PATH, DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR), array('',DIRECTORY_SEPARATOR), $filepath);
		return $list;
	}
	...
	function initupload($module, $catid,$args, $userid, $groupid = '8', $isadmin = '0',$userid_flash='0'){
		...
		$sess_id = SYS_TIME;
		$admin_url = shy_base::load_config('system','admin_url');
		$upload_path = empty($admin_url) ? APP_PATH : 'https://'.$admin_url.'/';
		$swf_auth_key = md5(shy_base::load_config('system','auth_key').$sess_id);
		$init =  'var swfu = \'\';
		$(document).ready(function(){
		swfu = new SWFUpload({
			flash_url:"'.JS_PATH.'swfupload/swfupload.swf?"+Math.random(),
			upload_url:"'.$upload_path.'index.php?app=attachment&controller=attachments&view=swfupload&dosubmit=1",
