...
class attachments {
	...
	function __construct() {
		...
		$this->upload_url = shy_base::load_config('system','upload_url');
		$this->upload_path = shy_base::load_config('system','upload_path');		
	...
	public function upload() {
		...
		if($a){
			...
			$this->upload_json($a[0],$this->upload_url.$filepath,$attachment->uploadedfiles[0]['filename']);
	...
	public function swfupload(){
		...
		if(isset($_POST['dosubmit'])){
			if( $_POST['swf_auth_key'] != md5(shy_base::load_config('system','auth_key').$_POST['SWFUPLOADSESSID']) || ($_POST['isadmin']==0 && !$grouplist[$_POST['groupid']]['allowattachment'])) exit();
			shy_base::load_sys_class('attachment','',0);
	...
	public function swfdelete() {
		$attachment = shy_base::load_sys_class('attachment');
	...
	public function album_dir() {
		...
		$local = str_replace(array(SHY_PATH, SHUYANG_PATH ,DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR), array('','',DIRECTORY_SEPARATOR), $filepath);
