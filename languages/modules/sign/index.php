...
class index {
    ...
    public function sign() {
        ...
        if (!$userid) {
            showmessage('请登录', PASSPORT_PATH . 'member-login.html');
        }
        $now_time = time();
        $set = $this->sign_set_db->get_one(array('siteid' => $siteid));
        ...
        if ($now_time < $todayBegin || $now_time > $todayEnd) {
            showmessage('不在签到时间范围', HTTP_REFERER);
        }
        ...
        if (empty($sign_time_check_today)) {
            ...
            showmessage('签到成功', HTTP_REFERER);
        } else {
            showmessage('已签到', HTTP_REFERER);
        }
    }
    /**
     * AJAX 签到接口
