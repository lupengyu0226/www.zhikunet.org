...
class index extends admin {
	...
	public function public_update_config() {
		...
		if (empty($dir)) { 
			showmessage(L('please_select_the_content'), '?app=scan&controller=index&view=init');
		}
		$info['dir'] = var_export($dir, true);
		setcache('scan_config', $info, 'scan');
		showmessage(L('configuration_file_save_to_the'), '?app=scan&controller=index&view=public_file_count');
	}
	...
	public function public_file_count() {
		...
		foreach ($scan['dir'] as $v) {
			if (is_dir($v)) {
				...
				$list = array_merge($list, array(str_replace(SHUYANG_PATH, '', $v)=>md5_file($v)));
			}
		}
		setcache('scan_list', $list, 'scan');
		showmessage(L('documents_to_file_the_statistics'), '?app=scan&controller=index&view=public_file_filter');
	}
	...
	public function public_file_filter() {
		...
		showmessage(L('file_through_a_feature_the_function_is'), '?app=scan&controller=index&view=public_file_func');
	}
	...
	public function public_file_func() {
		...
		if (isset($scan['func']) && !empty($scan['func'])) {
			foreach ($file_list as $key=>$val) {
				$html = file_get_contents(SHUYANG_PATH.$key);
		...
		showmessage(L('feature_function_complete_a_code_used_by_filtration'), '?app=scan&controller=index&view=public_file_code');
	}
	...
	public function public_file_code() {
		...
		if (isset($scan['code']) && !empty($scan['code'])) {
			foreach ($file_list as $key=>$val) {
	            ...
	            if(strtolower(substr($key, -4)) == '.php' && function_exists('zend_loader_file_encoded') && zend_loader_file_encoded(SHUYANG_PATH.$key)) {
	            	$badfiles[$key]['zend'] = 'zend encoded';
	            }
			}
		}
		setcache('scan_bad_file', $badfiles, 'scan');
		showmessage(L('scan_completed'), '?app=scan&controller=index&view=scan_report&menuid=1005');
	}
	
	public function scan_report() {
		$badfiles = getcache('scan_bad_file', 'scan');
		if (empty($badfiles)) {
			showmessage(L('scan_to_find_a_result_please_to_scan'), '?app=scan&controller=index&view=init');
		}
	...
	public function view() {
		...
		if (!file_exists(SHUYANG_PATH.$url)) {
			showmessage(L('file_not_exists'));
		}
		$html = file_get_contents(SHUYANG_PATH.$url);
	...
	public function md5_creat() {
		...
		switch ($pro) {
			case '1'://统计文件
				...
				ob_flush();
				ob_clean();
				$list = scan_file_lists(SHUYANG_PATH, 1, 'php', 0, 1);
				setcache('md5_'.date('Y-m-d'), $list, 'scan');
				echo '<script type="text/javascript">location.href="?app=scan&controller=index&view=md5_creat&pro=2&safe_edi='.$_SESSION['safe_edi'].'"</script>';
				break;
				
			case '2':
				showmessage(L('viewreporttrue'),'?app=scan&controller=index&view=init');
				break;
		}
	}
}