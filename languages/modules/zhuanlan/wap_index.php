<?php
	...
	class wap_index{
		...
		function __construct() {
			...
			if($this->setting[$this->siteid]['status']!=1) mobilemsg('zhuanlan_close_status');
		}
	public function init() {
		...
		include template('zhuanlan', 'wap_index');
		//cache_page(3600);
	}
    //获取更多内容,新闻内容专用
	public function more() {
        ...
        $page++;
		ob_start();
        include template('zhuanlan', 'wap_get_more');
	...
	public function show() {
		...
		if(!empty($info) && $info['status']==1){
			...
			//cache_page_start();
			include template('zhuanlan','wap_show');
			//cache_page(3600);
		}else{
			mobilemsg(L('zhuanlan_close_status'), HTTP_REFERER);
		}
	}
}
?>