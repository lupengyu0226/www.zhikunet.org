...
public function responseMsg(){
    ...
    private function receiveText($object){
        ...
        $this->replyword->insert(array('siteid'=>'1','keyword'=>$keyword,'name'=>'微信','inputtime'=>$addtime,'username'=>$memberdata['nickname'],'type'=>$object->MsgType,'passed'=>'0'));
		return $resultStr;
    }
   //接收位置消息
   ...
   //接收图片消息
    private function receiveImage($object){
		...
		$memberdata=get_userinfo($openid);
		$addtime = date('Y-m-d H:i:s',SYS_TIME);
        $this->replyword->insert(array('siteid'=>'1','keyword'=>$keyword,'name'=>'微信','inputtime'=>$addtime,'username'=>$memberdata['nickname'],'photo'=>$object->PicUrl,'type'=>$object->MsgType,'passed'=>'0'));
        return $resultStr;
	}
     //接收语音消息
    ...
    private function receiveEvent($object){
        ...
        switch ($object->Event){
			...
			//保存关注用户信息
			  $contentStr = array(); 
			  $openid = $object->FromUserName; //获取用户的openID
			  $memberdata=get_userinfo($openid);//根据用户的openID获取用户的详细信息
			  $openid_arr=array('openid'=>$openid);
			  $openid_data=JSON($openid_arr);
			  $access_token = get_access_token();
			  if($access_token){		  
			  //获取查询用户所在分组ID		  		  
				$url="https://api.weixin.qq.com/cgi-bin/groups/getid?access_token=".$access_token;		  
				$result = https_request($url, $openid_data);		  
				$groupid_arr=json_decode($result,true);
				$memberdata['groupid']=$groupid_arr['groupid'];
				$memberdata['openid']=$memberdata['openid'].'@';
			  }			  
			  $openid=$openid.'@';
			  $info = $this->dbmember->get_one(array('openid'=>$openid)); 
			  if($info){ 
				$this->dbmember->update(array('subscribe'=>"1",'status'=>"1",'sex'=>$memberdata['sex'],'language'=>$memberdata['language'],'nickname'=>$memberdata['nickname'],'city'=>$memberdata['city'],'platform'=>"微信",'province'=>$memberdata['province'],'country'=>$memberdata['country'],'headimgurl'=>$memberdata['headimgurl'],'subscribe_time'=>$memberdata['subscribe_time'],'remark'=>$memberdata['remark'],'unionid'=>$memberdata['unionid'],'groupid'=>$memberdata['groupid']),array('openid'=>$memberdata['openid'])); 
			  }else{ 
				$sql = array('subscribe'=>$memberdata['subscribe'],'openid'=>$memberdata['openid'],'status'=>"1",'sex'=>$memberdata['sex'],'language'=>$memberdata['language'],'nickname'=>$memberdata['nickname'],'city'=>$memberdata['city'],'platform'=>"微信",'province'=>$memberdata['province'],'country'=>$memberdata['country'],'headimgurl'=>$memberdata['headimgurl'],'subscribe_time'=>$memberdata['subscribe_time'],'remark'=>$memberdata['remark'],'unionid'=>$memberdata['unionid'],'groupid'=>$memberdata['groupid']);
            ...
            case "unsubscribe":
				...
				$memberdata=get_userinfo($openid); 
				$memberdata['openid']=$memberdata['openid'].'@'; 
				$this->dbmember->update(array('subscribe'=>"0",'status'=>"0"),array('openid'=>$memberdata['openid'])); 
                break;
           case "CLICK":
				$data =$this->db->select();	
				$even=array();
