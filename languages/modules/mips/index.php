...
class index {
	...
	function __construct() {
		...
		define('MIPS_SITEURL', $this->mips['domain'] ? $this->mips['domain'].'/' : APP_PATH.'index.php?app=mips&siteid='.$this->siteid);
		if($this->mips['status']!=1) mipsmsg(L('mips_close_status'));
	}
	//首页
	public function init() {
		$this->weixin = shy_base::load_app_class('wxauth', 'mips');
		...
		//SEO
		$SEO = seo($siteid);
		$sitelist  = getcache('mips_site','mips');
		$default_style = $sitelist[$siteid]['default_style'];
		$CATEGORYS = getcache('category_content_'.$siteid,'commons');
		include template('mips','index',$default_style);
		//cache_page(120);
	}
	//内容页
	public function show() {
		...
		$id = intval($_GET['id']);
		$MIPS = $this->mips;
		if(!$catid || !$id) mipsmsg(L('information_does_not_exist'),'blank');
		$_userid = $this->_userid;
		$_username = $this->_username;
		$_groupid = $this->_groupid;
		...
		if(!isset($CATEGORYS[$catid]) || $CATEGORYS[$catid]['type']!=0) mipsmsg(L('information_does_not_exist'),'blank');
		$this->category = $CAT = $CATEGORYS[$catid];
		$this->category_setting = $CAT['mipssetting'] = string2array($this->category['mipssetting']);
		...
		if(!$r || $r['status'] != 99){
			...
			mipsmsg(L('info_does_not_exists'),'blank');
			exit();
		}	
		...
		$template = $template ? $template : $CAT['mipssetting']['show_template'];
		...
		define('STYLE',$CAT['mipssetting']['template_list']);
		...
		if($rs['paginationtype']==1) {
			...
			$contentpage = shy_base::load_app_class('contentpage', 'mips');
		...
		if($rs['paginationtype']!=0) {
			...
			if($CONTENT_POS !== false) {
				$this->url = shy_base::load_app_class('urls', 'mips');
				...
				for($i=1; $i<=$pagenumber; $i++) {
					$pageurls[$i] = $this->url->mipsshow($id, $i, $catid, $rs['inputtime']);
				}
				...
				//当不存在 [/page]时，则使用下面分页
				$pages = ipad_pages($pagenumber,$page, $pageurls);
				//判断[page]出现的位置是否在第一位 
				if($CONTENT_POS<7) {
		...
		include template('mips',$template);
	}
	//列表页
	public function lists() {
		...
		if(!isset($CATEGORYS[$catid])){
			header('HTTP/1.1 404 Not Found');
			header('status: 404 Not Found');
			mipsmsg(L('category_not_exists'),'blank');
			exit();
		}
		$CAT = $CATEGORYS[$catid];
		$siteid = $GLOBALS['siteid'] = $CAT['siteid'];
		...
		//SEO
		if(!$mipssetting['meta_title']) $mipssetting['meta_title'] = $catname;
		$SEO = seo($siteid, '',$mipssetting['meta_title'],$mipssetting['meta_description'],$mipssetting['meta_keywords']);
		define('STYLE',$mipssetting['template_list']);
		$page = intval($_GET['page']);

		$template = $mipssetting['category_template'] ? $mipssetting['category_template'] : 'category';
		$template_list = $mipssetting['list_template'] ? $mipssetting['list_template'] : 'list';
		...
		if($type==0) {
			...
			$urlrules = str_replace('|', '~',$urlrules[$mipssetting['category_ruleid']]);
			...
			$_URL = shy_base::load_app_class('urls','mips');
		...
		//单网页
			...
			if($r) extract($r);
			$template = $mipssetting['page_template'] ? $mipssetting['page_template'] : 'page';
			$arrchild_arr = $CATEGORYS[$parentid]['arrchildid'];
			if($arrchild_arr=='') $arrchild_arr = $CATEGORYS[$catid]['arrchildid'];
			$arrchild_arr = explode(',',$arrchild_arr);
			array_shift($arrchild_arr);
			$keywords = $keywords ? $keywords : $mipssetting['meta_keywords'];
			$SEO = seo($siteid, 0, $title,$mipssetting['meta_description'],$keywords);
			include template('mips',$template);
		}
	}
	//JSON 输出
	...
	function comment() {
		if($_POST['dosumbit']) {
			...
			$username = $this->mips['sitename'].L('weixin_friends');
			}else{
			$username = $this->mips['sitename'].L('shuyang_friends');
			}
	...
	function comment_list() {
		...
		include template('mips', 'comment_list');
	}
    //获取更多内容,话题专用
	public function more() {
        ...
        $page++;
		ob_start();
        include template('mips', 'get_more');
	...
	public function newsmore() {
        ...
        $page++;
		ob_start();
        include template('mips', 'get_newsmore');
        $html = ob_get_contents();
        ob_end_clean();
        echo json_encode(array('data'=>$html,'page'=>$page));
        exit();
	...
	public function tvmore() {
        ...
        $page++;
		ob_start();
        include template('mips', 'get_tvmore');
        $html = ob_get_contents();
        ob_end_clean();
        echo json_encode(array('data'=>$html,'page'=>$page));
        exit();
