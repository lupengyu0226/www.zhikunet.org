...
class mips_admin extends admin {
	private $db,$type_db;
	function __construct() {
		parent::__construct();
		$this->sites = shy_base::load_app_class('sites','admin');
		$this->db = shy_base::load_model('mips_model');
	}
	...
	function init() {
		...
		$show_dialog = true;
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=mips&controller=mips_admin&view=add\', title:\''.L('add_site').'\', width:\'440\', height:\'380\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('mips_add_site'));
	...
	function add() {
		if($_POST['dosubmit']) {
			...
			if($this->db->get_one(array('siteid'=>$siteid))) {
				showmessage(L('mips_add_samesite_error'),HTTP_REFERER);
			}
			$sitename = trim(new_addslashes($_POST['sitename']));
			$logo = trim($_POST['logo']);
			$domain = trim($_POST['domain']);
	...
	function delete() {
		$siteid = intval($_GET['siteid']) ? intval($_GET['siteid']) : showmessage(L('parameter_error'),HTTP_REFERER);
		if($siteid == 1) showmessage(L('mips_permission_denied_del'),HTTP_REFERER);
		$this->db->delete(array('siteid'=>$siteid));
		$this->type_db->delete(array('siteid'=>$siteid));
		$this->mips_site_cache();
		showmessage(L('mips_del_succ'),HTTP_REFERER);
	}
	...
	function public_status() {
		 ...
		 $this->mips_site_cache();
		 showmessage(L('mips_change_status'),HTTP_REFERER);
	}
	public function public_show_cat_ajx() {
		$parentid = intval($_GET['parentid']);
		$siteid = intval($_GET['siteid']);
		echo form::select_category('',0,'name="addcat['.$parentid.'][]"',L('mips_type_bound'),0,0,0,$siteid);
	}
	
	private function mips_site_cache() {
		$datas = $this->db->select();
		$array = array();
		foreach ($datas as $r) {
			$array[$r['siteid']] = $r;
		}
		setcache('mips_site', $array,'mips');		
	}
