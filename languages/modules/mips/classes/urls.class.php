...
class urls{
	 ...
	 * @return array 0=>url , 1=>生成路径
	 */
	public function mipsshow($id, $page = 0, $catid = 0, $time = 0, $prefix = '',$data = '',$action = 'edit',$upgrade = 0) {
		$page = max($page,1);
		$urls = $catdir = '';
		$category = $this->categorys[$catid];
		$mipssetting = string2array($category['mipssetting']);
		$content_ishtml = $mipssetting['content_ishtml'];
		//当内容为转换或升级时
		if($upgrade || (isset($_POST['upgrade']) && defined('IN_ADMIN') && $_POST['upgrade'])) {
			if($_POST['upgrade']) $upgrade = $_POST['upgrade'];
			$upgrade = '/'.ltrim($upgrade,WEB_PATH);
			...
			$show_ruleid = $mipssetting['show_ruleid'];
			$urlrules = $this->urlrules[$show_ruleid];
			if(!$time) $time = SYS_TIME;
			$urlrules_arr = explode('|',$urlrules);
			if($page==1) {
			...
			if($content_ishtml && $url) {
				...
				$url_arr[0] = $url_arr[1] = 'MIP_PATH'.$urls;
			}
		}
		return $url_arr;
	}

	public function mipmipsshow($id, $page = 0, $catid = 0, $time = 0, $prefix = '',$data = '',$action = 'edit',$upgrade = 0) {
		$page = max($page,1);
		$urls = $catdir = '';
		$category = $this->categorys[$catid];
		$mipssetting = string2array($category['mipssetting']);
		...
		if($upgrade || (isset($_POST['upgrade']) && defined('IN_ADMIN') && $_POST['upgrade'])) {
			...
			if($content_ishtml && $url) {
				...
				$url_arr[0] = $url_arr[1] = mips_PATH.$urls;
			}
		}
		return $url_arr;
	}
