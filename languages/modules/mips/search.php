...
shy_base::load_app_func('global','mips');
class search {
	function __construct() {
		$this->db = shy_base::load_model('search_model');
		$this->content_db = shy_base::load_model('content_model');
		define('mips_SITEURL',shy_base::load_config('system','mips_path'));
	}
	...
	public function init() {
		...
		$type_module = getcache('type_module_'.$siteid,'search');

		if(isset($_GET['q'])) {
			if(trim($_GET['q'])=='') {
				header('Location: '.mips_SITEURL.'index.php?controller=search');exit;
			}
			...
			$mori = '';
			if($q == '2012世界末日'){
			$mori = '<script type="text/javascript" src="http://pic.05273.cn/statics/js/mori.js"></script>';
			}
			if($q == '世界末日'){
			$mori = '<script type="text/javascript" src="http://pic.05273.cn/statics/js/mori.js"></script>';
			}
			...
			/* S 搜索感知*/
			if($page == 1 && $_GET['q']==$q) {//翻页不重复记录
				$condition = array();
				$condition['search_word']  = $_GET['q'];
				$condition['search_from'] = 'wap';
				$condition['ip'] = ip();
				$time = time();
				$this->search_word_db = shy_base::load_model('user_search_word_model');
				$search_word = $this->search_word_db->get_one($condition,'id,search_times');
				if($search_word){
					$this->search_word_db->update( array('search_times'=>$search_word['search_times']+1, 'last_search_time'=>$time, 'ip'=>$condition['ip']) ,array('id'=>$search_word['id']) );
					$_SESSION['check_search'] = time();
				}else{
					if($_SESSION['check_search'] == time())continue;
					$condition['last_search_time'] = $time;
					$this->search_word_db->insert($condition);
				}
			}
			/* E 搜索感知*/
			...
			if($setting['sphinxenable']) {
				$sphinx = shy_base::load_app_class('search_interface','search', 1);
				...
				shy_base::load_sys_class('segment', '', 0);
				$segment = new segment();
				//分词结果
			...
			if($setting['relationenble']) {
				...
				$this->keyword_db = shy_base::load_model('search_keyword_model');
			...
			//如果结果不为空
			  if(!empty($result) || !empty($commend['id'])) {
				...
				//获取模型id
				$model_type_cache = getcache('type_model_'.$siteid,'search','search');
				...
				if($modelid) {
					...
					if(empty($this->content_db->model_tablename)) {
						$this->content_db = shy_base::load_model('mall_content_model');
						$this->content_db->set_model($modelid);
					...
					//读取专辑搜索接口
					$special_api = shy_base::load_app_class('search_api', 'special');
			...
			include	template('mips','search_list');
		} else {
			include	template('mips','search_index');
		}
	}
	...
	public function public_suggest_search() {
		...
		$this->keyword_db = shy_base::load_model('search_keyword_model');
		$suggest = $this->keyword_db->select("pinyin like '$pinyin%'", '*', 10, 'searchnums ASC');
