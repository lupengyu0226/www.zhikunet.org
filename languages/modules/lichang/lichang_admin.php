...
class lichang_admin extends admin {
	...
	public function init() {
		$lichang_program = getcache('lichang_program', 'commons');
		$lichang_program = isset($lichang_program[$this->get_siteid()]) ? $lichang_program[$this->get_siteid()] : array();
		...
		if ($catid) {
			...
			$data = $lichang_db->listinfo($sql, $sql_order, $page);
			...
			$pages = $lichang_db->pages;
		}
		$order_list = array('-1'=>L('total'));
		foreach ($lichang_program as $k=>$v) {
			$order_list[$k]=$v['name'];
		}
		shy_base::load_sys_class('form', '', 0);
		include $this->admin_tpl('lichang_list');
	}
	...
	public function setting() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$lichang_program[$this->get_siteid()] = $data;
			setcache('lichang_program', $lichang_program, 'commons');
			showmessage(L('operation_success'), HTTP_REFERER);
		} else {
			$lichang_program = isset($lichang_program[$this->get_siteid()]) ? $lichang_program[$this->get_siteid()] : array();
			include $this->admin_tpl('lichang_setting');
		}
	}
}
