...
class comment_admin extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->comment_setting_db = shy_base::load_model('comment_setting_model');
		$this->comment_data_db = shy_base::load_model('comment_data_model');
		$this->comment_db = shy_base::load_model('comment_model');
	...
	public function listinfo() {
		...
		if (isset($_GET['search'])) {
			...
			switch ($searchtype) {
				case '0':
					$sql = "SELECT `commentid` FROM `shuyang_comment` WHERE `siteid` = '$this->siteid' AND `title` LIKE '%$keywords%' AND `tableid` = '$tableid' ";
				...
				case '1':
					...
					$sql = "SELECT `commentid` FROM `shuyang_comment` WHERE `commentid` LIKE 'content_%-$keywords-%' ";
					$this->comment_db->query($sql);
					$data = $this->comment_db->fetch_array();
					if (!empty($data)) {
						foreach ($data as $d) {
	...
	function public_elite() {
		...
		if (!$max_table) {
		 ...
		 $elite = intval($_GET['elite']) && intval($_GET['elite'])== 1 ? '1' : '0';
		 $tableid = isset($_GET['tableid']) ? intval($_GET['tableid']) : $max_table;
		 $id = intval($_GET['id']) ? intval($_GET['id']) : showmessage(L('评论不存在'),HTTP_REFERER);
		 $this->comment_data_db->table_name($tableid);
		 $this->comment_data_db->update(array('elite'=>$elite), array('id'=>$id));
		 showmessage(L('设置成功'),HTTP_REFERER);
	}
	public function del() {
