...
<div class="pad-lr-10">
 ...
 <div class="comment_button"><a href="?app=comment&controller=comment_admin&view=lists&show_center_id=1&commentid=<?php echo $commentid?>&hot=0"<?php if (empty($hot)) {?> class="on"<?php }?>>最新</a> <a href="?app=comment&controller=comment_admin&view=lists&show_center_id=1&commentid=<?php echo $commentid?>&hot=1"<?php if ($hot==1) {?> class="on"<?php }?>>最热</a></div> 	
...
function check(id, type, commentid) {
	...
	$.get('?app=comment&controller=check&view=ajax_checks&id='+id+'&type='+type+'&commentid='+commentid+'&safe_edi='+safe_edi+'&'+Math.random(), function(data){if(data!=1){if(data==0){alert('<?php echo L('illegal_parameters')?>')}else{alert(data)}}else{$('#tbody_'+id).remove();}});
}
</script>
</body>
</html>
