...
</span><?php echo $data['username']?> (<?php echo $data['ip']?><?php echo $ip_area->get($data['ip']); ?>) <?php echo L('chez')?> <?php echo format::date($data['creat_at'], 1)?> <?php echo L('release')?> </h5>
    <div class="content">
...
function check(id, type, commentid) {
	...
	$.getJSON('?app=comment&controller=check&view=public_get_one'+'&safe_edi='+safe_edi+Math.random(), function(data){
		if (data) {
