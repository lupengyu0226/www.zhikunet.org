...
class index {
	...
	public function init() {
		...
		$siteid =& $this->siteid;
		$username = param::get_cookie('_username',L('shuyang_friends'));
	...
	public function post() {
		...
		$comment = shy_base::load_app_class('comment');
		$id = isset($_GET['id']) && intval($_GET['id']) ? intval($_GET['id']) : '';
		$SITE = siteinfo($this->siteid);
		$username = param::get_cookie('_username',$ip_area->get(ip()).L('shuyang_friends'));
		$userid = param::get_cookie('_userid');
		$comment_setting_db = shy_base::load_model('comment_setting_model');
		$setting = $comment_setting_db->get_one(array('siteid'=>$this->siteid));
		if (!empty($setting)) {
		...
		preg_match('/(http(s)?):\/\/.+/i',strip_tags($content),$m);
	...
	public function ajax() {
		...
		$shy_tag = shy_base::load_app_class('comment_tag');
		$comment = array();
		if ($comment = $shy_tag->get_comment(array('commentid'=>$commentid))) {
			$page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
			$offset = ($page-1)*$num;
			$data = array('commentid'=>$commentid, 'site'=>$siteid, 'limit'=>$offset.','.$num, 'direction'=>$direction);
			$comment['data'] = $shy_tag->lists($data);
			shy_base::load_sys_class('format', '', 0);
