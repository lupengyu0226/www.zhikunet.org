...
if (!module_exists('tanmu')) showmessage(L('module_not_exists'));
class tanmu_api {
	private $db;
	...
	function get_info($module, $contentid, $siteid) {
		...
		if ($r) {
			return array('title'=>$r['title'], 'url'=>go($catid, $contentid, 1), 'allow_tanmu'=>(isset($data_info['allow_tanmu']) ? $data_info['allow_tanmu'] : 1));
		} else {
			return false;
		}
	}
