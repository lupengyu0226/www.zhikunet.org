...
class html {
			...
			if($CONTENT_POS !== false) {
				...
				if($END_POS !== false) {
					...
					if(preg_match_all("|\[page\](.*)\[/page\]|U", $content, $m, PREG_PATTERN_ORDER)) {
						$titles = array();
						$titles[0]['title'] = $title . '[1]';
						$titles[0]['url'] = $pageurls[1][0];
						foreach($m[1] as $k=>$v) {
							$p = $k+1;
	 ...
	 */
	 public function  kuaixun_js() {
		$kuaixun_db = shy_base::load_model('module_model');
		$kuaixun_data = $kuaixun_db->select(array('module'=>'kuaixun'));
		$kuaixun_setting = string2array($kuaixun_data[0]['setting']);
		if($this->siteid==1) {
