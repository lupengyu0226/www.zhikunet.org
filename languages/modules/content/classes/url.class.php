...
class url{
	...
	public function show($id, $page = 0, $catid = 0, $time = 0, $prefix = '',$data = '',$action = 'edit',$upgrade = 0) {
		...
		if($upgrade || (isset($_POST['upgrade']) && defined('IN_ADMIN') && $_POST['upgrade'])) {
			...
			$domain_dir = '';
			if($prefix!='') $category['url']=$prefix;
			
			if (preg_match('/^(http(s)?:)?\/\//i',$category['url']) && strpos($category['url'], '?')===false) {
				if (preg_match('/^((http(s)?:)?\/\/)([^\/]+)/i', $category['url'], $matches)) {
					$match_url = $matches[0];
					$url = $match_url.'/';
					if($prefix!='') {
						if($page==1) {
							$urlrule=str_replace($url,'',$prefix);
						}
						else{	
							$lasttext = strrchr($prefix,'.');
							$len = -strlen($lasttext);
							$path = substr($prefix,0,$len);
						//$urlrule=str_replace($url,'',$prefix);
						$urlrule=str_replace($url,'',$path.'_'.$page.$lasttext);
						}
					}
				}
				$db = shy_base::load_model('category_model');
			...
			$urls = str_replace(array('{$categorydir}','{$catdir}','{$year}','{$month}','{$day}','{$catid}','{$typeid}','{$mid}','{$id}','{$page}'),array($categorydir,$catdir,$year,$month,$day,$catid,$typeid,strtoupper(substr(md5($catid.'_'.$id),8,16)),$id,$page),$urlrule);//$id
			$create_to_html_root = $category['create_to_html_root'];
			
			if($create_to_html_root || $category['sethtml']||$prefix!='') {
				$html_root = '';
			} else {
	...
	public function category_url($catid, $page = 1) {
		...
		if (!$setting['ishtml']) { //如果不生成静态
			...
			foreach ($parentids as $pid) { //循环查询父栏目是否设置了二级域名
				...
				if (preg_match('/^(http(s)?:)?\/\//i',$r['url']) && strpos($r['url'], '?')===false) {
					$r['url'] = preg_replace('/^([(http(s)?:)?\/\/]{0,})([^\/]*)([\/]{1,})/i', '$1$2/', $r['url'],-1); //取消掉双'/'情况
		...
		if (preg_match('/^(http(s)?:)?\/\//i', $url) && IsDomain(parse_url($url)['host'])===true){//绑定了域名
