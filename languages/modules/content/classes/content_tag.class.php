...
class content_tag {
	...
	public function __construct() {
		...
		$this->position = shy_base::load_model('position_data_model');
	}
	/**
	 * 初始化模型
	 * @param $catid
	...
	public function count($data) {
		...
		}elseif($data['action'] == 'stats'){
			$sql=" status=99 ";
			if(isset($data['username']))  $sql.=" AND username='".$data['username']."' ";
			return $this->stats_db->count($sql);
	 ...
	 */
	 public function stats($data) {
		...
		$order = $data['order'];
		$sql=" status=99 ";
		$image = (empty($data['image']) || intval($data['image']) == 0) ? 0 : 1;
		if($image) $sql .= " AND `image` != ''";
		if(isset($data['username']))  $sql.=" AND username='".$data['username']."' ";
		$return = $this->stats_db->select($sql, '`id`', $data['limit'], $order);
		$ids = array();
		foreach ($return as $v) {
		...
		if (!empty($ids)) {
			unset($return);
			$ids = implode('\',\'', $ids);
			$return= $this->stats_db->select("`id` IN ('$ids')",'*','', $order,'', 'id');
			foreach ($return as $k=>$v) {
				if (isset($return[$k]['data'])){
					$return[$k]['data'] = string2array($return[$k]['data']);
				}
			}
		}
		if($data['id']) unset($return[$data['id']]);
		return $return;
	}
	/**
	 * 列表页标签
	...
	public function lists($data) {
		...
		$order = $data['order'];
		$return = $this->db->select($sql, '`id`', $data['limit'], $order);
		$ids = array();		
		foreach ($return as $v) {
			if (isset($v['id']) && !empty($v['id'])) {
				$ids[] = $v['id'];
		...
		if (!empty($ids)) {
			unset($return);
			$ids = implode('\',\'', $ids);
			$return= $this->db->select("`id` IN ('$ids')",'*','', $order,'', 'id');		
		}	
		//调用副表的数据
	...
	public function relation($data) {
		...
		if($data['relation']) {
			...
			if($setting['sphinxenable']) {//不适用like语句
				$sphinx = shy_base::load_app_class('search_interface', 'search',1);
				$sphinx = new search_interface();
				$keywords=str_replace(' ','|',$keywords);
				$res = $sphinx->search($keywords, array($siteid), array(1), array(0,SYS_TIME),0, 10,'@id desc');
				if(!empty($res['matches'])) {
					...
					$sql2 = $sql." AND ".$where." ".(isset($data['id']) && intval($data['id']) ? " AND `id` != '".abs(intval($data['id']))."'" : '');
	 ...
	 * @param $data
	 */
	public function hits_weixin($data){
		 $this->modelid = 1;//这里就写成固定的，模型id去后台模型管理找
		 $this->hits_db = shy_base::load_model('hits_model');
		 $this->db->set_model($this->modelid);
		 $this->tablename = $this->db->table_name;
		 $sql = $desc = $ids = '';
	...
	public function category($data) {
		...
		foreach ($categorys as $catid=>$cat) {
			...
			if(!preg_match('/^(http(s)?:)?\/\//i', $cat['url'])) {
