...
class sitemodel extends admin {
	...
	public function init() {
		...
		$pages = $this->db->pages;
		$this->public_cache();
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=content&controller=sitemodel&view=add\', title:\''.L('add_model').'\', width:\'580\', height:\'420\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_model'));
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$cache_api = shy_base::load_app_class('cache_api','admin');
	...
	public function delete() {
		$this->sitemodel_field_db = shy_base::load_model('sitemodel_field_model');
		$modelid = intval($_GET['modelid']);
	...
	function import(){
		if(isset($_POST['dosubmit'])) {
			...
			if($is_exists) showmessage(L('operation_failure'),'?app=content&controller=sitemodel&view=init');
			...
			if($modelid){
				...
				$this->public_cache();
				showmessage(L('operation_success'),'?app=content&controller=sitemodel&view=init');
			}
		} else {
			shy_base::load_sys_class('form','',0);
			$show_validator = '';
			...
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=content&controller=sitemodel&view=add\', title:\''.L('add_model').'\', width:\'580\', height:\'400\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_model'));
	...
	public function cache_field($modelid = 0) {
		$this->field_db = shy_base::load_model('sitemodel_field_model');
		$field_array = array();
