...
<li<?php if ($_GET['order']==1 || !isset($_GET['order'])) {?> class="on"<?php }?>><a href="?app=content&controller=push&view=init&classname=position_api&action=position_list&order=1&modelid=<?php echo $_GET['modelid']?>&catid=<?php echo $_GET['catid']?>&id=<?php echo $_GET['id']?>"><?php echo L('push_to_position');?></a></li>
<li<?php if ($_GET['order']==2) {?> class="on"<?php }?>><a href="?app=content&controller=push&view=init&module=special&action=_push_special&order=2&modelid=<?php echo $_GET['modelid']?>&catid=<?php echo $_GET['catid']?>&id=<?php echo $_GET['id']?>"><?php echo L('push_to_special');?></a></li>
<li<?php if ($_GET['order']==3) {?> class="on"<?php }?>><a href="?app=content&controller=push&view=init&module=content&classname=push_api&action=category_list&order=3&tpl=push_to_category&modelid=<?php echo $_GET['modelid']?>&catid=<?php echo $_GET['catid']?>&id=<?php echo $_GET['id']?>"><?php echo L('push_to_category');?></a></li>
</ul>

<div class='content' style="height:auto;">
<form action="?app=content&controller=push&view=init&module=<?php echo $_GET['module']?>&action=<?php echo $_GET['action']?>" method="post" name="myform" id="myform"><input type="hidden" name="modelid" value="<?php echo $_GET['modelid']?>"><input type="hidden" name="catid" value="<?php echo $_GET['catid']?>">
<input type='hidden' name="id" value='<?php echo $_GET['id']?>'>
<input type="hidden" value="content" name="app">
<input type="hidden" value="content" name="controller">
<input type="hidden" value="public_relationlist" name="view">
...
function change_siteid(siteid) {
		$("#load_catgory").load("?app=content&controller=content&view=public_getsite_categorys&siteid="+siteid);
		$("#siteid").val(siteid);
}
//移除ID
