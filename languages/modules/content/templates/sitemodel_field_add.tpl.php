...
$(function(){
	...
	$("#field").formValidator({onshow:"<?php echo L('input').L('fieldname')?>",onfocus:"<?php echo L('fieldname').L('between_1_to_20')?>"}).regexValidator({regexp:"^[a-zA-Z]{1}([a-zA-Z0-9]|[_]){0,19}$",onerror:"<?php echo L('fieldname_was_wrong');?>"}).inputValidator({min:1,max:20,onerror:"<?php echo L('fieldname').L('between_1_to_20')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data : "app=content&controller=sitemodel_field&view=public_checkfield&modelid=<?php echo $modelid?>",
		datatype : "html",
		cached:false,
...
　<a href="?app=content&controller=sitemodel_field&view=init&modelid=<?php echo $modelid?>&menuid=<?php echo $_GET['menuid']?>"><em><?php echo L('manage_field');?></em></a><span>|</span></div>
  <div class="bk10"></div>
</div>
<form name="myform" id="myform" action="?app=content&controller=sitemodel_field&view=add" method="post">
...
<script type="text/javascript">
	...
	$.each( ['<?php echo implode("','",$att_css_js);?>'], function(i, n){
        ...
        if(fieldtype!="") {
            $.getJSON("?app=content&controller=sitemodel_field&view=public_field_setting&fieldtype="+fieldtype, function(data){
