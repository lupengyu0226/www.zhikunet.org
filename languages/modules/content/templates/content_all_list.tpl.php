...
<input type="hidden" value="content_all" name="controller">
...
<div class="table-list">
    <table width="100%">
        <thead>
            ...
            <th width="15%"><?php echo L('keywords');?></th>
...
<tbody>
    ...
    ?>
        ...
        <td><a href="<?php echo S($r['url']);?>" target="_blank" title="<?php echo $r['title'];?>"><i class="icon-on icon-mobile"><em class="home"></em></i></a>&nbsp;&nbsp;<a href="javascript:;" onclick="javascript:openwinx('?app=content&controller=content&view=edit&catid=<?php echo $r['catid'];?>&id=<?php echo $r['id']?>','')"><?php echo $r['title'];?></a><?php if($r['thumb']!='') {echo ' <i class="iconfont icon-img" style="color:green"></i>'; } if($r['posids']) {echo ' <i class="iconfont icon-lliconrecommend" style="color:red"></i>';} if($r['islink']) {echo ' <i class="iconfont icon-wailian"></i>';}?></td>
		...
		}?>访问</a> <a class="xbtn btn-inverse btn-xs" href="javascript:view_comment('<?php echo id_encode('content_'.$r['catid'],$r['id'],$this->siteid);?>','<?php echo safe_replace($r['title']);?>')"><?php echo L('comment');?></a> <?php 
		if (is_today($r['inputtime'])==1){
			echo '<a class="xbtn btn-warning btn-xs" href="?app=content&controller=content&view=pushbaiduzl&safe_edi='.$safe_edi.'&bdurls=https://wap.zhikunet.org/mip-'.$r['catid'].'-'.$r['id'].'-1.html">增量推送</a>';
			} else{ 
			echo '<a class="xbtn btn-white btn-xs" href="?app=content&controller=content&view=pushbaiducl&safe_edi='.$safe_edi.'&bdurls=https://wap.zhikunet.org/mip-'.$r['catid'].'-'.$r['id'].'-1.html">存量推送</a>';
   }?></td>
    </tr>
     <?php }
    }
    ...
    <div class="btn"><label for="check_box"><?php echo L('selected_all');?>/<?php echo L('cancel');?></label>
        <input type="hidden" value="<?php echo $safe_edi;?>" name="safe_edi">
        <input type="button" class="button" value="<?php echo L('delete');?>" onclick="myform.action='?app=content&controller=content&view=delete&dosubmit=1&catid=<?php echo $r['catid'];?>&steps=<?php echo $steps;?>';return confirm_delete()"/>
