...
<a class="add fb" href="javascript:;" onclick=javascript:openwinx('?app=content&controller=content&view=add&menuid=&catid=<?php echo $catid;?>&safe_edi=<?php echo $_SESSION['safe_edi'];?>','')><em><?php echo L('add_content');?></em></a>　
...
<tbody>
	...
	?>
		...
		<td align='center'><a href="javascript:;" onclick="javascript:openwinx('?app=content&controller=content&view=edit&catid=<?php echo $catid;?>&id=<?php echo $r['id']?>','')"><?php echo L('edit');?></a> | <a href="javascript:view_comment('<?php echo id_encode('content_'.$catid,$r['id'],$this->siteid);?>','<?php echo safe_replace($r['title']);?>')"><?php echo L('comment');?></a></td>
	</tr>
     <?php }
	}
	?>
...
function update_param(fid, obj) {
	...
	$.get('index.php', {app:'content', controller:'content', view:'update_param', field:field, id:id, value:value, modelid:modelid, safe_edi:'<?php echo $_SESSION['safe_edi'];?>'}, function (data){
		if (data!=200){
            $(obj).val(old_data);
		}
	})
