...
class sitemodel_field extends admin {
	...
	function __construct() {
		...
		$this->model_db = shy_base::load_model('sitemodel_model');
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->cache_field($modelid);
			showmessage(L('add_success'),'?app=content&controller=sitemodel_field&view=init&modelid='.$modelid.'&menuid=59');
		} else {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if ($field_type == 'box' && !empty($setting)) {
				...
				if($setting['is_sql'] == 1){
					$options = explode("\n",$setting['options']);
					$sql = "SELECT ".$options[1]." FROM ".$options[0];
					if (!empty($options[2])) $sql .= " WHERE ".$options[2];
					if (!empty($options[3])) $sql .= " ORDER BY ".$options[3];
					if (!empty($options[4])) $sql .= " LIMIT ".$options[4];
			...
			$this->cache_field($modelid);
			showmessage(L('update_success'),'?app=content&controller=sitemodel_field&view=init&modelid='.$modelid.'&menuid=59');
		} else {
			$show_header = $show_validator = $show_dialog = '';
			shy_base::load_sys_class('form','',0);
			require MODEL_PATH.'fields.inc.php'; 
