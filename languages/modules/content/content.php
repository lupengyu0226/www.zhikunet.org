...
class content extends admin {
	...
	public function init() {
		...
		if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
         ...
         $show_header = $show_dialog  = $show_safe_edi = '';
			...
			if ($modelid=="") {include $this->admin_tpl('sitemodel_manage2');exit();}
			$admin_username = param::get_cookie('admin_username');
			//查询当前的工作流
			$setting = string2array($category['setting']);
			$workflowid = $setting['workflowid'];
	...
	public function delete() {
		if(isset($_GET['dosubmit'])) {
			...
			$this->hot_db = shy_base::load_model('hot_content_model');
			...
			$this->position_data_db = shy_base::load_model('position_data_model');
			$this->search_db = shy_base::load_model('search_model');
			$this->stats_db = shy_base::load_model('stats_model');
			$this->comment = shy_base::load_app_class('comment', 'comment');
			...
			foreach($_POST['ids'] as $id) {
				$r = $this->db->get_one(array('id'=>$id));
				file_put_contents(SHUYANG_PATH.'silian.txt',$r['url']."\n",FILE_APPEND);//保存删除的死链
				if($content_ishtml && !$r['islink']) {
					$urls = $this->url->show($id, 0, $r['catid'], $r['inputtime'],$r['url']);
					$fileurl = $urls[1];
					if($this->siteid != 1) {
					...
					$path = ltrim($path,'/');
					$filelist = glob(SHUYANG_PATH.$path.'{_,-,.}*',GLOB_BRACE);
					foreach ((array)$filelist as $delfile) {
						$lasttext = strrchr($delfile,'.');
						if(!in_array($lasttext, array('.htm','.html','.shtml'))) continue;
						@unlink($delfile);
						//删除发布点队列数据
						$delfile = str_replace(SHUYANG_PATH, '/', $delfile);
				...
				//删除稿件统计中的数据
				$this->stats_db->delete(array('hitsid'=>'c-'.$modelid.'-'.$id,'catid'=>$catid,'aid'=>$id,'siteid'=>$siteid));
				//删除推荐位数据
				$this->position_data_db->delete(array('id'=>$id,'catid'=>$catid,'module'=>'content'));
				//删除关键词库ITAG里数据
				$this->hot_db->delete(array('contentid'=>$id, 'modelid'=>$modelid));
				//删除全站搜索中数据
	...
	public function pass() {
		...
		if($workflowid) {
			...
			//更改内容状态
				...
				$modelid = $this->categorys[$catid]['modelid'];
				$this->db->set_model($modelid);
				$this->db->search_db = shy_base::load_model('search_model');
				//审核通过，检查投稿奖励或扣除积分
				if ($status==99) {
	...
	public function public_categorys() {
		...
		if(!empty($this->categorys)) {
			foreach($this->categorys as $r) {
				...
				if($r['type']==1 || $from=='block') {
					if($r['type']==0) {
						$r['vs_show'] = "<a href='?app=block&controller=block_admin&view=public_visualization&menuid=".$_GET['menuid']."&catid=".$r['catid']."&type=show' target='right'>[".L('content_page')."]</a>";
					} else {
						$r['vs_show'] ='';
					}
					$r['icon_type'] = '<i class="iconfont icon-txt"></i> ';
					$r['add_icon'] = '';
					$r['type'] = 'add';
				} else {
					$r['icon_type'] = $r['vs_show'] = '';
					$r['type'] = 'init';
					$r['add_icon'] = "<a target='right' href='?app=content&controller=content&menuid=".$_GET['menuid']."&catid=".$r['catid']."' onclick=javascript:openwinx('?app=content&controller=content&view=add&menuid=".$_GET['menuid']."&catid=".$r['catid']."&hash_page=".$_SESSION['hash_page']."','')><i class='iconfont icon-tianjia3' title='".L('add')."'></i> </a> ";
				}
				$categorys[$r['catid']] = $r;
			}
		}
		if(!empty($categorys)) {
			$tree->init($categorys);
				switch($from) {
					case 'block':
						$strs = "<span>\$icon_type \$add_icon<a href='?app=block&controller=block_admin&view=public_visualization&menuid=".$_GET['menuid']."&catid=\$catid&type=list' target='right'>\$catname</a> \$vs_show</span>";
						$strs2 = "<i class='iconfont icon-wenjianjia'></i> <a href='?app=block&controller=block_admin&view=public_visualization&menuid=".$_GET['menuid']."&catid=\$catid&type=category' target='right'>\$catname</a>";
					break;

					default:
						$strs = "<span>\$icon_type \$add_icon<a href='?app=content&controller=content&view=\$type&menuid=".$_GET['menuid']."&catid=\$catid' target='right' onclick='open_list(this)'>\$catname</a></span>";
						$strs2 = "<i class='iconfont icon-wenjianjia'></i> \$catname";
						break;
				}
	...
	public function public_preview() {
		...
		if($rs['paginationtype']!=0) {
			...
			if($CONTENT_POS !== false) {
				...
				for($i=1; $i<=$pagenumber; $i++) {
					$pageurls[$i][0] = 'index.php?app=content&controller=content&view=public_preview&steps='.intval($_GET['steps']).'&catid='.$catid.'&id='.$id.'&page='.$i;
				}
		...
		function ajax_manage(type) {
			if(type==1) {
				$.get('?app=content&controller=content&view=pass&ajax_preview=1&catid=".$catid."&steps=".$steps."&id=".$id."&safe_edi=".$safe_edi."');
			} else if(type==2) {
				$.get('?app=content&controller=content&view=pass&ajax_preview=1&reject=1&catid=".$catid."&steps=".$steps."&id=".$id."&safe_edi=".$safe_edi."');
			} else if(type==3) {
				$.get('?app=content&controller=content&view=delete&ajax_preview=1&dosubmit=1&catid=".$catid."&steps=".$steps."&id=".$id."&safe_edi=".$safe_edi."');
			}
	...
	public function public_checkall() {
		...
		$edi_pages = $this->content_check_db->edi_pages;
	 ...
	 * 熊掌号增量推送
	 */
	public function pushbaiduzl() {
		$bdurls[] = $_GET['bdurls'];
		$url = $_GET['bdurls'];
		$bdurls = guanfanghao_zengliang($bdurls); 
		$bdurls = string2array($bdurls);
		if($bdurls['success_realtime']==0) {
			showmessage('推送失败，今日剩余:'.$bdurls['remain_realtime'],HTTP_REFERER);
		} else {
			$bdurls['success_realtime'] = str_replace("1",'增量数据推送成功',$bdurls['success_realtime']);
			$this->mip_db = shy_base::load_model('mip_model');
			$this->mip_db->update(array('ispush'=>'1'),array('url'=>$url));
			showmessage($bdurls['success_realtime'].','.'今日剩余:'.$bdurls['remain_realtime'],HTTP_REFERER);
		}
	}
	/**
	 * 熊掌号存量推送
	 */
	public function pushbaiducl() {
		$bdurls[] = $_GET['bdurls'];
		$url = $_GET['bdurls'];
		$bdurls = guanfanghao_cunliang($bdurls); 
		$bdurls = string2array($bdurls);
		if($bdurls['success_batch']==0) {
			showmessage('推送失败，今日剩余:'.$bdurls['remain_batch'],HTTP_REFERER);
		} else {
			$bdurls['success_batch'] = str_replace("1",'存量数据推送成功',$bdurls['success_batch']);
			$this->mip_db = shy_base::load_model('mip_model');
			$this->mip_db->update(array('ispush'=>'1'),array('url'=>$url));
			showmessage($bdurls['success_batch'].','.'今日剩余:'.$bdurls['remain_batch'],HTTP_REFERER);
		}
	}
	/**
	 * 原创推送
	 */
	public function pushyuanchuang() {
		$bdurls[] = $_GET['bdurls'];
		$url = $_GET['bdurls'];
		$bdurls = push_yuanchuang($bdurls); 
		$bdurls = string2array($bdurls);
		if($bdurls['success_original']==0) {
			showmessage('推送失败，今日剩余:'.$bdurls['remain_original'],HTTP_REFERER);
		} else {
			$bdurls['success_original'] = str_replace("1",'数据推送成功',$bdurls['success_original']);
			showmessage($bdurls['success_original'].','.'今日剩余:'.$bdurls['remain_original'],HTTP_REFERER);
		}
	}
	public function public_sub_categorys() {
		...
		if(!empty($this->categorys)) {
			foreach($this->categorys as $r) {
				...
				if($r['type']==1 || $from=='block') {
					...
					$r['icon_type'] = $r['vs_show'] = '';
					$r['type'] = 'init';
					$r['add_icon'] = "<a target='right' href='?app=content&controller=content&menuid=".$_GET['menuid']."&catid=".$r['catid']."' onclick=javascript:openwinx('?app=content&controller=content&view=add&menuid=".$_GET['menuid']."&catid=".$r['catid']."&hash_page=".$_SESSION['hash_page']."','')><i class='iconfont icon-txt'></i></a> ";
				}
				$categorys[$r['catid']] = $r;
			}
		}
		if(!empty($categorys)) {
			$tree->init($categorys);
				switch($from) {
					case 'block':
						$strs = "<span class='\$icon_type'>\$add_icon<a href='?app=block&controller=block_admin&view=public_visualization&menuid=".$_GET['menuid']."&catid=\$catid&type=list&safe_edi=".$_SESSION['safe_edi']."' target='right'>\$catname</a> \$vs_show</span>";
					break;

					default:
						$strs = "<span class='\$icon_type'>\$add_icon<a href='?app=content&controller=content&view=\$type&menuid=".$_GET['menuid']."&catid=\$catid&safe_edi=".$_SESSION['safe_edi']."' target='right' onclick='open_list(this)'>\$catname</a></span>";
						break;
				}
