...
class create_html extends admin {
	...
	public function update_urls() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->url = shy_base::load_app_class('url');
			...
			if($modelid) {
				...
				if(!isset($set_catid)) {
					...
					$forward = "?app=content&controller=create_html&view=update_urls&set_catid=1&pagesize=$pagesize&dosubmit=1";
					showmessage($message,$forward,200);
				}
				$catid_arr = getcache('update_url_catid'.'-'.$this->siteid.'-'.$_SESSION['userid'],'content');
				$autoid = $autoid ? intval($autoid) : 0;
				if(!isset($catid_arr[$autoid])) showmessage(L('create_update_success'),'?app=content&controller=create_html&view=update_urls',200);
				...
				if($pages > $page) {
					...
					$forward = "?app=content&controller=create_html&view=update_urls&set_catid=1&pagesize=$pagesize&dosubmit=1&autoid=$autoid";
				}
				showmessage($message,$forward,200);
			}
	...
	private function urls($id, $catid= 0, $inputtime = 0, $prefix = ''){
		...
		$this->stats_db->_add(array('data'=>array('description'=>$r['description'],'thumb'=>$r['thumb']),'aid'=>$r['id'],'siteid'=>$this->siteid,'catid'=>$r['catid'],'url'=>$url,'title'=>$r['title'],'hitsid'=>'c-'.$modelid.'-'.$r['id'],'username'=>$r['username'],'inputtime'=>$r['inputtime']));
		return $urls;
	}
	/**
	* 生成内容页
	*/
	public function show() {
		if(isset($_POST['dosubmit'])) {
			extract($_POST,EXTR_SKIP);
			$this->html = shy_base::load_app_class('html');
			...
			if($modelid) {
				...
				if(!isset($first) && is_array($catids) && $catids[0] > 0)  {
					...
					$where .= " AND catid IN($catids) ";
					$first = 1;
				} elseif(is_array($catids)&&count($catids)==1 && $catids[0] == 0) {
					$catids = array();
					foreach($this->categorys as $catid=>$cat) {
						if($cat['child'] || $cat['siteid'] != $this->siteid || $cat['type']!=0) continue;
				...
				if(is_array($catids)&&count($catids)==1 && $catids[0]==0) {
					$message = L('create_update_success');
					$forward = '?app=content&controller=create_html&view=show';
				...
				if($pages > $page) {
					...
					$message = L('need_update_items',array('total'=>$total,'creatednum'=>$creatednum,'percent'=>$percent));
					$forward = $start ? "?app=content&controller=create_html&view=show&type=$type&dosubmit=1&first=$first&fromid=$fromid&toid=$toid&fromdate=$fromdate&todate=$todate&pagesize=$pagesize&page=$page&pages=$pages&total=$total&modelid=$modelid" : preg_replace("/&page=([0-9]+)&pages=([0-9]+)&total=([0-9]+)/", "&page=$page&pages=$pages&total=$total", $http_url);
				} else {
				...
				if(!isset($set_catid)) {
					...
					$forward = "?app=content&controller=create_html&view=show&set_catid=1&pagesize=$pagesize&dosubmit=1";
					showmessage($message,$forward,200);
				}
				if(is_array($catids)&&count($catids)==1 && $catids[0]==0) {
					$message = L('create_update_success');
				...
				$autoid = $autoid ? intval($autoid) : 0;
				if(!isset($catid_arr[$autoid])) showmessage(L('create_update_success'),'?app=content&controller=create_html&view=show',200);
				$catid = $catid_arr[$autoid];
				...
				if($pages > $page) {
					...
					$percent = round($creatednum/$total, 2)*100;
					$message = '【'.$this->categorys[$catid]['catname'].'】 '.L('have_update_items',array('total'=>$total,'creatednum'=>$creatednum,'percent'=>$percent));
					$forward = $start ? "?app=content&controller=create_html&view=show&type=$type&dosubmit=1&first=$first&fromid=$fromid&toid=$toid&fromdate=$fromdate&todate=$todate&pagesize=$pagesize&page=$page&pages=$pages&total=$total&autoid=$autoid&set_catid=1" : preg_replace("/&page=([0-9]+)&pages=([0-9]+)&total=([0-9]+)/", "&page=$page&pages=$pages&total=$total", $http_url);
				} else {
					$autoid++;
					$message = L('start_update').$this->categorys[$catid]['catname']." ...";
					$forward = "?app=content&controller=create_html&view=show&set_catid=1&pagesize=$pagesize&dosubmit=1&autoid=$autoid";
				}
				showmessage($message,$forward,200);
			}
	...
	*/
	public function category() {
		if(isset($_POST['dosubmit'])) {
			...
			if(!isset($set_catid)) {
				...
				$forward = "?app=content&controller=create_html&view=category&set_catid=1&pagesize=$pagesize&dosubmit=1&modelid=$modelid&referer=$referer";
			...
			if(!isset($catid_arr[$autoid])) {
				if(!empty($referer) && $this->categorys[$catid_arr[0]]['type']!=1) {
					showmessage(L('create_update_success'),'?app=content&controller=content&view=init&catid='.$catid_arr[0],200);
				} else {
					showmessage(L('create_update_success'),'?app=content&controller=create_html&view=category',200);
				}
			}
			$catid = $catid_arr[$autoid];
			$page = $page ? $page : 1;
			...
			if($page <= $total_number) {
				...
				$forward = "?app=content&controller=create_html&view=category&set_catid=1&pagesize=$pagesize&dosubmit=1&autoid=$autoid&page=$page&total_number=$total_number&modelid=$modelid&referer=$referer";
			} else {
				$autoid++;
				$message = $this->categorys[$catid]['catname'].L('create_update_success');
				$forward = "?app=content&controller=create_html&view=category&set_catid=1&pagesize=$pagesize&dosubmit=1&autoid=$autoid&modelid=$modelid&referer=$referer";
			}
			showmessage($message,$forward,200);
		} else {
			$show_header = $show_dialog  = '';
	...
	//生成最新滚动
	public function public_gundong() {
		$this->html = shy_base::load_app_class('html');
		$size = $this->html->gundongindex();
		showmessage(L('gundong_create_finish',array('size'=>sizecount($size))));
	}
	/**
	* 批量生成内容页
	*/
