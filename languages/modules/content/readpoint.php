...
class readpoint {
	...
	function __construct() {
		...
		if(!$this->userid) {
			header("Location: index.php?app=member&controller=index&view=login&forward=".urlencode(get_url()));
			exit;
		}
	...
	public function init() {
		...
		if($paytype) {
			if(spend::amount($readpoint, L('msg_readpoint'), $this->userid, $this->username, '', '', $flag)==false) {
				$msg = spend::get_msg();
				$http_referer = APP_PATH.'index.php?app=pay&controller=deposit&view=pay';
			} else {
