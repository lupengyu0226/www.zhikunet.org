...
class fanghu_tag {
	...
	public function shy_tag() {
		...
		if(is_array($result)) {
			$specials = array(L('please_select', '', 'fanghu'));
			foreach($result as $r) {
		...
		return array(
			'action'=>array('lists'=>L('lists', '', 'fanghu')),
			'lists'=>array(
				'siteid'=>array('name'=>L('sitename', '', 'fanghu'),'htmltype'=>'input_select', 'defaultvalue'=>get_siteid(), 'data'=>$sitelist),
			),
		);
	}
}
