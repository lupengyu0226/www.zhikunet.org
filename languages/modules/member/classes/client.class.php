...
class client {
	...
	public function ps_getavatar_upload_html($uid) {
		...
		$upurl = base64_encode($this->ps_api_url.'/index.php?app=phpsso&controller=index&view=uploadavatar&auth_data='.$auth_data);
		$str = <<<EOF
...
EOF;
	...
	*/
	...
	private function _ps_send($action, $data = null) {
 		return $this->_ps_post($this->ps_api_url."/index.php?app=phpsso&controller=index&view=".$action, 500000, $this->auth_data($data));
	}
