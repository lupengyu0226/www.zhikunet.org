...
class baoliaoclass {
	...
	final public function check_member() {
		...
		if(ROUTE_M =='member' && ROUTE_C =='index' && in_array(ROUTE_A, array('login', 'register', 'mini','top_mini','blog_mini','send_newmail'))) {
			...
			//判断是否存在auth cookie
			if ($shuyang_auth) {
				$auth_key = $auth_key = md5(shy_base::load_config('system', 'auth_key').$_SERVER['HTTP_USER_AGENT']);
