...
class member_group extends admin {
	...
	function __construct() {
		parent::__construct();
		$this->db = shy_base::load_model('member_group_model');
	}
	...
	function manage() {
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=member&controller=member_group&view=add\', title:\''.L('member_group_add').'\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('member_group_add'));
	...
	function add() {
		if(isset($_POST['dosubmit'])) {
			...
			if($this->db->insert_id()){
				$this->_updatecache();
				showmessage(L('operation_success'),'?app=member&controller=member_group&view=manage', '', 'add');
			}
		} else {
	...
	function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->_updatecache();
			showmessage(L('operation_success'), '?app=member&controller=member_group&view=manage', '', 'edit');
		} else {					
			$show_header = $show_scroll = true;
