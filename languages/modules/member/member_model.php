...
//模型原型存储路径
define('MODEL_PATH',SHY_PATH.'modules'.DIRECTORY_SEPARATOR.'member'.DIRECTORY_SEPARATOR.'fields'.DIRECTORY_SEPARATOR);
//模型缓存路径
...
class member_model extends admin {
	...
	function manage() {
		...
		$pages = $this->db->pages;
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=member&controller=member_model&view=add\', title:\''.L('add_model').'\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('member_model_add'));
	...
	function add() {
		if(isset($_POST['dosubmit'])) {
			...
			if($is_exists) showmessage(L('operation_failure'),'?app=member&controller=member_model&view=manage', '', 'add');
			...
			if($modelid) {
				...
				//更新模型缓存
				shy_base::load_app_class('member_cache','','');
				member_cache::update_cache_model();
				showmessage(L('operation_success'),'?app=member&controller=member_model&view=manage', '', 'add');
			} else {
				showmessage(L('operation_failue'),'?app=member&controller=member_model&view=manage', '', 'add');
			}
		} else {
	...
	function edit() {
		if(isset($_POST['dosubmit'])) {
			$modelid = isset($_POST['info']['modelid']) ? $_POST['info']['modelid'] :showmessage(L('operation_success'),'?app=member&controller=member_model&view=manage', '', 'edit');
			$info['name'] = $_POST['info']['modelname'];
			...
			shy_base::load_app_class('member_cache','','');
			member_cache::update_cache_model();
			showmessage(L('operation_success'),'?app=member&controller=member_model&view=manage', '', 'edit');
		} else {					
			$show_header = $show_scroll = true;
	...
	function delete() {
		...
		if ($this->db->delete($where)) {
			...
			//修改用户模型组为普通会员
			$this->member_db = shy_base::load_model('members_model');
