...
class member extends admin {
	...
	function init() {
        ...
        $memberinfo['online'] = $this->session_db->count("ip");
		...
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?app=member&controller=member&view=add\', title:\''.L('member_add').'\', width:\'700\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('member_add'));
	...
	function search() {
		...
		$pages = $this->db->pages;
		$big_menu = array('?app=member&controller=member&view=manage&menuid=72', L('member_research'));
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($status > 0) {
				...
				if($this->db->insert_id()){
					showmessage(L('operation_success'),'?app=member&controller=member&view=add', '', 'add');
				}
			} elseif($status == -4) {
	...
	function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			$basicinfo['yuanyin'] = $_POST['info']['yuanyin'];
			...
			$status = $this->client->ps_member_edit($info['username'], $info['email'], '', $info['password'], $userinfo['phpssouid'], $userinfo['encrypt']);
  			if($status >= 0) { 
				...
				showmessage(L('operation_success'), '?app=member&controller=member&view=manage', '', 'edit');
			} else {
				showmessage(L('operation_failure'), HTTP_REFERER);
			}
		} else {
