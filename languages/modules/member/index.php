...
class index extends foreground {
	...
	public function init() {
		...
		$this->message_group_db = shy_base::load_model('message_group_model');
		$this->message_data_db = shy_base::load_model('message_data_model');
		$infos = $this->message_group_db->listinfo($where,$order = 'id DESC',$page, $pages = '8');
		$infos = new_html_special_chars($infos);
		$status = array();
		if (is_array($infos) && !empty($infos)) {
			foreach ($infos as $info){
				$d = $this->message_data_db->select(array('userid'=>$memberinfo['userid'],'group_message_id'=>$info['id']));
	 			if(!$d){
        ...
        $day_where = " `userid`='{$memberinfo['userid']}' AND `signtime` > '{$day_start_time}' AND `signtime` < '{$day_end_time}'";
	...
	public function register() {
		...
		if(!$member_setting['allowregister']) {
			showmessage(L('deny_register'), 'member-login.html');
		}
		//加载短信模块配置
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($userinfo['mobile']!=""){
				if(!preg_match('/^1([0-9]{9,10})$/',$userinfo['mobile'])) {
			...
			if($member_setting['enablemailcheck']) {	//是否需要邮件验证
				...
				showmessage(L('operation_success'), 'member-register.html?t=3');
			} else {
				//查看当前模型是否开启了短信验证功能
			...
			if(shy_base::load_config('system', 'phpsso')) {
				...
				if($status > 0) {
					...
					if($member_setting['enablemailcheck']) {
						...
						$url = PASSPORT_PATH."member-register.html?code=$code&verify=1";
						...
						showmessage(L('operation_success'), 'member-register.html?t=2');
					} else {
						//如果不需要邮箱认证、直接登录其他应用
						$synloginstr = $this->client->ps_member_synlogin($userinfo['phpssouid']);
						showmessage(L('operation_success').$synloginstr, 'member-index.html');
					}
					
				}
			} else {
				showmessage(L('enable_register').L('enable_phpsso'), 'member-login.html');
			}
			...
			if(!empty($_GET['verify'])) {
				$code = isset($_GET['code']) ? trim($_GET['code']) : showmessage(L('operation_failure'), 'member-login.html');
				$code_res = sys_auth($code, 'DECODE', get_auth_key('email'));
				$code_arr = explode('|', $code_res);
				$userid = isset($code_arr[0]) ? $code_arr[0] : '';
				$userid = is_numeric($userid) ? $userid : showmessage(L('operation_failure'), 'member-login.html');

				$this->db->update(array('groupid'=>$this->_get_usergroup_bypoint()), array('userid'=>$userid));
				showmessage(L('operation_success'), 'member-index.html');
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($mobile){
				...
				if($r && $r['id_code']==$mobile_verify) {
					...
					showmessage("手机号码更新成功！",'?app=member&controller=index&view=account_change_mobile&t=1');
				} else {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(shy_base::load_config('system', 'phpsso')) {
				...
				if(isset($memberinfo['uid'])) {
					...
					if($status == -1) {	//用户不存在
						showmessage(L('user_not_exist'), 'member-login.html');
					} elseif($status == -2) { //密码错误
						showmessage(L('password_error'), 'member-login.html');
					} else {
						showmessage(L('login_failure'), 'member-login.html');
					}
				}
				...
				if(!$r) showmessage(L('user_not_exist'),'member-login.html');
				...
				if($r['password'] != $password) {				
					...
					showmessage(L('password_error', array('times'=>$times)), 'member-login.html', 3000);
				}
			...
			param::set_cookie('_nickname', $nickname, $cookietime);
			//param::set_cookie('cookietime', $_cookietime, $cookietime);
			$forward = isset($_POST['forward']) && !empty($_POST['forward']) ? urldecode($_POST['forward']) : 'member-index.html';
	...
	public function logout() {
		$setting = shy_base::load_config('system');
			...
			$forward = isset($_GET['forward']) && trim($_GET['forward']) ? $_GET['forward'] : 'member-login.html';
	...
	public function jubao() {
		...
		$memberinfo['starnum'] = $grouplist[$memberinfo['groupid']]['starnum'];
		$page = isset($_GET['page']) && trim($_GET['page']) ? intval($_GET['page']) : 1;
		$jubaolist = $this->jubao_db->listinfo(array('userid'=>$memberinfo['userid']), 'id DESC', $page, 10);
		$pages = $this->jubao_db->pages;
        include template('member', 'jubao_list');
	}
	...
	public function jubao_view() {
		...
		$id = $_GET['id'];
		$page = isset($_GET['page']) && trim($_GET['page']) ? intval($_GET['page']) : 1;
		$jubaolist = $this->jubao_db->listinfo(array('id'=>$id,'userid'=>$memberinfo['userid']), 'id DESC', $page, 10);
		$pages = $this->jubao_db->pages;
        include template('member', 'jubao_view');
        }
	}
	/**
	 * 我的好友
	...
	public function public_sina_login() {
		...
		define('WEB_CALLBACK', PASSPORT_PATH.'member-public_sina_login.html?callback=1');
		...
		if(isset($_GET['callback']) && trim($_GET['callback'])) {
			...
			if(!empty($me['id'])) {
				...
				if(!empty($r)) {
					...
					param::set_cookie('cookietime', $_cookietime, $cookietime);
					param::set_cookie('_nickname', $nickname, $cookietime);
					$forward = isset($_GET['forward']) && !empty($_GET['forward']) ? $_GET['forward'] : 'member-index.html';
	...
	public function public_qq_loginnew(){
                ...
                $callback = shy_base::load_config('system', 'qq_callback');
				//加载短信模块配置
				$siteid = isset($_REQUEST['siteid']) && trim($_REQUEST['siteid']) ? intval($_REQUEST['siteid']) : 1;
				$sms_setting_arr = getcache('sms','sms');
				$sms_setting = $sms_setting_arr[$siteid];		
					...
					if(!empty($openid)){
						$r = $this->db->get_one(array('connectid'=>$openid,'from'=>'qq'));
						 ...
						 if(!empty($r)){
								...
								if($r['overduedate'] < SYS_TIME) {
								$updatearr = array('vip'=>'0', 'groupid'=>'6');
								$this->db->update(array('vip'=>'0', 'groupid'=>'6'), array('userid'=>$userid));
								}		
								if(!$cookietime) $get_cookietime = param::get_cookie('cookietime');
								$_cookietime = $cookietime ? intval($cookietime) : ($get_cookietime ? $get_cookietime : 0);
								$cookietime = $_cookietime ? TIME + $_cookietime : 0;
	...
	*/
	public function public_xzh(){
                ...
                $callback = shy_base::load_config('system', 'xzh_callback');
	...
	public function public_forget_password () {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$url = PASSPORT_PATH."member-public_forget_password.html?code=$code";
			...
			if(isset($sitelist[$memberinfo['siteid']]['name'])) {
				...
				$sitename = 'SHUYANGWANGCMS__MAIL';
			}
			sendmail($email, L('forgetpassword'), $message, '', '', $sitename);
			showmessage(L('operation_success'), 'member-login.html');
		} elseif($_GET['code']) {
			...
			if(is_array($code) && is_numeric($code[0]) && date('y-m-d h', SYS_TIME) == date('y-m-d h', $code[1])) {
				...
				if(empty($memberinfo['phpssouid'])) {
					showmessage(L('operation_failure'), 'member-login.html');
				}
				$updateinfo = array();
		...
		//处理提交申请，以手机号为准
			...
			if($mobile){
				...
				if($r && $r['id_code']==$mobile_verify) {
					...
					$_SESSION['userid'] = '';
					$_SESSION['code'] = '';
					showmessage("密码已重置成功！请查收手机",'member-login.html');
				} else {
					showmessage("短信验证码错误！请重新获取！");
				}
			}else{
