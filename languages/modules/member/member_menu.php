...
class member_menu extends admin {
	function __construct() {
		parent::__construct();
		$this->db = shy_base::load_model('member_menu_model');
	}
	...
	function manage() {
		...
		foreach($result as $r) {
			...
			$r['str_manage'] = '<a class="xbtn btn-info btn-xs" href="?app=member&controller=member_menu&view=edit&id='.$r['id'].'&menuid='.$_GET['menuid'].'">'.L('edit').'</a>   <a class="xbtn btn-danger btn-xs" href="javascript:confirmurl(\'?app=member&controller=member_menu&view=delete&id='.$r['id'].'&menuid='.$_GET['menuid'].'\',\''.L('confirm',array('message'=>$r['cname'])).'\')">'.L('delete').'</a> ';
			$array[] = $r;
		}
	...
	function add() {
		if(isset($_POST['dosubmit'])) {
			...
			//开发过程中用于自动创建语言包
			$file = SHY_PATH.'languages'.DIRECTORY_SEPARATOR.'zh-cn'.DIRECTORY_SEPARATOR.'member_menu.lang.php';
			if(file_exists($file)) {
