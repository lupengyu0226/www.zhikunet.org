...
class content extends foreground {
	...
	public function publish() {
		...
		$zhuanlan_info = $this->zhuanlan_db->get_one(array('username'=>$this->memberinfo['username']));
		if($zhuanlan_info['status']==0||$zhuanlan_info['status']==2) {
			showmessage(L('您尚未开通沭阳号专栏').L(',禁止投稿'), PASSPORT_PATH.'mp-register.html');
		}
		//判断每日投稿数
		...
		if(isset($_POST['dosubmit'])) {
			...
			//缓存结果 END
			if($info['status']==99) {
				showmessage(L('contributors_success'), APP_PATH.'index.php?app=member&controller=content&view=published');
			} else {
				showmessage(L('contributors_checked'), APP_PATH.'index.php?app=member&controller=content&view=published');
			}
			...
			if (!$catid) showmessage(L('category').L('publish_deny'), APP_PATH.'index.php?app=member');

			//判断本栏目是否允许投稿
			if (!$priv_db->get_one(array('catid'=>$catid, 'roleid'=>$memberinfo['groupid'], 'is_admin'=>0, 'action'=>'add'))) showmessage(L('category').L('publish_deny'), APP_PATH.'index.php?app=member');
			$category = $CATEGORYS[$catid];
			if($category['siteid']!=$siteid) showmessage(L('site_no_category'),'?app=member&controller=content&view=publish');
			$setting = string2array($category['setting']);
	...
	public function published() {
		...
		foreach($infos as $_v) {
			...
			$_v['url'] = $_v['status']==99 ? go($_v['catid'],$_v['id']) : APP_PATH.'index.php?app=content&controller=index&view=show&catid='.$_v['catid'].'&id='.$_v['id'];
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($category['type']==0) {
				...
				foreach($_POST['info'] as $_k=>$_v) {
					...
					} elseif(($fields==null) || in_array($_k, $fields)){
	...
	public function delete(){
		...
		$check_pushed_db = shy_base::load_model('content_check_model');
		...
		if(!$array){
			...
			$table_name = $content_db->table_name;
 			$content_db->delete_content($id,$file); //删除文章
	...
	public function info_publish() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			//缓存结果 END
			if($info['status']==99) {
				showmessage(L('contributors_success'), APP_PATH.'index.php?app=member&controller=content&view=info_top&id='.$id.'&catid='.$catid.'&msg=1');
			} else {
				showmessage(L('contributors_checked'), APP_PATH.'index.php?app=member&controller=content&view=info_top&id='.$id.'&catid='.$catid.'&msg=1');
			}
			...
			*/
			...
			if (!$priv_db->get_one(array('catid'=>$catid, 'roleid'=>$memberinfo['groupid'], 'is_admin'=>0, 'action'=>'add'))) showmessage(L('category').L('publish_deny'), APP_PATH.'index.php?app=member');
			$category = $CATEGORYS[$catid];
			if($category['siteid']!=$siteid) showmessage(L('site_no_category'),'?app=member&controller=content&view=info_publish');
			$setting = string2array($category['setting']);
			if($zone == 0 && !isset($_GET['catid'])) {
				...
				$step = 2;	
				$step_1 = '<a href="'.APP_PATH.'index.php?app=member&controller=content&view=info_publish&siteid='.$siteid.'&city='.$citypinyin.'">'.$category['catname'].'</a>';
				include template('member', 'info_content_publish_select');
				exit;
			} elseif($zone == 0 && isset($_GET['catid'])) {
				$step = 3;	
				$step_1 = '<a href="'.APP_PATH.'index.php?app=member&controller=content&view=info_publish&siteid='.$siteid.'">'.$CATEGORYS[$category['parentid']]['catname'].'</a>';
				$step_2 = '<a href="'.APP_PATH.'index.php?app=member&controller=content&view=info_publish&siteid='.$siteid.'&city='.$citypinyin.'&catid='.$category['parentid'].'">'.$category['catname'].'</a>';			
				...
				$step = 4;				
				$step_1 = '<a href="'.APP_PATH.'index.php?app=member&controller=content&view=info_publish&siteid='.$siteid.'&city='.$citypinyin.'">'.$CATEGORYS[$category['parentid']]['catname'].'</a>';	
				$step_2 = '<a href="'.APP_PATH.'index.php?app=member&controller=content&view=info_publish&siteid='.$siteid.'&city='.$citypinyin.'&catid='.$category['parentid'].'">'.$category['catname'].'</a>';	
				$step_3 = '<a href="'.APP_PATH.'index.php?app=member&controller=content&view=info_publish&siteid='.$siteid.'&city='.$citypinyin.'&catid='.$catid.'">'.$zone_name.'</a>';	
	...
	function info_top() {
		...
		$catid = $_GET['catid'];
		$pos_data = shy_base::load_model('position_data_model');
	...
	function info_top_cost() {
		...
		if(isset($_POST['dosubmit'])) {
			$posids = array();
			$push_api = shy_base::load_app_class('push_api','admin');
			$pos_data = shy_base::load_model('position_data_model');
