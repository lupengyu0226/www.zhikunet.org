...
$(function(){
	...
	$("#field").formValidator({onshow:"<?php echo L('input').L('fieldname')?>",onfocus:"<?php echo L('fieldname').L('between_1_to_20')?>"}).regexValidator({regexp:"^[a-zA-Z]{1}([a-zA-Z0-9]|[_]){0,19}$",onerror:"<?php echo L('filedname_incorrect')?>"}).inputValidator({min:1,max:20,onerror:"<?php echo L('fieldname').L('between_1_to_20')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data : "app=member&controller=member_modelfield&view=public_checkfield&modelid=<?php echo $modelid?>",
		datatype : "html",
		cached:false,
...
<div class="pad_10">
<form name="myform" id="myform" action="?app=member&controller=member_modelfield&view=add" method="post">
...
<!--
	function field_setting(fieldtype) {
		$.getJSON("?app=member&controller=member_modelfield&view=public_field_setting&fieldtype="+fieldtype, function(data){
