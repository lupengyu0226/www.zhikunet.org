...
<script type="text/javascript">
  $(document).ready(function() {
	...
	$("#email").formValidator({onshow:"<?php echo L('input').L('email')?>",onfocus:"<?php echo L('email').L('format_incorrect')?>",oncorrect:"<?php echo L('email').L('format_right')?>"}).regexValidator({regexp:"email",datatype:"enum",onerror:"<?php echo L('email').L('format_incorrect')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data :"app=member&controller=member&view=public_checkemail_ajax&phpssouid=<?php echo $memberinfo['phpssouid']?>",
		datatype : "html",
		async:'false',
	...
	$("#nickname").formValidator({onshow:"<?php echo L('input').L('nickname')?>",onfocus:"<?php echo L('nickname').L('between_2_to_20')?>"}).inputValidator({min:2,max:20,onerror:"<?php echo L('nickname').L('between_2_to_20')?>"}).regexValidator({regexp:"ps_username",datatype:"enum",onerror:"<?php echo L('nickname').L('format_incorrect')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data :"app=member&controller=index&view=public_checknickname_ajax&userid=<?php echo $memberinfo['userid'];?>",
		datatype : "html",
		async:'false',
		success : function(data){
            if( data == "1" ) {
...
<form name="myform" action="?app=member&controller=member&view=edit" method="post" id="myform">
...
<fieldset>
	...
	<table width="100%" class="table_form">
		...
		<tr>
			<td><?php echo L('yuanyin')?></td>
			<td>
			<select name="info[yuanyin]" id="">
			<option value="<?php echo $memberinfo['yuanyin']?>" <?php if($yuanyin=='<?php echo $yuanyin;?>'){echo "selected";}?>><?php echo $memberinfo['yuanyin']?></option>
			<option value="涉黑" >涉黑</option>
			<option value="发送垃圾信息" >发布垃圾信息</option>
			<option value="发布非法信息" >发布非法信息</option>
			<option value="发布违规信息" >发布违规信息</option>
			<option value="谩骂" >谩骂</option>
			<option value="注册机">注册机</option>
			</select> <div class="onShow">禁止用户登录一定要输入原因</div>
			</td>
		</tr>
		<tr>
...
<!--
	function changemodel(modelid) {
		redirect('?app=member&controller=member&view=edit&userid=<?php echo $memberinfo['userid']?>&modelid='+modelid+'&safe_edi=<?php echo $_SESSION['safe_edi']?>');
	}
//-->
</script>
</html>
