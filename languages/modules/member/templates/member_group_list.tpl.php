...
<form name="myform" id="myform" action="?app=member&controller=member_group&view=delete" method="post" onsubmit="check();return false;">
<div class="pad-lr-10">
...
?>
    <tr>
		...
		<td align="center"><a class="xbtn btn-info btn-xs"  href="javascript:edit(<?php echo $v['groupid']?>, '<?php echo $v['name']?>')"><?php echo L('edit')?></a></td>
    </tr>
<?php
	}
?>
...
<input type="submit" class="button" name="dosubmit" onclick="document.myform.action='?app=member&controller=member_group&view=sort'" value="<?php echo L('sort')?>"/>
</div> 
...
function edit(id, name) {
	...
	window.top.art.dialog({title:'<?php echo L('edit').L('member_group')?>《'+name+'》',id:'edit',iframe:'?app=member&controller=member_group&view=edit&groupid='+id,width:'700',height:'500'}, function(){var d = window.top.art.dialog({id:'edit'}).data.iframe;d.document.getElementById('dosubmit').click();return false;}, function(){window.top.art.dialog({id:'edit'}).close()});
}

function check() {
	if(myform.action == '?app=member&controller=member_group&view=delete') {
		var ids='';
