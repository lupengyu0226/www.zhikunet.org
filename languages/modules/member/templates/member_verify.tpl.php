...
<form name="myform" action="?app=member&controller=member_verify&view=delete" method="post"  onsubmit="checkuid();return false;">
<div class="pad-lr-10">
...
<input type="submit" class="button" name="dosubmit" value="<?php echo L('verify_pass')?>" onclick="document.myform.action='?app=member&controller=member_verify&view=pass'"/>

<input type="submit" class="button" name="dosubmit" value="<?php echo L('reject')?>" onclick="document.myform.action='?app=member&controller=member_verify&view=reject'"/>

<input type="submit" class="button" name="dosubmit" value="<?php echo L('delete')?>" onclick="return confirm('<?php echo L('sure_delete')?>');"/>

<input type="submit" class="button" name="dosubmit" value="<?php echo L('ignore')?>" onclick="document.myform.action='?app=member&controller=member_verify&view=ignore'"/>
...
function member_verify(userid, modelid, name) {
	...
	window.top.art.dialog({title:'<?php echo L('member_verify')?>',id:'modelinfo',iframe:'?app=member&controller=member_verify&view=modelinfo&userid='+userid+'&modelid='+modelid,width:'700',height:'500'}, function(){var d = window.top.art.dialog({id:'modelinfo'}).data.iframe;d.document.getElementById('dosubmit').click();return false;}, function(){window.top.art.dialog({id:'modelinfo'}).close()});
}
//-->
</script>
</body>
