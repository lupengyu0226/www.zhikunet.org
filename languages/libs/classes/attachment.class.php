...
class attachment {
	...
	function __construct($module='', $catid = 0,$siteid = 0,$upload_dir = '') {
		...
		$this->upload_root = shy_base::load_config('system','upload_path');
		$cdnws = shy_base::load_config('system','cdnws');
	...
	function upload($field, $alowexts = '', $maxsize = 0, $overwrite = 0,$thumb_setting = array(), $watermark_enable = 1) {
		...
		if(!$this->is_allow_upload($_FILES[$field]['size'])) {
			$this->error = '13';
			return false;
		}
		$aids = array();
    ...
    function download($field, $value,$watermark = '0',$ext = 'gif|jpg|jpeg|bmp|png', $absurl = '', $basehref = '')
    {
        ...
        $this->att_db = shy_base::load_model('attachment_model');
        $upload_url = shy_base::load_config('system','upload_url');
        $cdnws   = shy_base::load_config('system','cdnws');
        $this->field = $field;
        $dir = date('Y/md/');
        $uploadpath =   $cdnws[array_rand($cdnws)].$dir; //$upload_url.$dir;//保存随机路径 以便于轮询
        ...
        if (preg_match_all("/src=('|\")data:([^;]*);base64,([^'\"]+)('|\")/",$string, $matches)) {//匹配data:图片，如果没有该参数，就进行下载
            ...
            foreach($matches[3] as $r=>$matche){
                ...
                $_filename=explode("/",$matches[2][$r]);
                $filename=$_filename[1];
                $filename = $this->getname($filename);
                file_put_contents($uploaddir.$filename,base64_decode($matche));//写入文件
				if($thumb_enable) {
					$image = new image($thumb_enable,$this->siteid);
					$image->thumb($uploaddir.$filename,$uploaddir.$filename,$thumb_setting[0],$thumb_setting[1]);
                ...
                $datapath[]=str_replace($this->upload_root,'',$cdnws[array_rand($cdnws)].$uploaddir.$filename);
            }
            $string=str_replace($dataurl,$datapath,$string);
        }
        if(!preg_match_all("/(href|src)=([\"|']?)([^ \"'>]+)\\2/is", $string, $matches))  return $value;
        ...
        foreach($remotefileurls as $k=>$file) {
            if(strpos($file, '://') === false || strpos($file, $upload_url) !== false||strpos_arr($file,$cdnws) !== false) continue;
            $upload_func = "$this->_copy";//$this->upload_func;
            $data=$this->_copy($file);
            if ($data['info']['http_code']!=200) continue;
            //$filename=str_replace('image/','',trim($data['info']['content_type']));
            $filename=str_replace('image/','',explode(';',trim($data['info']['content_type']))[0]);
            if (!preg_match("/($ext)/is",$filename))  continue;
            ...
            if(@file_put_contents($newfile,$data['data'])!==false) { //远程图片本地化 @copy($file['tmp_name'], $savefile)  $this->_copy($file,$newfile)
                $oldpath[] = $k;
                $GLOBALS['downloadfiles'][] = $newpath[] = $cdnws[array_rand($cdnws)].$dir.$filename;//$uploadpath.$filename;
                @chmod($newfile, 0644);
	...
	function add($uploadedfile) {
		...
		$uploadedfile['status'] = shy_base::load_config('system','attachment_stat') ? 0 : 1;
	...
	*/
	...
	/**
	 * 是否允许上传
	 * @param $uploads 上传文件filesize 字节
	 */
	function is_allow_upload($uploads = 0) {
		if ($_SESSION['roleid']) {
			return TRUE;
		}
		$starttime = SYS_TIME - 86400;
		$site_setting = $this->_get_site_setting($this->siteid);
		return ($uploads && (($uploads / 1024) < $site_setting['upload_maxsize']));
	}
	/**
	 * 返回错误信息
	 */
    ...
    private function _copy($file){
		$array=array();
		$curl = curl_init();
		$downip = mt_rand(0,255).'.'.mt_rand(0,255).'.'.mt_rand(0,255).'.'.mt_rand(0,255);
		$CURLOPT_HTTPHEADER=array(array('X-FORWARDED-FOR:'.$downip, 'CLIENT-IP:'.$downip));
		...
		curl_setopt($curl, CURLOPT_HTTPHEADER,$CURLOPT_HTTPHEADER[array_rand($CURLOPT_HTTPHEADER)]);
		curl_setopt($curl,CURLOPT_USERAGENT,'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.99 Safari/537.36'); 
		// 运行cURL，请求网页
	...
	function uploaderror($id = 0)	{
		file_put_contents(SHUYANG_PATH.'xxx.txt', $id);
	}
