...
class segment {
	...
	function __construct($loaddic=true) {
  	if($loaddic) {
  	  ...
  	  $dicfile = SHY_PATH.'libs'.DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.'dict'.DIRECTORY_SEPARATOR.'dict.csv';
  	  $fp = fopen($dicfile,'r');
	  $i=1;
  	  while($line = fgets($fp,64)){
  		  //if($i>20) break;
		  $ws = explode(' ',$line);
		  #$ws = explode(' ',str_replace('"','',$line));
		  //print_r($ws); 
  ...
  function get_source($str) {
    ...
    if(CHARSET == 'utf-8') $str = mb_convert_encoding($str,'gbk','utf-8');
  ...
  function split_result($str='',$try_num_name=true,$try_diff=true) {
    ...
    if(CHARSET== 'utf-8') $okstr = mb_convert_encoding($this->result_string,'utf-8','gbk');
