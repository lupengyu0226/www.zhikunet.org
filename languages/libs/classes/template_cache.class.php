...
final class template_cache {
	...
	public function template_compile($module, $template, $style = 'default') {
		...
		} elseif (strpos($module, 'mall/') !== false) {
			$module = str_replace('/', DIRECTORY_SEPARATOR, $module);
			$tplfile = $_tpl = SHY_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html';
		} else {
			$plugin = str_replace('plugin/', '', $module);
			$module = str_replace('/', DIRECTORY_SEPARATOR, $module);
			$tplfile = $_tpl = SHY_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html';
		}
			...
			switch ($op) {
				...
				case 'get':
						...
						if (isset($page)) {
							...
							$str .= '$r = $get_db->sql_query("'.$sql.'");$s = $get_db->fetch_next();$edi_pages=edi_pages($s[\'count\'], $page, $pagesize, $urlrule);';
							$str .= '$r = $get_db->sql_query("'.$sql.'");$s = $get_db->fetch_next();$wap_pages=wap_pages($s[\'count\'], $page, $pagesize, $urlrule);';
							$str .= '$r = $get_db->sql_query("'.$sql.'");$s = $get_db->fetch_next();$miplists_pages=miplists_pages($s[\'count\'], $page, $pagesize, $urlrule);';
							$str .= '$r = $get_db->sql_query("'.$sql.'");$s = $get_db->fetch_next();$ipad_pages=ipad_pages($s[\'count\'], $page, $pagesize, $urlrule);';
						}
				...
				case 'block':
					$str .= '$block_tag = shy_base::load_app_class(\'block_tag\', \'block\');';
					$str .= 'echo $block_tag->shy_tag('.self::arr_to_html($datas).');';
					break;
			}
		} else {
			if (!isset($action) || empty($action)) return false;
			if (module_exists($op) && file_exists(SHY_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.$op.DIRECTORY_SEPARATOR.'classes'.DIRECTORY_SEPARATOR.$op.'_tag.class.php')) {
				$str .= '$'.$op.'_tag = shy_base::load_app_class("'.$op.'_tag", "'.$op.'");if (method_exists($'.$op.'_tag, \''.$action.'\')) {';	
				if (isset($start) && intval($start)) {
				...
				if (isset($page)) {
					...
					$str .= '$edi_pages = edi_pages($'.$op.'_total, $page, $pagesize, $urlrule);';
					$str .= '$wap_pages = wap_pages($'.$op.'_total, $page, $pagesize, $urlrule);';
					$str .= '$miplists_pages = miplists_pages($'.$op.'_total, $page, $pagesize, $urlrule);';
					$str .= '$ipad_pages = ipad_pages($'.$op.'_total, $page, $pagesize, $urlrule);';
				}
