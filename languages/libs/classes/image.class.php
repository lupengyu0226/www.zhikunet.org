...
class image {
	...
	function watermark($source, $target = '', $w_pos = '', $w_img = '', $w_text = 'shuyang',$w_font = 8, $w_color = '#ff0000') {
		$w_pos = $w_pos ? $w_pos : $this->w_pos;
		$w_img = $w_img ? $w_img : $this->w_img;
		if(!$this->watermark_enable || !$this->check($source)) return false;
		if(!$target) $target = $source;
		$w_img = SHUYANG_PATH.$w_img;
		...
		if(!empty($w_img) && file_exists($w_img)) {
			...
			$ifwaterimage = 0;
			$temp = imagettfbbox(ceil($w_font*2.5), 0, SHY_PATH.'libs/data/font/elephant.ttf', $w_text);
			$width = $temp[2] - $temp[6];
