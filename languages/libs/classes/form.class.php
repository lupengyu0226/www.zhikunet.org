...
class form {
	...
	public static function editor($textareaid = 'content', $toolbar = 'basic', $module = '', $catid = '', $color = '', $allowupload = 0, $allowbrowser = 1,$alowuploadexts = '',$height = 200,$disabled_page = 0, $allowuploadnum = '10') {
		...
		if($toolbar == 'basic') {
			...
			$toolbar .= "['Bold', 'Italic', 'TextColor'],['JustifyLeft','JustifyCenter','JustifyRight','JustifyBlock', 'Image'],['Maximize'],\r\n";
		} elseif($toolbar == 'full') {
			if(defined('IN_ADMIN')) {
				$toolbar = "['Source',";
			} else {
        ...
        if($allowupload) $str .= "filebrowserUploadUrl : 'index.php?app=attachment&controller=attachments&view=upload&module=".$module."&catid=".$catid."&dosubmit=1',\r\n";
		if($color) {
	...
	public static function select_category($file = '',$catid = 0, $str = '', $default_option = '', $modelid = 0, $type = -1, $onlysub = 0,$siteid = 0,$is_push = 0) {
		...
		if($is_push=='1'){
			$priv = shy_base::load_model('category_priv_model');
	...
	public static function menu_linkage($linkageid = 0, $id = 'linkid', $defaultvalue = 0) {
	...
	public static function select_template($style, $module, $id = '', $str = '', $pre = '') {
		$tpl_root = shy_base::load_config('system', 'tpl_root');
		$templatedir = SHY_PATH.$tpl_root.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR;
		$confing_path = SHY_PATH.$tpl_root.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.'config.php';
	...
	public static function checkcode($id = 'checkcode',$code_len = 4, $font_size = 20, $width = 130, $height = 50, $font = '', $font_color = '', $background = '') {
		return "<img id='$id' onclick='this.src=this.src+\"&\"+Math.random()' src='".SITE_PROTOCOL.SITE_URL.WEB_PATH."index.php?app=api&controller=index&op=checkcode&code_len=$code_len&font_size=$font_size&width=$width&height=$height&font_color=".urlencode($font_color)."&background=".urlencode($background)."'>";
	}
	/**
	 * url  规则调用
	 * 
