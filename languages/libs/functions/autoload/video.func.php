...
function get_comments($commentid) {
	...
	$db = shy_base::load_model('comment_model');
...
function video_filters_url($fieldname,$array=array(),$type = 1,$modelid, $isphp = 0) {
	...
	//伪静态url规则管理，apache伪静态支持9个参数
	 $urlrule =APP_PATH.'index.php?app=content&controller=index&view=lists&catid='.$_GET[catid].'&modelid='.$modelid.$urlpars.'&page={$page}';
	//根据get传值构造URL
...
function player_code($id = 'video_player',$channelid,$vid,$width = 622, $height = 460, $style_projectid = '') {
	...
	$playerurl = shy_base::load_config('ku6server', 'player_url').$vid.'/style/'.$style_projectid.'/v.swf';
