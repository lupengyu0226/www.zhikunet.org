<?php
/**
 *  global.func.php 飞天系统公共函数库
 *
 * @copyright			(C) 2005-2015 EDI
 ...
 * @return mixed
 */
function shy_addslashes($string){
	$string=new_stripslashes($string);
	if(!is_array($string)) return addslashes($string);
	foreach($string as $key => $val) $string[$key] = shy_addslashes($val);
	return $string;
}
/**
 *手机站URL 规则
 ...
 * @return string
 */
function is_mips($string){
	$search = 'www';
	$replace = 'mip';
	return str_replace($search, $replace, $string);
}
function is_https($string){
	 $search = array("//img","http://");
	 $replace = array("https://img","https://");
	 return str_replace($search, $replace, $string);
}
/**
 ...
 */
 ...
 function is_today($inputtime) {
    $inputtime = date('y-m-d',$inputtime);
    $a_ux = strtotime($inputtime);
 ...
 * @return string
 */
function is_avatar($string){
	$search = 'http://account.05273.cn/uploadfile/avatar/';
	$replace = '//avatar.05273.cn/';
	return str_replace($search, $replace, $string);
}
/**
 ...
 * @return string
 */
function trim_indent($string){
	 $search = array(" ","　","\n","\r","\t","&rdquo;","&ldquo;","&middot;","&amp;bull;","&quot;",'"',"&amp;lsquo;","&amp;rsquo;","&amp;183;","&amp;8212;");
	 $replace = array("","","","","","","","","","","","","","","");
	 return str_replace($search, $replace, $string);
}
/**
...
function trim_html($string){
	 $search = array("&ldquo;","&rdquo;","&mdash;","&bull;");
	 $replace = array("","","——","");
	 return str_replace($search, $replace, $string);
}
/**
 ...
 * @return string
 */
function trim_jsonld($string){
	 $search = array("\n","\r","\t","&rdquo;","&ldquo;","&middot;","&amp;bull;","&quot;",'"');
	 $replace = array("","","","","","","","","");
	 return str_replace($search, $replace, $string);
}
/**
 ...
 * @return string
 */
function trim_weapp($string){
	 $search = array("http://img1","http://img2","http://img3","http://upload");
	 $replace = array("https://img","https://img","https://img","https://img");
	 return str_replace($search, $replace, $string);
}
/**
 * 将文本格式成适合js输出的字符串
...
function get_url() {
	...
	if(isset($_SERVER['QUERY_STRING'])){
		...
		$_SERVER['QUERY_STRING']=http_build_query($myArray);

	}
	if(isset($_SERVER['REQUEST_URI'])){
		//parse_str($_SERVER['REQUEST_URI'],$myArray);
		//$_SERVER['REQUEST_URI']=http_build_query($myArray);
	}
...
* 先搜索$data是否以array开头，如果不是则将其反序列化 unserialize
*/
...
function sys_auth($string, $operation = 'ENCODE', $key = '', $expiry = 0) {
	$ckey_length = 4;
	$key = md5($key != '' ? $key : shy_base::load_config('system', 'auth_key'));
...
function mobilemsg($msg, $url_forward = 'goback', $ms = 1250, $dialog = '', $returnjs = '') {
	if(defined('IN_ADMIN')) {
		include(admin::admin_tpl('showmessage', 'admin'));
	} else {
		include(template('mobile', 'minimessage'));
	}
	exit;
}

function mipsmsg($msg, $url_forward = 'goback', $ms = 1250, $dialog = '', $returnjs = '') {
	if(defined('IN_ADMIN')) {
		include(admin::admin_tpl('showmessage', 'admin'));
	} else {
		include(template('mobile', 'minimessage'));
 ...
 * @return 分页
 */
function edi_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 5) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
 ...
 * @return 分页
 */
function miplists_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 5) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
	...
	if($num > $perpage) {
		...
		$multipage .= '';
		if($curr_page>0) {
			$multipage .= ' <mip-link href="'.MOBILE_PATH.pageurl($urlrule, $curr_page-1, $array).'" class="a1">'.L('previous').'</mip-link>';
			if($curr_page==1) {
				$multipage .= ' <span>1</span>';
			} elseif($curr_page>6 && $more) {
				$multipage .= ' <mip-link href="'.MOBILE_PATH.pageurl($urlrule, 1, $array).'">1</mip-link>..';
			} else {
				$multipage .= ' <mip-link href="'.MOBILE_PATH.pageurl($urlrule, 1, $array).'">1</mip-link>';
			}
		}
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				$multipage .= ' <mip-link href="'.MOBILE_PATH.pageurl($urlrule, $i, $array).'">'.$i.'</mip-link>';
			} else {
				$multipage .= ' <span>'.$i.'</span>';
			}
		}
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' ..<mip-link href="'.MOBILE_PATH.pageurl($urlrule, $pages, $array).'">'.$pages.'</mip-link> <mip-link href="'.MOBILE_PATH.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</mip-link>';
			} else {
				$multipage .= ' <mip-link href="'.MOBILE_PATH.pageurl($urlrule, $pages, $array).'">'.$pages.'</mip-link> <mip-link href="'.MOBILE_PATH.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</mip-link>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <span>'.$pages.'</span> <mip-link class="a1">'.L('next').'</mip-link>';
		} else {
			$multipage .= ' <mip-link href="'.MOBILE_PATH.pageurl($urlrule, $pages, $array).'">'.$pages.'</mip-link> <mip-link href="'.MOBILE_PATH.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</mip-link>';
		}
	}
 ...
 * @return 分页
 */
function ipad_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 5) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
	...
	if($num > $perpage) {
		...
		$multipage .= '';
		if($curr_page>0) {
			$multipage .= ' <mip-link href="'.MIP_PATH.pageurl($urlrule, $curr_page-1, $array).'" class="a1">'.L('previous').'</mip-link>';
			if($curr_page==1) {
				$multipage .= ' <span>1</span>';
			} elseif($curr_page>6 && $more) {
				$multipage .= ' <mip-link href="'.MIP_PATH.pageurl($urlrule, 1, $array).'">1</mip-link>..';
			} else {
				$multipage .= ' <mip-link href="'.MIP_PATH.pageurl($urlrule, 1, $array).'">1</mip-link>';
			}
		}
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				$multipage .= ' <mip-link href="'.MIP_PATH.pageurl($urlrule, $i, $array).'">'.$i.'</mip-link>';
			} else {
				$multipage .= ' <span>'.$i.'</span>';
			}
		}
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' ..<mip-link href="'.MIP_PATH.pageurl($urlrule, $pages, $array).'">'.$pages.'</mip-link> <mip-link href="'.MIP_PATH.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</mip-link>';
			} else {
				$multipage .= ' <mip-link href="'.MIP_PATH.pageurl($urlrule, $pages, $array).'">'.$pages.'</mip-link> <mip-link href="'.MIP_PATH.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</mip-link>';
			}
		} elseif($curr_page==$pages) {
 ...
 * @return 分页
 */
 function wap_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 5) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
	...
	if($num > $perpage) {
		...
		$multipage .= '';
		if($curr_page>1) {
			$multipage .= '<a href="'.MOBILE_PATH.pageurl($urlrule, 1, $array).'">首页</a>  <a href="'.MOBILE_PATH.pageurl($urlrule, $curr_page-1, $array).'" class="a1">'.L('previous').'</a>';
		}
		if($curr_page>0) {
			if($curr_page==1) {
				$multipage .= ' <span>1</span>';
			} elseif($curr_page>6 && $more) {
				$multipage .= ' <a href="'.MOBILE_PATH.pageurl($urlrule, 1, $array).'">1</a>..';
			} else {
				$multipage .= ' <a href="'.MOBILE_PATH.pageurl($urlrule, 1, $array).'">1</a>';
			}
		}
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				$multipage .= ' <a href="'.MOBILE_PATH.pageurl($urlrule, $i, $array).'">'.$i.'</a>';
			} else {
				$multipage .= ' <span>'.$i.'</span>';
			}
		}
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' ..<a href="'.MOBILE_PATH.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.MOBILE_PATH.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a>';
			} else {
				$multipage .= ' <a href="'.MOBILE_PATH.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.MOBILE_PATH.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a>';
			}
		} elseif($curr_page==$pages) {
...
function pageurl($urlrule,$page, $array = array()) {
	...
	if(is_array($array)&&isset($array['urlruleishtml'])&& $array['urlruleishtml']){//静态页面

		$url=$array['url'].str_replace($findme,$replaceme,str_replace(array('{$categorydir}','{$catdir}','{$year}','{$month}','{$day}','{$id}'),array($categorydir,$catdir,$year,$month,$day,$id),$urlrule));
	}else{
		$url=str_replace($findme,$replaceme,$urlrule);

	}
    if (IsDomain(parse_url($url)['host'])) {
        $url = preg_replace(array('/^((\w+):)?\/\//', '/\/{2,}/', '/~/'), array('$1~', '/', '//'), $url);
    } else {
        $url = preg_replace(array('/\/{2,}/'), array('/'), $url);
    }
...
function get_memberavatar($uid, $is_userid='', $size='30') {
	...
	shy_base::load_app_class('client', 'member', 0);
...
function wocali_memberavatar($uid, $is_userid='', $size='30') {
	if($is_userid) {
		$db = shy_base::load_model('member_model');
		$memberinfo = $db->get_one(array('userid'=>$uid));
		if(isset($memberinfo['phpssouid'])) {
	...
	if($datas['style']=='1') {
		...
		$string .= '<script type="text/javascript">
					$(function(){
						...
						$ld5.ld({ajaxOptions : {"url" : "'.APP_PATH.'index.php?app=api&controller=index&op=get_linkage&act=ajax_select&keyid='.$linkageid.'"},defaultParentId : 0,style : {"width" : 120}})	 
 ...
 * 传入field，取用户$field字段信息
 */
function zhuanlan_memberinfo($username, $field='') {
	if(empty($username)){return false;}
	static $zhuanlan_memberinfo;
	if (!isset($zhuanlan_memberinfo[$username])) {
		$zhuanlan_db = shy_base::load_model('zhuanlan_model');
		$zhuanlan_memberinfo[$username] = $zhuanlan_db->get_one(array('username'=>$username));
	}
	if(!empty($field) && !empty($zhuanlan_memberinfo[$username][$field])) {
		return $zhuanlan_memberinfo[$username][$field];
	} else {
		return $zhuanlan_memberinfo[$username];
	}
}
/**
 * 判断是否开启沭阳号
 * @param $username	用户名称
 */
function zhuanlan_exists($username = '') {
	$zhuanlan_db = shy_base::load_model('zhuanlan_model');
	$zhuanlan_memberinfo = $zhuanlan_db->get_one(array('username'=>$username));
	if($zhuanlan_memberinfo['status']=='1') {
		return '1';
	} else {
		return '0';
	}
...
function thumb($imgurl, $width = 100, $height = 100 ,$autocut = 1, $smallpic = 'nopic.gif') {
	...
	//$imgurl_replace= str_replace(array($upload_url,'http:'),array('',''), $imgurl);
	if(!empty($annex)){
		foreach($annex as $anex){
			if(strpos($imgurl,$anex)===false){
				$imgurl_replace= $imgurl;
			continue;
			}else{
				$imgurl_replace= str_replace(array($anex,'https:'),array('',''),$imgurl);
			break;
			}
		}
	}
    ...
    if(file_exists($upload_path.$newimgurl)) return $annex[array_rand($annex)].$newimgurl;//$upload_url.$newimgurl;
    
    if(!is_object($image)) {
        shy_base::load_sys_class('image','','0');
        $image = new image(1,0);
    }

    return !$image->thumb($upload_path.$imgurl_replace, $upload_path.$newimgurl, $width, $height, '', $autocut)===false? $annex[array_rand($annex)].$newimgurl : $imgurl;
 ...
 * @param $symbol 栏目间隔符
 */
function catposno($catid, $symbol=' > '){
	$category_arr = array();
	$siteids = getcache('category_content','commons');
	$siteid = $siteids[$catid];
	$category_arr = getcache('category_content_'.$siteid,'commons');
...
function go($catid,$id, $allurl = 0) {
	...
	if (!empty($allurl)) {
		if (!preg_match('/^(http(s)?:)?\/\//',$r['url'])) {
			if (!preg_match('/^(http(s)?:)?\/\//',$category[$catid]['url'])) {
 ...
 * @param $catid   栏目ID
 * @param $id      文章ID
 * @param 2017.12.29使用在微信接口
 */
function m_show($catid,$id) {
	static $category;
	if(empty($category)) {
		$siteids = getcache('category_content','commons');
		$siteid = $siteids[$catid];
	...
	$db->set_model($modelid);
	$r = $db->get_one(array('id'=>$id), '`url`');
	$search = array(APP_PATH);
	$replace = array(MOBILE_PATH);
	return str_replace($search, $replace, $r['url']);
}
/**
 * 将附件地址转换为绝对地址
 * @param $path 附件地址
...
function upload_key($args) {
	$shy_auth_key = md5(SHY_PATH.'upload'.shy_base::load_config('system','auth_key').$_SERVER['HTTP_USER_AGENT']);
	$authkey = md5($args.$shy_auth_key);
	return $authkey;
}
/**
 * 生成验证key
...
function get_auth_key($prefix,$suffix="") {
	if($prefix=='login'){
		$pc_auth_key = md5(SHY_PATH.'login'.shy_base::load_config('system','auth_key').ip());
	}else if($prefix=='email'){
		$pc_auth_key = md5(SHY_PATH.'email'.shy_base::load_config('system','auth_key'));
	}else{
		$pc_auth_key = md5(SHY_PATH.'other'.shy_base::load_config('system','auth_key').$suffix);
	}
 ...
 * @param string $font 字体名称 字体请放于shuyang\libs\data\font下
...
function string2img($txt, $fonttype = 5, $fontsize = 16, $font = '', $fontcolor = 'FF0000',$transparent = '1') {
	...
	if(function_exists("imagepng")) {
		...
		$txt = '<img src="'.APP_PATH.'index.php?app=api&controller=index&op=creatimg&txt='.$txt.'&fonttype='.$fonttype.'&fontsize='.$fontsize.'&font='.$font.'&fontcolor='.$fontcolor.'&transparent='.$transparent.'" align="absmiddle">';
	}
	return $txt;
}
...
function runhook($method) {
	...
	if(!empty($hook_appid)) {
		foreach($hook_appid as $appid => $p) {
			$pluginfilepath = SHY_PATH.'plugin'.DIRECTORY_SEPARATOR.$p.DIRECTORY_SEPARATOR.'hook.class.php';
			$getpclass = TRUE;
...
function p_template($plugin = 'content', $template = 'index',$style='default') {
	if(!$style) $style = 'default';
	$template_cache = shy_base::load_sys_class('template_cache');
	$compiledtplfile = SHUYANG_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.$template.'.php';

	if(!file_exists($compiledtplfile) || (file_exists(SHY_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html') && filemtime(SHY_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html') > filemtime($compiledtplfile))) {
		$template_cache->template_compile('plugin/'.$plugin, $template, 'default');
	} elseif (!file_exists(SHY_PATH.'plugin'.DIRECTORY_SEPARATOR.$plugin.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$template.'.html')) {
...
function cache_page_start() {
	...
	$contents = getcache(CACHE_PAGE_ID, 'StaticPageCache_tmp'.DIRECTORY_SEPARATOR.substr(CACHE_PAGE_ID, 0, 2));
...
function cache_page($ttl = 360, $isjs = 0) {
	...
	setcache(CACHE_PAGE_ID, $contents, 'StaticPageCache_tmp'.DIRECTORY_SEPARATOR.substr(CACHE_PAGE_ID, 0, 2));
}
 ...
 * 
 * @param string $url 网址
 * @return string 返回协议头(http 或者 https)
 */
function polling($url){
	if(!is_object($http)){  	
		shy_base::load_sys_class('http','',0);	
		$http = new http();
		}
	$return='';
	$Netprotocol=array('http:','');
	foreach($Netprotocol as $scheme){
			if($http->get_httpstatus($scheme.$url)==200){ 
				return $scheme;break;
			}
			else{
		}  	
...
function halt() {
   ...
   if (APP_DEBUG || IS_CLI) {
   	  ...
   	  $e = preg_replace("/shuyangwang\//is", "", $e); 
   	  $e = preg_replace("/www/is", "ThinkPHP", $e); 
   	  $e = preg_replace("/languages/is", "DaShuWang", $e); 
   	  $e = preg_replace("/libs/is", "Library", $e); 
   	  $e = preg_replace("/classes/is", "Think", $e); 
   	  $e = preg_replace("/application/is", "App", $e); 
