...
function sendmail($toemail, $subject, $message, $from='',$cfg = array(), $sitename='沭阳网') {
	if($sitename=='') {
		$siteid = get_siteid();
	...
	if($cfg && is_array($cfg)) {
		$from = $cfg['tfrom'];
		$mail = $cfg;
		$mail_type = $cfg['tmail_type']; //邮件发送模式
	} else {
		$cfg = getcache('tougao','commons');
		$from = $cfg['tmail_from'];
		$mail_type = $cfg['tmail_type']; //邮件发送模式
		if($cfg['tmail_user']=='' || $cfg['tmail_password'] ==''){
			return false;
		}
		$mail= Array (
			'mailsend' => 2,
			'maildelimiter' => 1,
			'mailusername' => 1,
			'server' => $cfg['tmail_server'],
			'port' => $cfg['tmail_port'],
			'auth' => $cfg['tmail_auth'],
			'from' => $cfg['tmail_from'],
			'auth_username' => $cfg['tmail_user'],
			'auth_password' => $cfg['tmail_password']
		);		
	}
	//mail 发送模式
	if($mail_type==0) {
	...
	$headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: SHUYANG-V9 {$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/html; charset=".CHARSET."{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
	...
	fputs($fp, ($mail['auth'] ? 'EHLO' : 'HELO')." shuyang\r\n");
	$lastmessage = fgets($fp, 512);
