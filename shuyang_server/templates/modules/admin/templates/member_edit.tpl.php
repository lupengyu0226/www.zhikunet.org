...
$(function(){
	...
	$("#email").formValidator({onshow:"<?php echo L('input')?>email",onfocus:"<?php echo L('notice_format')?>",oncorrect:"<?php echo L('right')?>"}).regexValidator({regexp:"email",datatype:"enum",onerror:"email<?php echo L('format_incorrect')?>"}).ajaxValidator({
			type : "get",
			url : "",
			data :"app=admin&controller=member&view=ajax_email&uid=<?php echo $userinfo['uid']?>",
			datatype : "html",
			async:'true',
...
<div class="subnav">
  <h2 class="title-1 line-x f14 fb blue lh28"><?php echo L('member_edit')?></h2>
	<div class="content-menu ib-a blue line-x">
		<a href="?app=admin&controller=member&view=manage">
		<em><?php echo L('member_manage')?></em></a>
		<span>|</span>
		<a href="?app=admin&controller=member&view=edit&uid=<?php echo $userinfo['uid']?>" class="on">
		<em><?php echo L('member_edit')?></em></a>
	</div>
</div>
<div class="pad-lr-10">
<form method=post action="?app=admin&controller=member&view=edit" id="myform">
