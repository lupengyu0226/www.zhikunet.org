...
$(function(){
	...
	$("#username").formValidator({onshow:"<?php echo L('input').L('username')?>",onfocus:"<?php echo L('username').L('between_6_to_20')?>"}).inputValidator({min:6,max:20,onerror:"<?php echo L('username').L('between_6_to_20')?>"}).ajaxValidator({
	    type : "get",
		url : "",
		data :"app=admin&controller=administrator&view=ajax_username",
		datatype : "html",
		async:'false',
...
<div class="content-menu ib-a blue line-x"><a href="?app=admin&controller=administrator&view=init"><em><?php echo L('listadmins')?></em></a><span>|</span> <a href="?app=admin&controller=administrator&view=add"  class="on"><em><?php echo L('add_admin')?></em></a></div>
</div>
<div class="pad-lr-10">
<form action="?app=admin&controller=administrator&view=add" method="post" id="myform">
