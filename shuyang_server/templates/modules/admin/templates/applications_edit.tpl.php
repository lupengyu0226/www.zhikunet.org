...
$(function(){
	...
	$("#name").formValidator({onshow:"<?php echo L('input').L('application_name')?>",onfocus:"<?php echo L('input').L('application_name')?>"}).inputValidator({min:1,max:20,onerror:"<?php echo L('input').L('application_name')?>"}).ajaxValidator({type : "get",url : "",data :"app=admin&controller=applications&view=ajax_name&id=<?php echo $appid?>",datatype : "html",async:'false',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "<?php echo L('application_name').L('exist')?>",onwait : "<?php echo L('connecting_please_wait')?>"}).defaultPassed();

	$("#url").formValidator({onshow:'<?php echo L('application_url_msg')?>',onfocus:'<?php echo L('application_url_msg')?>',tipcss:{width:'400px'}}).inputValidator({min:1,max:255,onerror:'<?php echo L('application_url_msg')?>'}).inputValidator({min:1,max:255,onerror:'<?php echo L('application_url_msg')?>'}).regexValidator({regexp:"http:\/\/(.+)\/$",onerror:'<?php echo L('application_url_msg')?>'}).ajaxValidator({type : "get",url : "",data :"app=admin&controller=applications&view=ajax_url&id=<?php echo $appid?>",datatype : "html",async:'false',success : function(data){	if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "<?php echo L('application_url').L('exist')?>",onwait : "<?php echo L('connecting_please_wait')?>"}).defaultPassed();
...
<div class="content-menu ib-a blue line-x"><a href="?app=admin&controller=applications&view=init"><em><?php echo L('application_list')?></em></a><span>|</span> <a href="?app=admin&controller=applications&view=add"><em><?php echo L('application_add')?></em></a></div>
</div>
<div class="pad-lr-10">
<form action="?app=admin&controller=applications&view=edit&appid=<?php echo $appid?>" method="post" id="myform">
<table width="100%"  class="table_form">
  ...
  <tr>
   ...
   <tr>
	...
	<option value="shuyangwang"<?php if ($data['type']=='shuyangwang'){echo ' selected';}?>>沭阳网飞天系统</option>
...
function change_apifile(value) {
	if (value=='shuyang'  && $('#apifilename').val() == '') {
		$('#apifilename').val('?app=api&controller=phpsso');
	}
}
</script>
