...
<div class="subnav">
	...
	<div class="content-menu ib-a blue line-x"><a href="?app=admin&controller=system&view=init"><em><?php echo L('register_setting')?></em></a><span>|</span> <a href="?app=admin&controller=system&view=uc"  class="on"><em><?php echo L('uc_setting')?></em></a><span>|</span><a href="?app=admin&controller=system&view=sp4"><em><?php echo L('sp4_password_compatible')?></em></a></div>
</div>
<div class="pad-lr-10">
<form action="?app=admin&controller=system&view=uc" method="post" name="form_messagequeue_manage">
...
<!--
	function mysql_test() {
	$.get('?app=admin&controller=system&view=myqsl_test', {host:$('#uc_dbhost').val(),username:$('#uc_dbuser').val(), password:$('#uc_dbpw').val()}, function(data){if(data==1){alert('<?php echo L('connect_success')?>')}else{alert('<?php echo L('connect_failed')?>')}});
}
//-->
</script>
</body>
