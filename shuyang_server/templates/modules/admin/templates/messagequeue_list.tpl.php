...
<form action="?app=admin&controller=messagequeue&view=delete" method="post" name="form_messagequeue_manage">
...
function renotice(noticeid, appid) {
	$.post('?app=admin&controller=messagequeue&view=renotice'+'&'+Math.random(), {noticeid:noticeid, appid:appid},
	function(data){
...
function notice(noticeid, appid) {
	$.post('?app=admin&controller=messagequeue&view=notice&noticeid='+noticeid+'&appid='+appid+'&'+Math.random(),function(data){
		if(data == 1) {
			$('#status_'+noticeid+'_'+appid).html('<?php echo '<div  class="onCorrect">'.L('notice_success').'</div>'?>');
		}
	});
