...
function L($language = 'no_language',$pars = array(), $modules = '') {
	...
	if(!$LANG) {
		...
		if(file_exists(SHY_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.ROUTE_M.'.lang.php')) require SHY_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.ROUTE_M.'.lang.php';
		if(!empty($modules)) {
			$modules = explode(',',$modules);
			foreach($modules AS $m) {
				require SHY_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.$m.'.lang.php';
			}
		}
	}
...
function template($module = 'content', $template = 'index', $style = 'default') {
	...
	if(file_exists(SHY_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html')) {
		...
		$compiledtplfile = SHUYANG_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.default.php';
		if(file_exists(SHY_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') && filemtime(SHY_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html') > filemtime($compiledtplfile)) {
 ...
 * showmessage('登录成功', array('默认跳转地址'=>'http://www.05273.com'));
 ...
 * @param int $ms 跳转等待时间
 */
function halt($msg, $url_forward = 'goback', $ms = 1250, $dialog = '') {
	if(defined('IN_ADMIN')) {
		include(admin::admin_tpl('error', 'admin'));
	} else {
		include(template('content', 'message'));
	}
	exit;
...
function show_ad($siteid, $id) {
	...
	$p = shy_base::load_model('poster_model');
...
function get_uc_database() {
	$config = shy_base::load_config('system');
