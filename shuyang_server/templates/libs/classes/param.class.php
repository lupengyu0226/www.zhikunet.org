...
class param {
	...
	public static function set_cookie($var, $value = '', $time = 0) {
		...
		$s = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on') || (isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && $_SERVER['HTTP_X_FORWARDED_PROTO'] == 'https')) ? 1 : 0;
		...
		if (is_array($value)) {
			...
			setcookie($var, sys_auth($value, 'ENCODE',md5(SHY_PATH.'cookie'.$var).shy_base::load_config('system','auth_key')), $time, shy_base::load_config('system','cookie_path'), shy_base::load_config('system','cookie_domain'), $s, $httponly);
		}
	}
