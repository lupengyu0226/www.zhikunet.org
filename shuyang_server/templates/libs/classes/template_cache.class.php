...
public function template_compile($module, $template, $style = 'default') {
		...
		$tplfile = $_tpl = SHY_PATH.'templates/'.$style.'/'.$module.'/'.$template.'.html';
		if ($style != 'default' && ! file_exists ( $tplfile )) {
			$tplfile = SHY_PATH.'templates/default/'.$module.'/'.$template.'.html';
		}
		...
		$filepath = SHUYANG_PATH.'caches/caches_template/'.$module.'/';
	    if(!is_dir($filepath)) {
...
public function template_compile_admin($module, $template) {
		...
		$tplfile = $_tpl = SHY_PATH.'templates/admin/'.$module.'/'.$template.'.html';
		$content = @file_get_contents ( $tplfile );
		$filepath = SHUYANG_PATH.'caches/caches_template/admintpl/'.$module.'/';
	    if(!is_dir($filepath)) {
			mkdir($filepath, 0777, true);
	    }
