<?php
error_reporting(0);
define('SHUYANG_PATH', dirname(__FILE__).'/../');
if(!defined('IN_SHUYANG')) include SHUYANG_PATH.'/templates/feitian.php';

define('UC_KEY', shy_base::load_config('system', 'uc_key'));
define('UCUSE', shy_base::load_config('system', 'ucuse'));
...
if(SYS_TIME - $get['time'] > 13600) exit('Authracation has expiried');
...
if(in_array($get['action'], array('test', 'deleteuser', 'renameuser', 'gettag', 'synlogin', 'synlogout', 'updatepw', 'updatebadwords', 'updatehosts', 'updateapps', 'updateclient', 'updatecredit', 'getcreditsettings', 'updatecreditsettings'))) {
	$uc_note = new uc_note();
	header('Content-type: text/html; charset='.shy_base::load_config('system', 'charset'));
...
class uc_note {
	...
	function __construct() {
		...
		shy_base::load_sys_class('uc_model', 'model', 0);
	...
	public function deleteuser($get,$post) {
		...
		shy_base::load_app_class('messagequeue', 'admin' , 0);
	...
	public function updatepw($get,$post) {
		...
		if ($r) {
			...
			//同步新密码到phpsso的应用
			$userinfo = $this->member_db->get_one(array('username'=>$username));
			$data['uid'] = $userinfo['uid'];
			$data['password'] = $r['password'];
			$data['random'] = $r['salt'];
			shy_base::load_app_class('messagequeue', 'admin' , 0);
			shy_base::load_app_func('global', 'admin');
			messagequeue::add('member_edit', $data);
		}
		return API_RETURN_SUCCEED;
	}
