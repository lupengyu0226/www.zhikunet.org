function flashupload(uploadid, name, textareaid, funcName, args, module, catid, authkey) {
	...
	window.top.art.dialog({title:name,id:uploadid,iframe:'index.php?app=attachment&controller=attachments&view=swfupload'+args+setting,width:'500',height:'420'}, function(){ if(funcName) {funcName.apply(this,[uploadid,textareaid]);}else {submit_ckeditor(uploadid,textareaid);}}, function(){window.top.art.dialog({id:uploadid}).close()});
}
...
function insert2editor(id,in_content,del_content) {	
	...
	$.get("index.php?app=attachment&controller=attachments&view=swfdelete",{data: del_content},function(data){});
