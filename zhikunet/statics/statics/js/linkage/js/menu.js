...
function get_menu_child(nodeid,nodename,parentid,path,title,key) {
	...
	var url = "index.php?app=api&controller=index&op=get_menu&act=ajax_getlist&parentid="+nodeid+"&cachefile="+returnfile+"&path="+path+'&title='+title+'&key='+key;
...
function get_menu_parent(obj,parentid,path,title,key) {
	...
	var content = container = '';
	var url = "index.php?app=api&controller=index&op=get_menu&act=ajax_getlist&parentid="+linkageid+"&cachefile="+returnfile+"&path="+path+'&title='+title+'&key='+key;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			$.each(data, function(i,data){ 
				container = data.split(',');
...
function get_menu_path(nodeid,returnid,path) {
	var show_path = '';
	var url = "index.php?app=api&controller=index&op=get_menu&act=ajax_getpath&parentid="+nodeid+"&keyid="+returnfile+'&path='+path;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			$.each(data, function(i,data){ 
				if (data) {
...
function get_menu_val(nodename,nodeid,cachepath) {
	...
	var url = "index.php?app=api&controller=index&op=get_menu&act=ajax_gettopparent&id="+nodeid+"&keyid="+returnfile+'&path='+cachepath;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
		}
		if (returnfunc) {
