...
function get_child(nodeid,nodename,parentid) {
	...
	var url = "index.php?app=api&controller=index&op=get_linkage&act=ajax_getlist&parentid="+nodeid+"&keyid="+returnkeyid;
...
function get_parent(obj,parentid) {
	...
	var content = container = '';
	var url = "index.php?app=api&controller=index&op=get_linkage&act=ajax_getlist&linkageid="+linkageid+"&keyid="+returnkeyid;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			$.each(data, function(i,data){ 
				container = data.split(',');
...
function get_path(nodeid,returnid) {
	var show_path = '';
	var url = "index.php?app=api&controller=index&op=get_linkage&act=ajax_getpath&parentid="+nodeid+"&keyid="+returnkeyid;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			$.each(data, function(i,data){ 
...
function set_val(nodename,nodeid) {
	...
	var url = "index.php?app=api&controller=index&op=get_linkage&act=ajax_gettopparent&linkageid="+nodeid+"&keyid="+returnkeyid;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			$("#city").val(data);
		}
