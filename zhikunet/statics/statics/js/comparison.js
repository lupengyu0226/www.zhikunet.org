...
function remove_all(){
	...
	$("#relation_text").html("");
	$.get(ajax_url, {app:'yp', controller:'index', view:'pk', action:'remove', catid:catid, random:Math.random()});
	$("#relation").val("");
	c_sum();
};
$("#checkbox :checkbox").click(function (){
	...
	if(q.attr("checked")==false){
		$('#'+sid).remove();
		$.get(ajax_url, {app:'yp', controller:'index', view:'pk', action:'reduce', id:id, catid:catid, random:Math.random()});
		if(relation_ids !='' ) {
			var newrelation_ids = '';
		...
		if(r_arr.length>=num){
			...
			$.get(ajax_url, {app:'yp', controller:'index', view:'pk', action:'add', id:id, title:title, thumb:img, catid:catid, random:Math.random()});
...
function comp_btn() {
	...
	if($("#comp_num").text()>1){
		//alert(_rel);
		 window.open(ajax_url+"?app=yp&controller=index&view=pk&catid="+catid+"&modelid="+modelid+"&pk="+_rel);
	}else{
