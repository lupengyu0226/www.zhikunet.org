...
class content_output {
	...
	function copyfrom($field, $value) {
		...
		if($value_data) {
			...
			if(!empty($copyfrom_array)) {
				...
				return $imgstr."<a href='$value' target='_blank'>{$copyfrom_link['sitename']}</a>";
			}
		} else {
			return $value;
		}
				...
				if(is_array($server_list)) {
					foreach($server_list as $_k=>$_v) {
						if($value && is_array($sel_server) && in_array($_k,$sel_server)) {
							...
							if($downloadlink) {
								$a_k = urlencode(sys_auth("i=$this->id&s=$_v[siteurl]&m=1&f=$fileurl&d=$downloadtype&modelid=$this->modelid&catid=$this->catid", 'ENCODE', md5(SHY_PATH.'down').shy_base::load_config('system','auth_key')));
								$list_str[] = "<a href='".APP_PATH."index.php?app=content&controller=down&a_k={$a_k}' target='_blank'>{$_v[sitename]}</a>";
							} else {
		...
		if(is_array($file_list)) {
			foreach($file_list as $_k=>$_v) {	
				if($_v[fileurl]){
					...
					if($downloadlink) {
						$a_k = urlencode(sys_auth("i=$this->id&s=&m=1&f=$_v[fileurl]&d=$downloadtype&modelid=$this->modelid&catid=$this->catid", 'ENCODE', md5(SHY_PATH.'down').shy_base::load_config('system','auth_key')));
						$list_str[] = "<a href='".APP_PATH."index.php?app=content&controller=down&a_k={$a_k}' target='_blank'>{$filename}</a>";
					} else {
	...
	function map($field, $value) {
		...
		if($setting['maptype']==1) {
			$str = "<script src='https://app.mapabc.com/apis?&t=flashmap&v=2.4&key=$setting[api_key]&hl=zh-CN' type='text/javascript'></script>";
		} elseif($setting['maptype']==2) {
			$str = "<script type='text/javascript' src='//api.map.baidu.com/api?v=2.0&ak=B5dc604fc3c74d4f300e0fd97761ab75'></script>";
		}
		$str .= '<div id="mapObj" class="view" style="width: '.$setting['width'].'%; height:'.$setting['height'].'px"></div>';
		...
		function drawPoints(){
			...
			mapObj.centerAndZoom(new BMap.Point(lngX,latY),15);
			drawPoints();
			';
			$str .='
			function drawPoints(){
