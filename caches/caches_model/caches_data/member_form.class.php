...
class member_form {
	...
	function images($field, $value, $fieldinfo) {
		...
		$string .= $str."<div class='picBut cu'><a href='javascript:void(0);' onclick=\"javascript:flashupload('{$field}_images', '".L('attachment_upload')."','{$field}',change_images,'{$upload_number},{$upload_allowext},{$isselectimage}','member','','{$authkey}')\"/> ".L('select_picture')." </a></div>";
		return $string;
		...
		if(defined('IN_ADMIN')) {
			...
			<!--
				...
				function get_verify() {
					...
					$.get("index.php?app=api&controller=index&op=sms",{ mobile: mobile,random:Math.random()}, function(data){
						if(data=="0") {
