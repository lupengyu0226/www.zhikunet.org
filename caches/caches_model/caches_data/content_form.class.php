...
class content_form {
	...
	function catid($field, $value, $fieldinfo) {
		...
		$publish_str = '';
		if(defined('IN_ADMIN') && ROUTE_A=='add') $publish_str = " <a href='javascript:;' onclick=\"omnipotent('selectid','?app=content&controller=content&view=add_othors&siteid=".$this->siteid."','".L('publish_to_othor_category')."',1);return false;\" style='color:#B5BFBB'>[".L('publish_to_othor_category')."]</a><ul class='list-dot-othors' id='add_othors_text'></ul>";
	...
	function title($field, $value, $fieldinfo) {
		...
		$str = '<input type="text" style="width:400px;'.($style_color ? 'color:'.$style_color.';' : '').($style_font_weight ? 'font-weight:'.$style_font_weight.';' : '').'" name="info['.$field.']" id="'.$field.'" value="'.$value.'" style="'.$style.'" class="measure-input " onBlur="$.post(\'index.php?app=api&controller=index&op=get_keywords&number=5&sid=\'+Math.random()*5, {data:$(\'#title\').val()}, function(data){if(data && $(\'#keywords\').val()==\'\') $(\'#keywords\').val(data); })" onkeyup="strlen_verify(this, \'title_len\', '.$maxlength.');"/><input type="hidden" name="style_color" id="style_color" value="'.$style_color.'">
		<input type="hidden" name="style_font_weight" id="style_font_weight" value="'.$style_font_weight.'">';
		if(defined('IN_ADMIN')) $str .= '<input type="button" class="button" id="check_title_alt" value="'.L('check_title','','content').'" onclick="$.get(\'?app=content&controller=content&view=public_check_title&catid='.$this->catid.'&sid=\'+Math.random()*5, {data:$(\'#title\').val()}, function(data){if(data==\'1\') {$(\'#check_title_alt\').val(\''.L('title_repeat').'\');$(\'#check_title_alt\').css(\'background-color\',\'#FF0000\');} else if(data==\'0\') {$(\'#check_title_alt\').val(\''.L('title_not_repeat').'\');$(\'#check_title_alt\').css(\'background-color\',\'#3498DB\')}})" style="width:73px;"/><img src="'.IMG_PATH.'icon/colour.png" width="15" height="16" onclick="colorpicker(\''.$field.'_colorpanel\',\'set_title_color\');" style="cursor:hand"/> 
	...
	function image($field, $value, $fieldinfo) {
		...
		$html = '';
		if (defined('IN_ADMIN')) {
			$html = "<input type=\"button\" style=\"width: 66px;margin-top: 5px;\" class=\"button\" onclick=\"$('#".$field."_preview').attr('src','".IMG_PATH."icon/upload-pic.png');$('#".$field."').val(' ');return false;\" value=\"".L('cancel_the_picture','','content')."\">";
			}
	...
	function images($field, $value, $fieldinfo) {
		...
		if($value) {
			...
			if(is_array($value)) {
				...
				if($show_type && defined('IN_ADMIN')) {
					$list_str .= "<div id='image_{$field}_{$_k}' style='margin:2px;float:left;background: #eef2f2;border: 1px solid #dce6ea;zoom: 1;padding: 1px;width: 135px;height: 180px;text-align: center;overflow: hidden;'><input type='hidden' name='{$field}_url[]' value='{$_v['url']}'> <img src='{$_v['url']}' id='thumb_preview' width='135' height='135' style='cursor:pointer;'> <br> <br> <input type='text' name='{$field}_alt[]' value='{$_v['alt']}' style='width:80px;' class='input-text'> <a href=\"javascript:remove_div('image_{$field}_{$_k}')\">".L('remove_out', '', 'content')."</a></div>";
				} else {
		...
		if($show_type && defined('IN_ADMIN')) {
			$string .= $str."<div class='picBut cu'><a herf='javascript:void(0);' onclick=\"javascript:flashupload('{$field}_images', '".L('attachment_upload')."','{$field}',change_thumbs,'{$upload_number},{$upload_allowext},{$isselectimage}','content','$this->catid','{$authkey}')\"/> ".L('select_pic')." </a></div>";
		} else {
	...
	function slider($field, $value, $fieldinfo) {
		extract($fieldinfo);
        $setting = string2array($setting);
		if(!$value) $value = $defaultvalue;
		return '
        <div class="slider-info">
            ...
            <input type="hidden" name="info['.$field.']" id="'.$field.'-name" value="'.$value.'">
        </div>
...
<script type="text/javascript">
	...
	function map($field, $value, $fieldinfo) {
		...
		if(defined('IN_ADMIN')) {
		return '<input type="button" name="'.$field.'_mark" id="'.$field.'_mark" value="'.$tips.'" class="button" onclick="omnipotent(\'selectid\',\''.APP_PATH.'index.php?app=api&controller=index&op=map&field='.$field.'&modelid='.$modelid.'\',\''.L('mapmark','','map').'\',1,700,420)"><input type="hidden" name="info['.$field.']" value="'.$value.'" id="'.$field.'" >';
	    } else {
		return '<input type="button" name="'.$field.'_mark" id="'.$field.'_mark" value="'.$tips.'" class="button" onclick="omnipotent(\'selectid\',\''.PASSPORT_PATH.'index.php?app=api&controller=index&op=map&field='.$field.'&modelid='.$modelid.'\',\''.L('mapmark','','map').'\',1,700,420)"><input type="hidden" name="info['.$field.']" value="'.$value.'" id="'.$field.'" >';		
		}
	}
